...
> `topics` **15개** — 이번 배치에서 가장 많다([[higgsfield-repo]] 9 · [[PageIndex]] 12 · [[TensorRT-LLM]] 5 · [[mem0]] 14).

## 도메인별 추출 (ai-news)

- **신뢰도**: ⭐⭐⭐ — ★67,310 · MIT · **자체 논문 3편 참조** · LF AI & Data · 2년 2개월 연속 개발. 이번 배치 최상위.
- **즉시 활용**: **YES.** 볼트의 `/obsi-cleanmk`·`/obsi-cleanmk` 계열이 **웹 페이지**를 처리한다면 docling은 **로컬 문서**를 처리한다 — 겹치지 않고 보완한다. 오디오(WAV/MP3/WebVTT) 파싱이 붙어 있어 `/down-analysis` 트랜스크립트 경로와도 접점이 있다.
