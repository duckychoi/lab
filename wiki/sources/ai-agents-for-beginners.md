---
title: microsoft/ai-agents-for-beginners — MS 공식 AI 에이전트 입문 커리큘럼
type: source
domain: ai-news
tags: [ai-news, microsoft, education, agent-framework, jupyter, curriculum]
created: 2026-04-22
updated: 2026-05-20
sources: []
reliability: high
---

# microsoft/ai-agents-for-beginners

> [!insight] 핵심 인사이트
> Microsoft가 공식으로 에이전트 입문 커리큘럼을 오픈소스화했다. 58K 스타는 에이전트 프레임워크 학습 수요가 얼마나 큰지 보여준다. [[AI-에이전트-프레임워크]] 생태계의 교육 인프라 역할.

**GitHub**: https://github.com/microsoft/ai-agents-for-beginners  
**스타**: ⭐64,744 (+818 오늘, 2026-05-20; 이전 ⭐62,967)  
**신뢰도**: ⭐⭐⭐ (Microsoft 공식)

## 도메인별 추출

- **즉시 활용**: YES — 12강 Jupyter Notebook으로 에이전트 설계 전 과정 학습
- **6개월 영향력**: 에이전트 개발자 풀 확대 → 생태계 성숙 가속
- **액션**: 12강 커리큘럼 훑어보기, 특히 메모리·툴 사용 파트

## 관련 페이지
- [[AI-에이전트-프레임워크]]
- [[에이전트-메모리-레이어]]
- [[hermes-agent]]

## 원본
- 출처: https://github.com/microsoft/ai-agents-for-beginners
