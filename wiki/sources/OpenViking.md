...
# volcengine/OpenViking (GitHub ⭐31,304)

> [!update] 2026-08-21 갱신 — ⭐31,304 (전일比 +654·3만대 안착)
> GitHub ⭐**31,304**(2026-08-21 자동수집) ← 30,650(08-20). 전일 페이지값 대비 **실증분 +654**로 3만 돌파 후 **3만대 안착** — 에이전트 컨텍스트(메모리·리소스·스킬)를 `viking://` 가상 파일시스템 + L0/L1/L2 티어드 로딩으로 통합하는 [[에이전트-메모리-레이어]] 최신 구현의 채택이 대관문 통과 뒤에도 꾸준. 얇은 이식·핸드오프 레이어 [[ai-memory]]와 **"에이전트 메모리" 두 결(통합 DB↔이식 레이어) 병존** 지속. 위키(index→domain→page 계층) 쿼리 흐름과 동형인 티어드 로딩은 여전히 내 쿼리 엔진 토큰 절감 차용 후보. AGPLv3(코어) 상용 임베딩 라이선스 주의. reliability high 유지. *raw 자동수집 수치 — GitHub 실WebFetch 미수행. raw 일증분 필드 "+950"은 절대값(31,304)과 불일치(전일 30,650 대비 실증분 +654·절대값 기준 채택·raw 표기 병기).*
...
- 스타: ⭐31,304 (2026-08-21, 전일比 +654·3만대 안착·raw 표기 +950 불일치) ← ⭐30,650 (08-20, +923·3만 돌파·raw 표기 +804 불일치) ← ⭐29,727 (08-19, +213) ← ⭐27,604 (07-29 갱신, 26,604→27,604 당일 +129 WebFetch 실측), AGPLv3(코어)·Apache 2.0(CLI/예제)
