...
**지표**: ⭐**117,669** · 포크 **17,948** · 이슈 **13,741** · **Rust** · **Apache-2.0** · 생성 2025-04-13 · 최종 푸시 **2026-08-25**(당일) — 전량 **GitHub API 실호출 검증**(2026-08-25) · 트렌딩 데일리 **2위** ← ⭐116,429·포크17,749·이슈13,620(08-24) ← ⭐114,383·포크17,459·이슈13,516(08-23)

> [!update] 2026-08-25 갱신 — ⭐**117,669**(+1,240)·**이틀 +3,286**·트렌딩 1위 → 2위·**커밋+스타 동반 상승 지속**
> ⭐**117,669** (2026-08-25 **GitHub API 실호출 검증**) ← 116,429(08-24) ← 114,383(08-23). **이틀 +3,286**으로 증가가 이어지고, **당일 푸시**도 계속된다. 포크 17,749 → **17,948**(+199)·이슈 13,620 → **13,741**(+121). 트렌딩은 **1위 → 2위**([[free-claude-code]]에 밀림).
> **이 레포의 위치는 이번 배치에서 더 선명해진다.** GitHub 5건 중 유일하게 **"소스가 실제로 읽히는 코딩 에이전트 본체"** 다 — **Rust·Apache-2.0**으로 구현 전체가 공개돼 있다. 같은 배치의 [[andrej-karpathy-skills]](CLAUDE.md 텍스트 한 개·커밋 0건 4개월)·[[free-claude-code]](접근 브리지·핵심 주장 미검증)와 비교하면, **스타의 성질이 다르다**: 저쪽은 북마크에 가깝고 이쪽은 **매일 커밋되는 코드베이스에 대한 추적**이다.
> ⚠️ **오픈이슈 13,741은 절대값으로 크지만 해석은 [[hermes-agent]](35,603)와 달라야 한다.** 포크당 이슈가 codex는 **0.77건**, hermes-agent는 **0.75건**으로 사실상 비슷한데, codex는 **당일 푸시로 계속 대응 중**인 반면 hermes-agent의 활동성은 이번 회차에 확인되지 않았다. **이슈 절대값이 아니라 "이슈 대비 처리 활동"이 신뢰성 신호**다.
> **즉시 활용 관점**: 코딩 에이전트를 직접 구현하거나 하네스 동작을 이해하려는 입장이면, **추측 대신 Rust 소스를 읽는 것**이 가능한 몇 안 되는 선택지다(08-23 actionable에 기록된 *"하네스 안전 규칙을 문서가 아닌 코드로 확인"* 항목의 근거).
