...
> [!note] 🔄 2026-09-04 판정 갱신 — "초기 단계" 폐기, 가설은 검증됨
> 이 페이지가 2026-08-12에 세운 가설(*"오픈 i2v가 베이스→재패키지→**가속 LoRA** 파생 계층을 형성 중"*)은 **3주 만에 검증됐다**. 단 **이 항목(larryvrh, DL 669)이 아니라 다른 제작자**가 그 자리를 가져갔다 — **[[Minimax-h3-Turbo-lightx2v]]** 가 **DL 1,117,509**(베이스 [[MiniMax-H3]] 5,118,457의 **21.8%**)로 올라섰다.
> → **"초기 단계" 판정은 계열 전체에 대해서는 폐기**한다. 다만 **이 페이지의 항목 자체는 여전히 DL 669 수준**이며, 당시 트렌딩 3위였음에도 승자가 되지 못했다 — *"순위와 사용은 다른 것을 잰다"* 의 추가 사례. **원 서술은 보존**한다.
> ⚠️ 08-12에 미검증으로 남긴 **가속 폭·품질 손실·ComfyUI 호환성**은 **3주 뒤에도 여전히 미검증**이다. 채택이 1,670배 커졌어도 **근거는 커지지 않았다.**
