...
## 🔄 2026-09-24 갱신 — DL 델타 + **v1 창≠누적 발견 (볼트 09-22 기록 정정)**

**볼트 실측** (v2 본체 + v1 대조군 동시 조회):

| 저장소 | createdAt | 경과 | 30일 창 | **누적** | 좋아요 | trendingScore |
|---|---|---|---|---|---|---|
| `prism-ml/Ternary-Bonsai-2-27B-gguf` (v2) | 2026-09-16 | **8일** | 2,815,979 | **2,815,979** | 1,976 | **1,806**(트렌딩 3위) |
| `prism-ml/Ternary-Bonsai-27B-gguf` (v1) | 2026-07-04 | **82일** | 653,298 | **1,879,078** | 1,393 | 70 |

### 🔴 볼트 기록 정정 — v1 수치는 **창값**이었다
이 페이지 100행이 기록한 *"대조군 v1 실측: 다운로드 **662,554**"* 는 **30일 창값**이다. **누적은 1,879,078** — **2.84배 차이.**
📌 그리고 이 차이가 바로 [[지표-창길이]] 의 창 길이를 역산하게 해 줬다: 653,298 / 1,879,078 = **34.77%**, 82일 × 0.3477 = **함의 창 ≈ 28.5일**. (⬜ 균일 다운로드율 가정 필요)

### 🎯 v2 는 창 = 누적 — 결측이 아니라 **나이**다
v2 는 **8일 된 저장소**라 30일 창이 생애 전체를 덮는다. 09-23에 이 페이지에서 1건으로 본 것이 09-24 소급 점검에서 **11/11 로 확인**됐다: **30일 이내 생성 저장소는 예외 없이 `downloads == downloadsAllTime`.**
→ **두 값이 같으면 `null` 이나 오류로 읽지 않는다. "창이 생애 전체를 덮음"이라 적는다.**

### 세대 교체 속도
v2 누적 **2,815,979**(8일) vs v1 누적 **1,879,078**(82일) → **v2 가 1.50배**. 🎯 **8일 된 v2 가 82일 된 v1 의 누적을 이미 50% 초과했다.** 일평균으로는 v2 351,997/일 vs v1 22,916/일 = **15.4배**.
📌 세대 관계이므로 [[원본-파생-역전]] 표에는 계속 **제외 유지**(원본/파생이 아님). 단 **[[검사가능성-후퇴]] 사례 1**(v1의 `.eval_results/` 3종을 v2에서 제거하며 점수 주장은 올렸다)이 여전히 유효하고, **그 상태로 채택이 15배 빠르다**는 점이 기록될 가치가 있다 — **검사 가능성 후퇴가 채택을 막지 않았다.**

델타(09-23 → 09-24): 누적 2,569,604 → **2,815,979** (**+246,375**) · lastModified **2026-09-24**(오늘 갱신).
⚠️ 확인 범위: **API 메타데이터만**(downloads·downloadsAllTime·likes·trendingScore·createdAt·lastModified). 🔴 모델 카드 미열람 · 🔴 화이트페이퍼 PDF 미파싱(09-20 actionable 여전히 대기).
