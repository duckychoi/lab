...
## 🔄 2026-09-23 갱신 — **창 가설의 결정적 증거를 제공한 표본**

> [!insight] 🎯 이 레포가 **누적 축의 정합성**을 확정시켰다
> 생성 **2026-09-16** → 09-23 조회 시점 **7일 차**. 창 길이보다 어리다.
> - `downloads`(최근 창): **2,569,604**
> - `downloadsAllTime`(누적): **2,569,604** ← **정확히 일치**
> 📌 **레포 나이 < 창 길이이면 두 값은 같아야 한다.** 예측이 정확히 맞았고, 성숙한 레포 2건([[Qwen3.8-27B]] 7.08M<9.68M · [[Qwen3.8-27B-GGUF]] 7.02M<13.99M)에서는 벌어진다. **3표본 전부 정합** → 새로 연 `downloadsAllTime` 축이 기존 30일 창 해석과 **모순 없이 맞물린다**는 검증(창 해석 자체는 볼트가 09-06부터 보유).
> ✅ 볼트 실측(HF API 직접 조회, `?expand[]=downloadsAllTime`) · 좋아요 **1,906** · Apache-2.0 · 최종수정 2026-09-17 · 파일 10개

> [!note] 📉 이전 기록과의 관계
> 볼트 기록 1,908,396 → **2,569,604**. 이 레포는 **아직 누적=최근 창**이므로 이 차이는 **진짜 증가(+661,208)** 로 읽어도 된다. 🎯 **같은 배치의 다른 두 모델과 달리 여기서만 증분 해석이 유효하다** — 창 길이를 알면 언제 증분으로 읽어도 되는지도 알 수 있다.

> [!insight] 📊 분포 지표 동반 — **볼트 요청 2를 충족한 이번 배치 유일 항목**
> 14개 thinking 벤치 **평균 84.78**, 내역: **수학 96.57 · 코딩 89.42 · 에이전틱 툴콜 74.92**.
> 🎯 **평균이 가리는 최저점이 에이전틱(74.92)** 이다. 평균만 봤다면 "84.78 = FP16의 98.2%"로 끝났을 텐데, 내역이 있어 **에이전트 용도에는 조심해야 한다**는 판단이 가능하다.
> 📌 같은 배치 [[ScriptMoE]](10 스크립트 평균 82.06%, 내역 없음) · [[HyperQ]](벤치 개수조차 불명)와 **정면 대비**. → [[표-부분인용]] 의 좋은 사례로 등록.
> 대조군: IQ2_XXS **72.59** · UD-Q4_K_XL과 **0.4점 차**(3배 용량).

> [!warning] ✅ 기존 정정 유효 재확인
> *"1.72 bit / 5.9GB"* 는 **`(ideal)` 행**이고 **출하물은 PTQ1_0 1.75bit/5.95GB · PQ2_0 2.13bit/7.21GB.** *"~47 tok/s(M5 Max)"* 는 **PQ2_0** 쪽. *"9.3배 축소"* 도 `(ideal)` 표기.
> 구성: Qwen3.8-27B를 **전 구간 삼진(ternary)** 으로 내린 llama.cpp용 GGUF(임베딩·어텐션/MLP 투영·LM head 전부, **비전 타워만 Q8_0 별도 mmproj 팩**). 262K 컨텍스트를 백본 하이브리드 어텐션(~75% 선형)으로 유지.

## 관련 페이지 (추가)
- [[지표-창길이]] — 이 페이지가 결정적 증거를 제공
- [[표-부분인용]] · [[Prism-ML]] · [[Qwen3.8-27B]]
