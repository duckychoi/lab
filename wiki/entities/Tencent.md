...
> [!insight] 2026-09-10 — 에이전트 **설정 배포**까지 내려왔다 ([[teamai-cli]])
> `teamai-cli`(⭐**3,387**·당일 +556·TypeScript)는 skills/rules/MCP/docs를 공유 git 레포에 두고 **`push → MR 리뷰·머지 → SessionStart 훅이 pull`** 로 배포하는 하네스 동기화 도구다. **에이전트 설정에 코드리뷰를 붙인다**는 발상.
> → Tencent 패턴이 한 칸 더 내려왔다: 실행 런타임([[CubeSandbox]]) → 보안([[Multi-Layer-Agent-Red-Teaming]]) → 모델·메모리([[Hy3]]·[[TencentDB-Agent-Memory]]) → **설정 배포**([[teamai-cli]]). 에이전트 스택의 **거의 모든 층**을 채웠다.
> ⚠️ **완성도는 낮다** — 3층 중 **2층이 beta**(Team Context·Team Improvement), 지원 11종 중 **전체 커버리지는 5종뿐**, 벤치마크 없음.
> 🔴 **볼트가 raw 판정을 반증한 건**: raw는 *"실제 LICENSE는 Tencent 자체 라이선스"* 라 했으나 **LICENSE 원문은 MIT 전문을 그대로 싣는다**. GitHub `NOASSERTION` 은 Tencent 인사말 2줄이 자동탐지를 깨뜨린 결과였다 → [[파생표기-함정]]
...
## 관련 페이지
- [[teamai-cli]] — 팀 단위 에이전트 하네스 동기화 CLI (2026-09-10)
