...
## 🔄 2026-09-23 — 측정 규칙 개정: **누적 축이 열렸다**

> [!insight] ✅ 이 페이지 27·32행(30일 창 규칙, 09-15)은 **유효하다** — 수집기가 09-23에 "최대 발견"으로 되가져온 내용은 볼트가 이미 보유한 것이다.
> 그러나 **동반 기록이던 *"`downloadsAllTime` 은 null로 미제공 = 교차검증 경로 없음"* 은 틀렸다.**
> ✅ 실측: **`GET /api/models/{id}?expand[]=downloadsAllTime`** 을 쓰면 값이 온다(3/3).
> Qwen3.8-27B **9,681,199** · unsloth GGUF **13,993,047** · Ternary-Bonsai **2,569,604**.

> [!action] 개정된 HF 측정 규칙
> 1. 모델 조회는 **`?expand[]=downloadsAllTime` 필수** · 두 값 **항상 병기**(`N(30일) / M(누적)`)
> 2. `A→B` 증분은 **누적값에서만**. 30일 값 차이는 **"창 이동"**
> 3. **동결·감소를 "관측 실패"로 남기지 않는다** — 누적 축으로 교차 확인
> 4. 창 길이 **"30일"은 ⬜ 문서 미확인**(공식 `models-download-stats.md` 전문 3,934B에 기간 서술 없음) — 관측 기반 통용값으로 표기
> 📌 이 페이지의 규칙이 **09-22 로그에서 지켜지지 않았다**(맨 증분 표기) → [[지표-창길이]] 에 자기 한계로 기록.

## 관련 페이지 (추가)
- [[지표-창길이]] · [[메타데이터-부재-추론]] · [[원본-파생-역전]]
