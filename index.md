...
title: Wiki Index
updated: 2026-09-24
total_pages: 1296  # 2026-09-24 실측(sources 1154 · entities 82 · concepts 54 · synthesis 2 · domains 4 · queries 0) ← 09-23 1282
total_sources: 1154  # 2026-09-24: 09-24 자동수집 13건 → **신규 9 · 갱신 4**(+보너스 갱신 1 [[TradingAgents]]) · 신설 엔티티 4 · 개념 1 · 도메인 재판정 **3**([[HappyWorld-Bench]]·[[Spatial-Interactor]]→`slam-3dgs` · [[ComfyUI]]→`video-saas`). 🎯 **배치 최대 성과 — 09-23에 미수행으로 남긴 actionable을 집행했고 그 결과가 09-23의 예측을 기각했다**: [[원본-파생-역전]] 7쌍 전수 누적 재조회 → **방향 역전 0건**(파생은 대부분 30일 이내 생성이라 창=누적, 구조적). 부산물로 **창 길이를 28.5·31.4일로 역산**(⬜ 균일율 가정) → 볼트 ⬜ *"30일 문서 미확인"* 이 *"실측 2건과 정합"* 으로 등급 상승. 🔴 **수집기 정정 3건**: ① [[ComfyUI]] *"본체 누락 4번째"* → **08-11부터 페이지 존재**(301 리다이렉트가 중복검사 무력화 → [[정규명-우선-중복검사]] 신설) ② [[nasiko]] `NOASSERTION` → **LICENSE 196행이 Apache-2.0 원문**(탐지 실패이고 [[treg]]는 진짜 커스텀 — **한 값이 두 실체를 접는다**) ③ [[TradingAgents]] ★96,723(볼트)·76k+(래퍼 인용) → **실측 108,376 · 오늘 푸시** = 볼트 *"5주 정체"* 인사이트 **만료**. ✅ **독립 검증 15/15 통과**(GitHub 5 · HF모델 5 · 논문 5), **README 4건 전문 대조 결과 수집기 인용 축자 일치**, 한정어 보존 **9배치 연속**. 🎯 **배치 축: 계측** — GitHub 5건 중 **4건이 관측성 내장**(토큰/비용 자동수집·라이브 대시보드·trace export·노드별 비용) ← 이전: 09-23 자동수집 13건 → 신규 9 · 갱신 4 · [[지표-창길이]] 신설 · [[원본-파생-역전]] 반례 판정이 지지 사례로 뒤집힘
---
...
- [[Nasiko-Labs]] — [[nasiko]](★8,841·Rust) 개발. 🎯 **볼트가 이 조직 때문에 규칙을 고쳤다** — API `NOASSERTION` 을 *"조건 불명"* 으로 읽었으나 **LICENSE 196행이 Apache-2.0 원문**(`Copyright 2026 Nasiko Labs` 기입)이었다. 같은 값을 받은 [[treg]]는 진짜 커스텀 → **한 값이 두 실체를 접는다**. topics 에 `agent-security`·`mcp-gateway` 자칭. ⬜ 법인 실체·국적 미확인(README 742행 전수에 조직 소개 없음) *(NEW 2026-09-24)*
- [[vercel-labs]] — [[agent-browser]] 제작. README `LABS-PRODUCT` 공식 배지 실측. 🎯 **볼트 GitHub 소스 중 npm 실채택 최대** — 월 **5,326,951** DL · 127버전. 🔴 **그런데 ★ 당일 증분은 +62** → 볼트 *"★≠활성도"* 관측의 **반대 방향 최강 사례**(인지는 멎고 사용은 폭발). `skills.sh` 배지 보유 → [[에이전트-스킬]] 배급 채널 편입. ⬜ Vercel 본사와의 조직 관계 미확인 *(NEW 2026-09-24)*
- [[lahfir]] — [[agent-desktop]](★1,621) 제작 **개인 계정**. 🎯 **규모는 최소, 문서 규율은 상급** — 명령을 *"이름 58 / 동작 54"* 로 쪼개고, 못하는 4개를 `ACTION_NOT_SUPPORTED` 로 fail-closed, 권한을 3값(`unknown` = *"프롬프트 없이 확인 불가"*), 플랫폼을 7×3 표로 `Planned` 명시. 🔴 **단 유일한 수치를 `<img alt=>` 속성에 뒀다**(30,743→383 = 98.75%, 헤드라인 78–96%보다 위). ⬜ 실명·소속 미확인 *(NEW 2026-09-24)*
- [[TNT-Likely]] — [[PanWatch]](★1,717·MIT) 제작. **남의 프레임워크([[TradingAgents]])로 제품을 만든 첫 사례** — 볼트 [[금융-AI]] 축의 성격을 프레임워크에서 제품으로 확장. ✅ **수익 주장을 하지 않는다**(README 272행 전수 확인) → 볼트 *"수익 주장 미검증"* 경고 해당 없음. 🔴 인용한 본체 ★가 3.2만 낡음(*"76k+"*). ⬜ 개인/법인·국적 미확인(중국어 README·A주 우선은 **추정**) *(NEW 2026-09-24)*

- [[agent-substrate]] — [[substrate]] 개발 조직. 🎯 **구글이 의존하는데 구글이 아니다** — README에 *"공식 지원 Google 제품 아님 · Google OSS VRP 대상 아님"* 명시인데 [[ax]](`google/ax`)가 이 위에서 돈다(ax README 13·72행 실측). 🔴 **VRP 대상 아닌 코드가 샌드박스 격리를 담당.** ⬜ 구글과의 조직 관계 미확인 *(NEW 2026-09-23)*
- [[dream-num]] — [[univer]](★15,934) 개발. 📌 **레포 생성 2022-09-29 = 4년 된 오피스 SDK가 "The Office Harness for AI Agents"로 재포지셔닝.** 코드가 새 것이 아니라 포지션이 새 것. ⬜ 에이전트 전용 기능 실재 여부 미확인(README 405행 전문 미열람) *(NEW 2026-09-23)*
- [[superdesigndev]] — [[treg]](★2,398) 개발. 🔴 라이선스 **NOASSERTION**(SPDX 미식별, 볼트 API 실측 재확인) = 재배포 조건 불명. ⚠️ 자체 호스팅 treg.to 결합 → 포크해도 기능하지 않을 가능성 *(NEW 2026-09-23)*
...
- [[VectifyAI]] — `VectifyAI`/pageindex.ai. **엔진+러너+데이터 세 레포**를 나눠 낸다([[PageIndex]] ★35,766 · PageIndex-OSS-Benchmark · MMLongBench-Doc-V2). ✅ 벤치 설계에 규율 — *"a wrong answer is a **retrieval or reading failure, not a reasoning one**"*. ⚠️ **자기 데이터셋으로 자기를 잰다**([[m-a-p]] 와 같은 형태) 🎯 **단 러너·데이터·결과를 전부 열어 둔 "열린 자기평가"**. 🔴 정확도는 **그림 안에만** *(NEW 2026-09-20)*
- [[Docling-Project]] — Switzerland · repos 27. [[docling]](★67,310·MIT) 운영. ✅ **README에 LF AI & Data 배지** — 볼트 파싱/데이터 도구 중 **재단 거버넌스 첫 사례**. ✅ **포맷을 논문으로 정의**(arXiv 2408.09869 · DocTags 2503.11576 · DocLayNet 2206.01062). VLM 경로가 `ibm-granite/granite-docling-258M` + 위치 Switzerland → IBM Research Zurich 계열로 보이나 ⚠️ **조직 설명란이 비어 소속은 미확인**. 🔴 README에 정확도 표 0개 *(NEW 2026-09-20)*
- [[mem0ai]] — mem0.ai. **에이전트 메모리의 사실상 기준점**([[mem0]] ★65,688 · 3년 3개월). 🔴 **볼트가 8개월간 페이지 없이 자로만 써 왔다.** 오픈코어 3경로(OSS/셀프호스트/매니지드)이고 🔴 **벤치 점수는 매니지드 플랫폼의 것**(README 자인). ✅ **평가 하네스는 별도 오픈소스** → 🎯 **자는 열고 피측정물은 닫았다.** 🎯 **스킬 하나가 문자 그대로 "오픈소스 → 유료 플랫폼 이전"**(`mem0-oss-to-platform`) — 스킬 배급 축에서 **상용 퍼널 첫 사례** *(NEW 2026-09-20)*

- [[TokenRhythm]] — Qwen3.5 계열을 **에이전틱 사후학습**으로 재조정해 배포([[NeoHorse-1-4B]] 4.21B·[[NeoHorse-1-9B]] 8.95B, 둘 다 apache-2.0·2026-09-05 동시 공개). 🎯 **자리의 성격**: [[Comfy-Org]]=재포장 < [[DavidAU]]=양자화 < TokenRhythm=**사후학습** — 셋 다 "모델을 만들지 않는 모델 공급자"이고 **개입 깊이가 다르다.** ✅ 라벨 정직(`base_model:finetune` 실측 일치, [[파생표기-함정]] 아님). ✅ **모논문 인제스트 완료 → [[NeoHorse-1-Paper]]**(09-19 · 업보트 **170** 재측정, 09-18 기록 421 정정 · ★599는 학습코드 없는 README 레포). **하네스가 학습 데이터 생산 장치**인 유일 사례 — 매 턴의 능력수요·서비스 티어·상호작용을 기록해 **구조 검증 + 6차원 의미 평가**로 학습 예제화. ✅ **소속 확인(09-19)**: TokenRhythm Technologies + Infinigence AI·칭화대·베이징대·CUHK·[[Alibaba]]·VC 2곳, 교신 Yunhe Wang·Yu Wang. 🔴 국적·규모는 여전히 미확인 *(NEW 2026-09-18 · 갱신 09-19)*

- [[Cloudflare]] — 엣지·보안 인프라 기업. AI 축에서는 **모델을 만들지 않고 에이전트의 판정 절차를 만든다.** [[security-audit-skill]](★8,529·MIT) 제작 — 핵심 발명은 판정 3분류를 **스키마로 강제**하는 것: `needs_validation` 은 *"an exact unresolved fact and **no severity**"* 🎯 **심각도를 붙일 수 없는 칸을 따로 만들었다**(등급이 아니라 범주). ✅ 자기 실행 조건의 실패 모드까지 선언(샌드박스 없으면 *"keeps the lead as `needs_validation` instead of executing target code"*). 🔴 탐지율 수치는 레포에 없음 · `topics` 0개 *(NEW 2026-09-17)*

- [[DavidAU]] — HF 커뮤니티 양자화·탈검열 재배포자. 🔍 **볼트 실측: 트렌딩 상위 200에 이 저자의 `Qwen3.8-27B` 파생이 최소 5건**(9·33·137·138·151위)이고 명명이 **시리즈**다(`Cold-Fusion-NNN`). 🔴 볼트는 이미 [[DavidAU-Qwen3.8-27B-TURBO]](735-882)를 보유한 상태에서 709를 받았다 — **리터럴 grep 0히트** → [[파생저장소-식별]] 핵심 사례. reliability **low**(arc-c 709/701/591의 **분모·하네스 미공개** · *"Closed source level of intelligence"*). 🎯 **다만 단계별 표 4개를 정직하게 다 적었고 우선순위까지 밝혔다** — 09-17 오류는 그 정직성을 못 읽은 데서 나왔다 *(NEW 2026-09-17)*

- [[Comfy-Org]] — ComfyUI 공식 조직. **모델을 만들지 않고 런타임에 맞게 재포장한다**([[Comfy-Org-YuE2]] 카드 본문: *"Repackaged model files for ComfyUI"*). [[Vercel]] 이 스킬 배급층을 가져간 것과 **같은 종류의 자리**. ✅ 파생 관계를 본문에 명시. 🔴 볼트 발견 3건: 채택 근거 `int8_convrot` **미문서화** · `tags` 에 [[YuE2-3B]] 누락 + `finetune` **오라벨**(실제 재포장) · **라이선스 분기**(CC-BY-NC-4.0) *(NEW 2026-09-17)*
...
- [[Edge0]] — **35B를 줄이지 않고 메모리 계층을 바꾸는** 엣지 추론 조직(`Edge0` · MLX). 4비트 전문가를 스토리지에 두고 **prerouter로 한 스텝 선예측 스트리밍** → 활성 3GiB. ⚠️ 조직 실체 미확인 · 벤치 자체측정 · **Apple Silicon 전용** · `preview`. 가중치+프레임워크 실공개로 [[IFM]](low)보다 높은 medium *(NEW 2026-09-13)*

- [[vxcontrol]] — [[pentagi]](⭐23,624·Go·MIT) 제작. 🎯 README에 `Current Capability Boundaries` 절을 따로 두고 *"autonomous **and assistant-guided**"·"not BAS"·"conceptual or future work"* 를 직접 적는다. ⚠️ 다만 **같은 README 63행은 여전히 "Fully Autonomous"** — 경계를 적는 습관과 헤드라인을 고치는 습관은 별개다 *(NEW 2026-09-13)*

- [[SnailSploit]] — [[Claude-Red]](⭐3,788·MIT) 제작. ✅ **배지 `skills-78`/`categories-23` 가 git tree 실카운트와 정확히 일치** — [[vercel-skills]] 이후 대조해 온 배지 중 **첫 완전 일치**. 🎯 [[CloddsBot]] 과 같은 형식·같은 자릿수인데 한쪽만 셀 수 있게 만들어 뒀다 = **습관의 문제** *(NEW 2026-09-13)*

- [[Remco Hendriks]] — [[MetroLLM-Bench]] **단독 저자**. 26모델·955케이스를 혼자 평가하고 **같은 초록에 반증 5개**(27B에서 -0.91 · 규칙엔진 84.6 · 종합 1위는 타 모델)를 직접 적는다. ⚠️ 그럼에도 **75/25 split과 헤드라인을 나란히 놓지 않았다** → [[분포내-우위]]. **정직한 저자도 배치는 놓친다** *(NEW 2026-09-13)*

- [[alphaXiv]] — arXiv 위에 토론·리뷰 층을 얹는 서비스. [[OpenResearch]](Rust·MIT) 제작. 🎯 오늘 리서치 에이전트 3건 중 **논문 유통을 실제 운영해 본 유일한 팀**. 오픈소스+호스팅 병행 *(NEW 2026-09-12)*

- [[m-a-p]] — 오픈 음악 생성 [[YuE2-3B]] + 평가 벤치 **WildSongBench** 를 **함께** 내는 집단. ⚠️ **자기 모델을 자기 벤치로 잰다** — 카드가 Suno만 비교군으로 지목하고 실제 2위 Mureka 9는 표 안에만 뒀다 → [[한정어-탈락]]. 가중치 **CC BY-NC** *(NEW 2026-09-12)*

- [[nex-agi]] — [[Nex-N2.5-mini]](35B)·[[Nex-N2.5-Pro]](397B)·Max(1.6T) **3티어 에이전트 모델** 조직(Apache-2.0·`qwen3_5_moe`). *"비전은 입력 양식이 아니라 **환경을 인지·검증하는 인터페이스**"*. 🔴 문서 결함 3종(**패밀리 공용 카드 md5 동일** · `pipeline_tag` 자기모순 · 활성 파라미터 미기재) → [[단위-불일치]]. 조직 실체 미확인이나 **가중치 실공개+실사용 흔적**으로 medium *(NEW 2026-09-11)*

- [[Vercel]] — 프론트엔드 플랫폼. AI 축에서는 **자기 에이전트를 만들지 않고 배급 층을 가져갔다** — [[vercel-skills]] ⭐31,337로 79개(자기보고) 에이전트에 스킬을 설치·실행. 2026-09-11 **레지스트리 → 패키지 매니저**로 이동(`skills use` 무설치 파이프·와일드카드) *(NEW 2026-09-11)*

- [[SenseTime]] — 중국 멀티모달 대기업. **중간 표현을 계속 지우는 노선**([[sensenova-u1]] NEO-unify → [[SenseNova-U1.5]] **encoder-free·VAE-free**). 🪞 볼트의 *"오픈소스 가능성 낮음"* 예측이 **빗나감**(U1.5가 학습 코드 공개 예고) → **벤더 정체성에서 정책 추론 금지** *(NEW 2026-09-11)*

- [[pascalorg]] — [[Pascal-Editor]](⭐23,176·MIT·WebGPU) 제작 조직. **MCP를 GUI 앱 조작 인터페이스로 쓴 첫 본격 사례**. 로컬 우선(계정·API키 불필요·자동업로드 없음). 🎯 스킬 2개뿐이나 `furniture-fit` 이 **툴 스키마를 런타임에 선조회해 없으면 좁은 결과를 보고** → [[자기제한-명시]] *(NEW 2026-09-10)*

- [[MiniMax]] — 옴니모달 [[MiniMax-H3]]의 벤더. **"오픈 가중치 ≠ 오픈 시스템"** 대표 사례(품질 결정 모듈 `H3-Context-IR` 비공개·인코더는 [[Alibaba]] Qwen3-VL-32B) *(2026-08-29 신규)*
<!-- 인물, 기업, 제품, 프로젝트 -->
...
- [[IFM]] — ✅ **09-19 정체 확인: [[MBZUAI]] 산하 Institute of Foundation Models**(아래 "미확인" 기록은 이력) · [[K2-Horizon-MoVA-36B-A4B]](36B/활성4B MoE+MoVA) 공개 조직. 🔴**조직 실체 전혀 미확인** · 벤치 전부 자체보고 · 코드·체크포인트 예고만 → reliability **low** *(NEW 2026-09-09)*
...
- [[browser-use-org]] — 에이전트 웹접근 표준 제작 조직. 본체 [[browser-use]] ⭐113,014·MIT·22개월 연속 개발. **볼트가 파생([[video-use]])만 두 번 잡고 본체를 10개월 놓친 사건의 당사자** *(NEW 2026-09-08)*
- [[HeyGen]] — AI 아바타·영상 SaaS 기업. **모델은 닫고 렌더 층([[hyperframes]] ⭐47,070)을 Apache-2.0으로 개방** — 오픈코어의 "층 분할" 변형, 설명이 *"Built for agents"* *(NEW 2026-09-08)*
- [[k2-fsa]] — 오픈 음성(ASR/TTS) 조직. [[OmniVoice]] 600+ 언어 음성복제(HF DL 115만·⭐10,549·Apache-2.0·**arXiv 보유**). **논문+코드+가중치+데모 네 표면을 세트로 내는 조직** *(NEW 2026-09-08)*

---

- [[Fastino]] — [[GLiNER2]] 레포 + HF `fastino`(체크포인트 9종) 운영, 오픈 가중치와 상용 API 병행. 🔴 기본 설치가 클라우드 클라이언트 · 2.5 품질 수치 0 · 카드 594MB FP16 ↔ 실물 FP32 1.15GB *(NEW 2026-09-19)*
- [[Stable-AI]] — [[LimiX-2]] 개발사. 🔴 **Stability AI 아님**(법인 稳准智能(雄安)科技有限公司, HF `stable-ai`). 칭화대 공동연구 · LimiX-1 Apache → LimiX-2 비상업 *(NEW 2026-09-19)*
- [[MBZUAI]] — [[IFM]] 모기관(Institute of Foundation Models at MBZUAI). LLM360 이래 완전 개방 노선 · [[K2-Horizon-7B]] 체크포인트 69개 실존 · 🔴 이번엔 코드가 가중치보다 늦다(xllm 빈 레포) *(NEW 2026-09-19)*
- [[China-Telecom]] — TeleChat·[[Xing4.0-29B-A4B]] 발행, Ascend+MindSpore 학습이 정체성. 🔴 "첫 모델"은 크기 한정어 필수(전작 TeleChat3-MoE 105B~1T 이미 Ascend) *(NEW 2026-09-19)*
- [[DeepCybo]] — [[PhysBrain]] → [[ActionPiece]] → [[PhysBrain-1.5]] 임바디드 노선(중관촌 계열). 🔴 버전이 오르며 평가가 제어 → 이해로 좁아짐 · 레포 코드 0 *(NEW 2026-09-19)*
- [[Stanford-CRFM]] — [[marin]] 공동 주도(Open Athena 와). 실패·오염·패배를 수치로 공개하는 오픈 개발 · 역방향 오염 사례 → [[포맷-불일치-오염]] *(NEW 2026-09-19)*
## concepts

- [[정규명-우선-중복검사]] **[개념]** — **리다이렉트를 따라가 정규명을 확정한 *뒤에* 중복 검사를 해야 한다. 순서가 뒤바뀌면 이미 가진 것이 신규로 들어온다.** [[파생저장소-식별]]이 *과다 포함*(다른 것이 같아 보임)을 다루면 이쪽은 *중복 포함*(같은 것이 달라 보임)이다. 발단: 수집기가 [[ComfyUI]] 301 리다이렉트를 **스스로 발견하고 정규명으로 API를 조회했는데** 중복 검사는 구 이름으로 안 돌려, **08-11부터 있던 페이지를 "본체 누락 4번째"로 재배달**했다 — 🎯 **한 단계 더 정확해졌기 때문에 어긋났다.** 더 깊은 층: **레포 README가 자기를 구 이름으로 자칭한다**(`Release Process` 3레포 중 Core만 `comfyanonymous`) → 볼트 페이지가 구 이름인 것은 낡음이 아니라 **당시 정답**. 🎯 **리다이렉트는 양방향 오류를 만든다** — 추적 안 하면 살아 있는 레포를 버리고(`abenzerps` DL 0 제외, 실제로는 개명), 추적하고 중복검사를 안 돌리면 가진 것을 또 받는다([[ComfyUI]]). 규약 5개(정규명 우선 · 두 이름 모두 grep · **레포 ID 대조**(ComfyUI=589831718) · 제목은 정규명·별칭은 본문 · 자칭명과 API명 병기). 🔴 표본 3, 전부 이름변경 계열 — **포크·미러 중복은 못 잡는다** *(NEW 2026-09-24)*

- [[지표-창길이]] — 🎯 **수집기의 "최대 발견"을 반박하고 그 자리에서 진짜 발견을 얻은 사례.** 수집기가 제시한 *"HF `downloads` 는 30일 창"* 은 볼트가 **09-06 관측 · 09-15 [[HuggingFace]] 27·32행에 이미 규칙화**한 내용(과장 정정). 🔴 **정작 틀린 건 볼트 자신의 기록** — *"`downloadsAllTime` null, 교차검증 경로 없음"* ([[Qwen3.8-27B-GGUF]] 85행) → **`?expand[]=downloadsAllTime` 로 값이 온다**(3/3). ✅ 가장 어린 레포([[Ternary-Bonsai-2-27B]], 7일 차)에서 두 값 **정확히 일치**해 내부 정합 확인. 🔀 파급: [[원본-파생-역전]] 반례가 **지지 사례로 역전**(누적 1.445배). 🔴 자기 한계: 09-15 자체 규칙이 09-22 로그에서 **미적용**. ⬜ 창 길이 30일은 **공식 문서 미확인**(전문 3,934B, 기간 서술 0) *(NEW 2026-09-23)*
...
- [[관련성-판단-주체]] — **"무엇이 관련 있는가"를 검색기가 아니라 모델이 정하게 바꾸는 이동이 서로 다른 층에서 독립적으로 나타났다.** [[Declarative-Attention]](09-03, **KV 읽기** 층: 모델이 참조 범위를 선언) ↔ [[PageIndex]](09-20, **문서 검색** 층: *"similarity ≠ relevance — relevance requires reasoning"*, 벡터DB·청킹 전면 제거). 🎯 [[하네스-설계-축]] 의 "빼라"와 방향이 같되 **빼는 것이 기능이 아니라 중개자**다. 🔴 **표본 2 — 축으로 확정하지 않는다.** 반증 후보 [[mem0]] 은 반대 방향(엔티티 추출·링크로 검색기 강화, ★65,688) · **같은 예산 비교 없음** *(NEW 2026-09-20)*

- [[하네스-설계-축]] — **모델을 바꾸지 않고 감싸는 층만 바꿔서 정확도·비용을 움직이는 것이 독립 축으로 성립했다.** 09-18 배치 **13건 중 4건(31%)** 이고 네 층이 전부 다르다: 구현([[harness-sdk]] ★7,345·벤치 0) · 자동탐색([[SoL-Pi]] NVlabs·★2,185) · 대조실험([[Harness-Design-Empirical]] 176설정·코드 없음) · 학습루프 내재화([[NeoHorse-1-4B]]). 🎯 **가장 강한 신호: 독립된 두 방법이 같은 부품을 지목했다** — SoL-Pi가 탐색으로 *남긴* 4개 중 2개가 컨텍스트, Harness-Design이 절제로 *재서* 1위가 컨텍스트 관리. 🎯 **그리고 결론이 대체로 "빼라"다**(복구 장치=이득 0 · 도구 추가=강한 모델에 무용 · 계획=정확도 불변) = **기능 추가 경쟁 → 절제 경쟁.** 🔴 **구현과 근거가 반대편에 있다** *(NEW 2026-09-18)*

- [[선발창-누락]] — **지표가 며칠에 걸쳐 쌓이는데 선발은 당일에 하면, 늦게 커지는 항목은 영원히 선발되지 않는다.** 🔴 실측: `arxiv:2609.08183`(NeoHorse 모논문) **업보트 421 · 공개 09-08 · 코드 ★535 · 볼트 0히트** — 같은 날 배달된 볼트 1위는 [[Uno]] **19**(22배), 09-18 배치 1위는 36(11.7배). 🎯 **[[DeepSeek-V4.1-Flash]] 의 "공개 후 72시간 다운로드는 지표 아님"(6→244,457, 40,743배)의 업보트 버전**이고, **중복 필터 문제가 아니라 창(window) 문제**다. 🔴 **대가**: 오늘 하네스 4건 중 2건이 그 논문의 프레이밍을 쓴다 — **결과를 받고 원인을 못 받았다.** ⚠️ 업보트 시간 상수 **미실측** *(NEW 2026-09-18)* · 🔴 **09-19 정정: 같은 API 재측정 170**(421 재현 안 됨) → 배수 **8.9배·4.7배**로 하향, 개념은 유지 → [[NeoHorse-1-Paper]]

- [[컨테이너-중복]] — **수집 단위 하나에 지식 객체 여러 개가 들어 있으면 `org/name` 리터럴 필터는 *올바르게* 통과시킨다. 필터가 틀린 게 아니라 단위가 1:1이 아니다.** 🎯 **09-17 파생 중복과 구조가 반대**: 파생은 `원본 1 → 파생 N`(이름이 갈라져서), 컨테이너는 `레포 1 ⊃ 모델 N`(**이름이 하나여서**). 사례 [[MiniCPM]] — 레포 리터럴 0히트가 정확한데 **모델 4건이 이미 볼트에 있었다**. 🔴 **09-17의 2·3차 키가 무력**(GitHub에 `cardData.base_model` 없음 · 컨테이너명 부분문자열은 구성원 전부 과탐). 📌 **자진신고가 유일한 탐지 경로** · **규약: 컨테이너 페이지는 기존 개별 페이지 수치를 복제하지 않는다**(복제하면 같은 사실이 두 곳에서 갈라진다). 🔴 표본 1 *(NEW 2026-09-18)*

- [[RSI-프레이밍]] — **2026-09에 두 독립 조직이 "재귀적 자기개선"을 채택했고 양쪽 다 달성이 아니라 지향으로 적었다.** [[SoL-Pi]]([[NVIDIA]] NVlabs): *"**RSI-inspired** approach"* · [[TokenRhythm]]: *"**Towards** RSI"* · *"**초기 프로토타입**"* · *"**next step**"*. 🎯 **RSI는 "왜 하는가"의 답이고 "무엇을 달성했나"의 답이 아니다** — 실제 달성분은 둘 다 좁고 측정 가능(토큰 절감률 · 평균 +5.93). ✅ **저자는 선을 지켰다. 넘는 것은 요약하는 쪽이다** → [[한정어-탈락]]. 📌 **규율: 시제를 먼저 본다**(`towards`·`-inspired`·`next step` = 미래형 표지) · **RSI 최소 조건은 루프 2회 이상인데 두 사례 다 "다음 단계"라 적었다 = 아직 1회** *(NEW 2026-09-18)*

- [[표-부분인용]] — **표에서 몇 점을 읽는가가 결론의 *정확도*가 아니라 *종류*를 바꾼다.** 🎯 **핵심: 부분인용은 원문 대조를 통과한다** — 인용한 숫자는 전부 정확하므로 오타·환각·과장과 달리 **대조로 걸리지 않는다.** 잡는 유일한 방법은 **인용하지 않은 부분을 세는 것.** 3사례: 행 축([[ms-marco-MiniLM-L6-v2]] 09-16, L4 추가로 변곡점) · **열 축**([[DeepSeek-R1]], `cons@64` 넣으면 **7B=32B**) · **표 개수**([[Qwen3.8-27B-TWIN-TURBO-709-GGUF]], 4표 중 1표 → **68점 오류**). 🎯 **볼트 규칙: 표 개수·귀속 대상 먼저 확인 → 열을 선택하지 말 것 → 선택했으면 "N열 중 M열" 명시.** 09-16 "3점 이상" 규칙은 **행만 제약했다** *(NEW 2026-09-17)*

- [[파생저장소-식별]] — **`org/name` 리터럴 중복 필터는 재포장·양자화 파생을 구조적으로 놓치며, 그 누락분은 HF 트렌딩 채널의 14%다.** 🔴 볼트 실측: 상위 200 중 `Qwen3.8-27B` 계열 **28건**(원본 1 + 파생 27 — GGUF·FP8·NVFP4·MLX·abliterated·Heretic·imatrix) = **7분의 1이 단일 베이스의 가족**. 🎯 **수집기 제안 정정: `tags` 가 아니라 `cardData.base_model` 을 읽어야 한다** — [[Comfy-Org-YuE2]] 는 cardData에 [[YuE2-3B]]+SheetSage2 둘 다인데 **tags엔 SheetSage2만** 있고 라벨은 `finetune`(실제 재포장). 🔴 그리고 **base_model은 직전 1홉만** 담는다([[Qwen3.8-27B-TWIN-TURBO-709-GGUF]] 는 4단 사슬인데 태그는 1단). 판정 절차: 새 축이 **문서화**됐으면 (b)독립채택 · 파일만 있으면 **(c)병합** · 순수 재포장이면 (a)중복 *(NEW 2026-09-17)*
...
- [[백필-우회]] — **필터가 비운 자리를 다음 순위로 채울 때 같은 필터를 다시 돌리지 않으면, 필터는 "누가 통과했는지"만 뒤집는다.** 🔴 2026-09-14 볼트가 정면으로 당했다: raw가 중복 8건을 **정확히** 탈락시켰는데 **배달된 13건 중 8건이 또 중복**. 결정적 증거는 채널별 분해 — **탈락이 발생한 채널만 중복 100%**(GitHub 5/5 · HF모델 3/3), **탈락 0인 논문 채널은 중복 0**. 🎯 **로그가 "8건 탈락"으로 성공처럼 보이기 때문에 2배치 동안 안 보였다 — 탈락률이 아니라 통과율을 감사해야 한다** *(NEW 2026-09-14)*

- [[짝-아티팩트-조회]] — 하나의 프로젝트는 논문·코드·가중치·변형판으로 나뉘어 공개된다. **한 산출물만 보고 내린 판단은 다른 산출물이 이미 반박했을 수 있다.** 🔴 볼트가 09-02 [[Qwen-Drive-1.0]] 에서 직접 범했다(가중치 레포가 **6일 앞서** 존재했는데 "수치 0개"로 기록). [[한정어-탈락]] 이 *문서 안*의 결손이라면 이건 *문서 밖*의 결손 *(NEW 2026-09-13)*

- [[분포내-우위]] — 벤치 **자신의 데이터로 학습한 모델**과 **제로샷 모델**을 같은 홀드아웃에서 비교할 때 생기는 구조적 우위. [[MetroLLM-Bench]] 의 *"4B가 GPT-5.6 상회"* 가 사례(717케이스로 학습). 🎯 **결손이 인용이 아니라 배치에서 생긴다** — 각 문장이 참이어도 나란히 놓지 않으면 잘못된 결론이 난다 *(NEW 2026-09-13)*

- [[한정어-탈락]] — **원문을 정확히 인용했는데 조건절을 안 옮겨서 틀린다.** [[파생표기-함정]](파생물을 읽음)·[[단위-불일치]](세는 대상이 다름)와 **처방이 다르다** — 인용문이 원문에 실제로 있어서 **원문 대조로는 안 잡힌다**. 09-12 4사례: ①강화([[hyperresearch]]) ②**약화**([[OpenResearch]]) ③귀속([[MiniCPM5-2B-GGUF]]) ④프레임 승계([[YuE2-3B]]). 🎯 **과장만이 아니라 과소평가도 만든다** *(NEW 2026-09-12)*

- [[단위-불일치]] — **원문을 정확히 읽어도, 그 수가 세는 단위와 내가 필요한 단위가 다르면 틀린다.** [[파생표기-함정]] 이 *읽기의 실패*라면 이건 *대응의 실패* — **원문 대조·API 실호출·2회 측정을 전부 통과한다**(수 자체는 정확하므로). 2026-09-11 사례 6건: **패밀리 공용 카드**(mini·Pro md5 `f706fcb33c93` 동일) · **백본 552B vs 체크포인트 763B**(차이=Engram 196B) · **총≠활성 파라미터** · **비교 기준 모델 불일치**(51.3%는 7B 대비, 공정 비교는 85%) · **자기보고 79 vs 표 실카운트 78** · description 1000+ vs README 10+7. 처방은 *"더 열심히 읽어라"가 될 수 없다* — **"이 수가 내 질문에 답하는 수인가"를 묻는 별도 단계**가 필요 *(NEW 2026-09-11, reliability high — 볼트 자체 실측)*

- [[검사가능성-공사]] — **모델이 잘하는지 묻기 전에, 무엇을 들고 있는지 볼 수 있게 만들어라.** ✅**2026-09-06 잠정 → 확정 승격**(09-02 승격 조건 *"다음 배치 2건 추가"* 충족 — [[Aspire]]·[[HarnessDev]]가 **히든 평가 분리**로 도달). 승격 근거는 개수가 아니라 **급소가 논증에서 측정으로 바뀐 것**: *"검사기가 피검사자와 같은 실패 모드를 공유하면 계기판이 아니라 거울이다"* 를 [[Aspire]]가 **부정적 결과로 확인**(에이전트가 좁은 자체평가를 신뢰해 국소 향상이 히든 평가로 **전이되지 않음**). 외부 정답 고정 **5형태** 정리: 기하([[Qwen-Drive-1.0]]) · 실행결과([[DeepSWE]]) · 사람 유지보수 목록([[humanizer]]) · **버전 태그 붙은 공식 문서**([[context7]] 🆕) · **히든 평가**([[Aspire]]·[[HarnessDev]] 🆕 — 정답의 *출처*가 아니라 **볼 수 있는 시점**을 외부화 = **접근 통제도 외부화의 한 형태**). ⚠️남는 급소 = *바깥이 안쪽보다 신뢰할 만한지는 이 개념 자체로 조달되지 않는다*(Aspire 520문항의 "전문가"가 누구인지 미확인) + [[HarnessDev]]가 붙인 새 한계 *검사 창을 잘 지어도 그 창은 특정 모델용일 수 있다*. 🪞볼트 미이행 유지: `queries/` **0건** = 자기가 쓴 위키를 자기가 읽고 판단 = **히든 평가에 해당하는 것이 없다** *(reliability medium→**high**, UPDATED 2026-09-06)*

- [[국소-수리-원리]] — **잘된 부분은 건드리지 말고 틀어진 지점만 고쳐라.** 2026-08-31 배치의 논문 3편 + 서베이 1편이 독립 도달한 원리를 **학습(손실 마스킹)·추론(주입 위치)·평가(정지 조건)** 세 층위로 정리. 전제는 *실패가 수리 가능한 동안 보여야* 하며, 분기점 판정을 생성기와 같은 계열 LLM에 맡기면 **심판 비독립성**에 걸린다 *(UPDATED 2026-09-02 — [[DiagEvo]]가 **첫 정량 근거** 제공(+4.5%p·+1.1%p)하며 reliability medium→**high**. 층위 3→**5**로 확장: 학습·추론·평가 + **커리큘럼**([[DiagEvo]]) + **아키텍처**([[Safin-1]]). ⚠️미해소 급소 = *틀어진 지점을 **누가** 판정하는가* — DiagEvo의 자기 투표는 심판 비독립성에 정면으로 걸린다)*
<!-- 아이디어, 이론, 프레임워크 -->

- [[LLM-Wiki]] — LLM을 활용한 복리 축적형 개인 지식베이스 구축 패턴 *(2026-08-28: [[claude-obsidian]]이 첫 패키지 구현체 — 직접 대조군)* *(**UPDATED 2026-09-03** — 🪞[[CoGR]]이 볼트의 **결측 층**을 드러냄: 922개 페이지를 grep/wikilink로 찾으면서 **태그가 유효했는지 한 번도 측정한 적 없다** = 아이템 측에 보상 신호 0. 원인은 구조적 — 스키마의 *"좋은 답변은 queries/에 저장"* 이 **5개월간 0회 실행**. **인제스트(쓰기)만 하고 쿼리(읽기)를 기록하지 않는 半쪽 시스템** → actionable 우선순위 **높음**)*
...
- [[시계열-예측-파운데이션-모델]] — TimesFM·Kronos 중심, 제로샷 시계열 예측 패러다임 *(**UPDATED 2026-09-03** — **약 5개월 만의 갱신**. [[TimesFM]] 3.0 공개(`timesfm-3.0-pytorch` **331M**·DL 46,862·라이선스 `other`로 **레포 Apache-2.0과 불일치**). 📌**에이전트 일색 배치 13건 중 유일한 비-LLM 축인데 트렌딩 2위** — 담론의 주변부에 있으면서 사용 축에서는 살아 있다. 📌**331M = [[GLM-5.3]] 753B의 1/2,275** → 이 분야는 *크게 만들어 범용* 이 아니라 **작게 만들어 어디에나 꽂는** 축이며 CPU·엣지 배포가 현실적. ⚠️**세대 교체 미완**: 2.5(DL 55,036) > 3.0(46,862)인데 좋아요는 3.0이 2.4배 · 정확도 벤치 여전히 미확인)*
- [[금융-AI]] — TradingAgents·Kronos·TimesFM, 멀티에이전트+파운데이션 모델 결합 *(NEW)*
- [[에이전트-메모리-레이어]] — cognee·claude-mem으로 대표되는 에이전트 메모리 인프라 패턴 *(**UPDATED 2026-09-06** — 🆕**여섯 번째 축 「연속 잠재 메모리」**([[LatentPress]]). 지적의 핵심이 기술이 아니라 **설계 관성**: *압축 컨텍스트는 사람이 읽을 텍스트나 렌더링 이미지로 운반된다 — **소비자가 언어모델일 때조차도**.* **동결 디코더가 입력 임베딩으로 직접 읽는 연속 메모리 토큰**(텍스트 복원 없음), 어댑터만 학습 4.2M~26.2M(**디코더의 ~0.1%**). LongMemEval **0.504@7.70배 압축 > 무압축 원문 0.490** — **압축본이 원문을 이겼다**(요약 0.184·OCR 0.426~0.312). ⚠️**reader-matched 결박** — 디코더 교체 시 자산 재생성 필요. 1~4축은 모델 교체에 중립적이었으나 이 축은 아니며, [[HarnessDev]]의 *하네스 개선은 모델 간 전이 제한*과 **두 층에서 동시 관측된 동형 함정**. ⚠️[[Random-Attention]]과 **반대 방향**(선택 신호 무용 vs 압축이 이득) — 미해결 대조 대상)* *(**UPDATED 2026-09-03** — **4번째 방식 「선택적 비독해」 신설**([[Declarative-Attention]]). 기존 넷(RAG·KV·압축·외부DB)이 *기억을 어디에 어떤 형태로 두는가* 를 나눈다면, 이건 **기억을 그대로 두고 「읽기 결정」만 모델에게 넘긴다** — 외부에 두지도 압축하지도 않고 **읽지 않기로 선택**. 핵심은 **관련성 판단의 주체**: RAG는 검색기가, 프록시 점수는 외부 스코어러가, 이건 **모델 자신**이 판단. 참조토큰 **-52.0%**/정확도 -1.27%p·**학습 불필요**. ⚠️추론 엔진 수정 전제)*
...
- [[월드모델]] — 미래 예측·계획 모델, 2025~2026 최대 트렌드(예측형 vs 생성형) *(NEW 2026-07-12)* *(**UPDATED 2026-09-03** — [[SolarWM]]: **모델이 아니라 「데이터 계약」을 공개한 첫 사례.** 143만 클립을 프레임 정렬 계약으로 통합하며 **선별 결정·출처**를 1급 필드로 승격 → *무엇을 왜 뺐는지* 가 함께 배포되면 **혼합 구성을 남이 다시 짤 수 있다**([[MiniMax-H3]]의 *오픈 가중치 ≠ 오픈 시스템* 과 정반대 노선). **5초 학습 → 분~시간 롤아웃**이 사실이면 *롤아웃 길이 = 학습 길이* 제약이 증류로 끊긴다. ⚠️**초록에 벤치 수치 0**)*
...
- [[파생표기-함정]] — **원문에서 파생된 표기(API 분류·표 서식·벤치마크 구성)를 원문 대신 읽으면 틀린다.** 볼트 규칙 계보 3번째: 09-08 *"description은 소스가 아니다"* → 09-09 *"이름도 소스가 아니다"* → **09-10 "API가 계산해준 필드도, 표의 서식도, 벤치 구성도 소스가 아니다"**. 사례 3건: [[teamai-cli]] `NOASSERTION`을 자체라이선스로 오독(**실제 LICENSE는 MIT 전문 수록**) · [[Qwen3.8-Flash-Next-NVFP4]] 표가 FP8 행을 볼드로 강조하나 **9개 중 5개는 NVFP4 우세** · [[Qwopus3.8-27B-Flash-GGUF]] 벤치가 전부 텍스트라 **멀티모달인데 텍스트 전용으로 오독**. 🔴 **파생표기는 원문보다 짧고 구조화돼 있어서 위험하다 — 틀려도 에러가 없다** *(NEW 2026-09-10)*
- [[요약자와-판정자-분리]] — **자료를 수집한 주체가 그 자료의 신뢰도를 판정하면 안 된다.** 외부 증거: [[SAEScientist-Bench]] *"에이전트는 대조는 설계하지만 **측정치를 빈번히 오해석한다**"* · [[Discovery-Certification-Protocol]] **LLM 없는 결정적 검증기**. 볼트 관찰: raw 오류가 **방향은 매번 바뀌고**(09-07/08 과장 → 09-09 누락 → 09-10 역전) **위치는 한 번도 안 바뀐다**(판정 단계). 🎯 DCP 핵심발상 **"발견의 증거는 결과가 아니라 대안 경로의 부재"** — 볼트 규칙 3개와 1:1 대응 *(NEW 2026-09-10)*
- [[자기제한-명시]] — **자기가 하지 않는 것을 먼저 밝히는 문서가 더 신뢰할 만하다.** 사례 4건: [[Pascal-Editor]] `furniture-fit`(*"높이·개폐·배송 검증을 주장하지 않는다"* + 툴스키마 런타임 선조회) · [[PI-Desktop]](*"완전한 OS 샌드박스가 아니라 사용자가 신뢰하는 코드"*) · [[Qwopus3.8-27B-Flash-GGUF]](트레이드오프·**수치 등급 구분**·결함 자인) · [[text-to-cad]](**조용한 실패 3종** 직접 경고). 🎯 **판별식: 문서가 자기에게 유리한 방향으로 틀렸는가, 불리한 방향으로 틀렸는가.** 09-09 [[GLM-5.3-CYBERSECURITY-FP8]](규정이 측정보다 관대)의 정반대 *(NEW 2026-09-10)*
...
- [[비매칭-비교]] — **"A가 B를 넘었다"는 하네스·예산·추론 설정·평가자가 같을 때만 모델 비교다.** 09-19 논문 4건 전부 비대칭: 보고치 복사([[RSIAgent]]) · 폐쇄 모델 최저 설정([[PhysBrain-1.5]]) · 예산 5배([[ModularRSI]] 표4) · 비교군 0([[MiniMax-H3-Physical-Reasoning]]) + 행별 참조모델 교체([[K2-Horizon-7B]]). 🎯 고지는 초록이 아니라 **부록**에 있다 *(NEW 2026-09-19)*
- [[하네스형-에이전틱-RL]] — 하네스가 루프를 소유하고 트레이너는 프록시로 요청–응답만 본다([[agent-lightning]] 명명). 온라인 RL / 오프라인 SFT([[NeoHorse-1-Paper]]) / 하네스 진화([[SoL-Pi]]·[[ModularRSI]]) 3방식. 🎯 **단위 문제**: 롤아웃당 2.41 샘플 · 🔴 보상해킹 3/4 네트워크 경유 *(NEW 2026-09-19)*
- [[Electron-CDP-브리지]] *(잠정·표본 2)* — API 없는 Electron 앱을 CDP 포트로 MCP 도구화([[tradingview-mcp]] · [[After-Effects-MCP]]). 🔴 내부 API 파손 · 약관 · 포트 노출 *(NEW 2026-09-19)*
- [[포맷-불일치-오염]] *(잠정·표본 1)* — 다른 포맷으로 섞인 테스트셋이 점수를 **깎아** 오염을 숨긴다([[marin]] 32B GSM8K −22점). 저점 + 프롬프트 취약성 = 오염 신호 *(NEW 2026-09-19)*
## sources

### 2026-09-24 배치 (13건 · 신규 9 · 갱신 4 · 재판정 3 · 🎯 볼트가 자기 미수행 actionable을 집행해 09-23 예측을 기각)

**GitHub 5 — 🎯 4건이 "에이전트가 무엇을 했고 얼마를 썼는지"를 1급 기능으로 넣었다 · 3건이 Rust**
- [[nasiko]] — A2A 에이전트 앞에 서는 단일 프로세스 프록시. 🎯 **에이전트에 진짜 API 키를 주지 않는다** — `OPENAI_BASE_URL` + **단기 신원 토큰**만 주고 provider/model/key는 서버측 해석(*"No agent or log ever sees a real API key"*). 3단 라우팅(임베딩 shortlist→맥락 rerank→LLM 선택) · OTel `gen_ai.*` 토큰/비용 자동수집 · Redis cascade 가드(depth·fan-out·예산·사이클). ⭐8,841(당일 +288)·Rust. ✅ **라이선스 Apache-2.0 확정** — API `NOASSERTION` 은 탐지 실패였다(LICENSE 196행 원문 확인, **수집기·볼트 1차 판정 둘 다 정정**). 확인 범위 **README 742행 전문**(수집기 45행) — 4대 능력 **축자 일치**. 🔴 성능·지연 수치 전문에 0개 *(NEW 2026-09-24)*
- [[agent-browser]] — Vercel Labs 브라우저 자동화 CLI, Chrome for Testing 직접 다운로드. ⭐43,140(당일 **+62 둔화**)·Rust·Apache-2.0·**5플랫폼 전부 네이티브**. 🎯 **★증분 둔화 레포가 npm 월 다운로드 5,326,951**(볼트 실측, 수집기 미조회) — **★는 발견을, npm DL은 사용을 센다** → [[상대속도-가림]] 최강 사례이자 [[TradingAgents]] "5주 정체" 근거의 **대체**. `dashboard start`(포트 4848) 라이브 뷰포트 내장. 🔴 오픈이슈 **813 = ★의 1.9%**(배치 최고) · `Commands` 540행 미열람 *(NEW 2026-09-24)*
- [[agent-desktop]] — 픽셀 대신 **OS 접근성 트리**로 앱 조작, 한정 ref(`@s8f3k2p9:e1`). ⭐1,621(당일 +107)·Rust·Apache-2.0. 🎯 **자기 경계를 값으로 노출**: *"58 command names, **54 operational**"* · 못하는 4개는 `ACTION_NOT_SUPPORTED` **fail-closed** · 권한 `granted/denied/**unknown**`(= *"프롬프트 없이는 확인 불가"*) → [[검사가능성-공사]] **3번째 독립 생태계**(볼트 자신의 ⬜ 3값화 actionable은 5일째 대기). 🎯 **볼트 최대 발견: 유일한 수치가 `<img alt=>` 속성 안에 있었다** — Slack 30,743→**383 토큰(98.75% 감소)**, 헤드라인 *"78–96%"* 보다 **위**. `benchmark/measured/baseline` 어휘는 487행 전수에 **0건**. 🟡 수집기 *"리눅스/윈도우 불가"* → README는 **`Planned`** *(NEW 2026-09-24)*
- [[PanWatch]](盯盘侠) — [[TradingAgents]] 9-Agent 파이프라인을 보유종목 버튼 하나로 태우는 자체호스팅 제품(4분석가→롱숏 토론→리스크 심사→PM 결정서, **3~5분**, Telegram/위챗/딩톡 푸시). ⭐1,717(당일 +95)·MIT·Python. 🎯 **볼트 [[금융-AI]] 축이 프레임워크만 모은 상태에서 "제품이 되면 어떤 모양인가"의 첫 데이터 포인트** + 노드별 진행률·비용 자체 계측(204·210행). 🔴 **이 래퍼가 인용한 본체 ★가 3.2만 낡았다**(*"76k+"* vs 실측 108,376) → **래퍼로 본체를 알면 안 된다.** ✅ 수익 주장 없음(272행 전수) *(NEW 2026-09-24)*
- [[ComfyUI]] — 🔄 **신규가 아니라 갱신**(2026-08-11부터 페이지 존재, 수집기 *"본체 누락 4번째"* **오판**) + 🔀 **도메인 재판정 `ai-news` → `video-saas`**. 조직 이관 `comfyanonymous` → **`Comfy-Org`**(HTTP 301, 레포 ID **589831718** 승계). ★126,600→**134,790**(+8,190/44일, 당일 +922→**+209**) · **GPL-3.0 확정**(기존 미해결 질문 해소 — 🔴 링크 시 전파 의무). 🎯 **이관이 문서에서 절반만 끝났다** — `Release Process` 3레포 중 **Core만 구 이름**, Desktop·Frontend는 `Comfy-Org`. → **[[정규명-우선-중복검사]] 신설** *(UPDATED 2026-09-24)*

**HF 논문 5 — 🔴 5건 전부 초록만 · 그중 2건은 초록 수치 0개 · 지연 1~5일**
- [[HappyWorld-Bench]] — 🔀 **재판정 `ai-news`→`slam-3dgs`**. 월드모델을 영상 품질이 아니라 **"에이전트가 상호작용한 뒤에도 세계가 일관되게 남는가"** 로 재는 3트랙 벤치. video **1,138 프롬프트**/spatial **300 장면**/embodied **254 케이스**, 평가 **14/9/8종**, W1–W6 6단 + Arena 인간 Elo. spatial **최고**(*at best*) 배치 정확도 **70.14%**·편집 실행 **73.33%**(절대, %p 아님) = **9종 중 최고가 3할을 틀린다.** 업보트 **35 = HF 데일리 1위** · **3일 차**. 🔴 video·embodied 수치 0개·분포 없음·W1~W6 정의 없음·`githubRepo` **null**(볼트 실측) *(NEW 2026-09-24)*
- [[SpeakerMem-R1]] — 다자 대화 **화자중심 이중트랙 메모리**(화자 라벨 원문 + 파생 상태, person/group 2뷰, 질의 시 엔티티·사건·시간 결합). Writer-R1 = SpeakerLevenshtein + **speaker-conditioned GRPO**, *"**while enabling local deployment**"* 를 초록에 명시. ✅ **이번 배치 유일 수치 완비**(GroupMemBench **47.9**/SocialMemBench **69.2**/EverMemBench **61.9** · 리더보드 **62.33** · LoCoMo 1,986문항 **70.85** · 통제 305문항 SFT **57.38→RL 68.20**) + **binary accuracy·token-F1 이중 보고 명시** + 어블레이션. 🎯 **진단이 축**: 실패는 검색이 아니라 **귀속**. 🔴 GroupMemBench **47.9% = 절반 이하** — 다자 메모리는 아직 대체로 실패. 📌 **대조 시 자체값 61.9% 사용**(리더보드는 외부 검증 존재 근거로만). GitHub ★70(배치 최다) · **2일 차** *(NEW 2026-09-24)*
- [[The-Past-Frames-the-Future]] — AR 비디오 생성 메모리 **서베이**. 🎯 **기여는 분류가 아니라 조작적 정의**: *"outer AR 스텝을 가로질러 유지되며 **원 증거가 로컬에 없어진 뒤에도** 미래 생성에 영향을 주는 지속 정보"* — **긴 컨텍스트를 넣은 것은 메모리가 아니다.** 5축(Forms/Functions/Operations/Learning/Evaluation), Operations 5단(쓰기·읽기·갱신·관리·통합)은 [[reat]] 장면간 상태전달 체크리스트로 즉시 전용 가능. 문제 위치도 정확 — *"관련성이 사라지기 **훨씬 전에** 컨텍스트에서 밀려난다"*. 업보트 33 · **1일 차** · GitHub ★30. 🔴 수치 0개(서베이) → 비교 인용 금지, reliability **medium** *(NEW 2026-09-24)*
- [[Spatial-Interactor]] — 🔀 **재판정 `ai-news`→`slam-3dgs`**. 정적 QA 대신 **`관측→행동→후속 관측` 궤적**으로 VLM에 상태 전이를 가르친다(L1 수동 world-state/L2 능동 self-state/L3 장기 궤적 · **LSI-108K** · SFT→**OPD**). 🎯 OPD 내부가 **`privileged self-distillation`** — 교사 분기만 **구간별 전이 설명이라는 특권 정보**를 받는다(**비대칭이 크기가 아니라 정보 접근**) → [[온폴리시-증류]] 신 변종. 🔴 **초록 정량 수치 0개 · 벤치마크 이름조차 없음**(*"multiple VLMs and spatial benchmarks"*) · 베이스 VLM 미명시 → **reliability low, 비교 인용 금지**([[BPO]] 동류). 업보트 22 · **5일 차** · ★6 *(NEW 2026-09-24)*
- [[RewardVerse]] — 비디오 보상 모델의 **`scalar drift`**(프롬프트마다 채점 척도 붕괴·이동)를 명명하고, **루브릭을 먼저 생성해 그것으로 채점**하는 중간 표현을 넣는다(RGPO 2단계: 자기진화 seed 루브릭으로 scorer 워밍업 → 루브릭 생성기 공동 최적화 + 인간 평점 지속 정렬). 16차원 EvalVerse + pointwise·pairwise. 🎯 **수치 없이도 구조는 베낄 수 있다** — 점수를 직접 묻지 말고 기준을 먼저 뽑게 한다(내 파이프라인 즉시 적용, 스택 불필요). 🔴 **초록 수치 0개인데 SOTA를 주장한다**([[Spatial-Interactor]]보다 한 칸 나쁘다) · 16차원 중 한 축도 미공개 · 베이스라인 미명시 → **reliability low**. 🔴 **볼트 정정: 수집기가 부속 GitHub 누락**(`2kxx/RewardVerse` ★3 — 필드는 있었다). 업보트 20 · **5일 차** *(NEW 2026-09-24)*

**HF 모델 3 — 🔄 전부 갱신용 DL 델타(신규 아님) · [[원본-파생-역전]] 소급 점검의 입력**
- [[Qwen3.8-27B]] — 🔄 갱신. **30일 창 6,912,469 / 누적 9,846,662**. 🎯 **같은 하루에 창은 −167,177 줄고 누적은 +165,463 늘었다** — 두 지표의 독립성을 단일 모델·단일 일자에서 보여주는 첫 관측. 파생 unsloth GGUF 누적 13,993,047→**14,460,000**(+466,953) → **역전 배수 1.445→1.469배**(파생이 하루에 2.83배 더 받아갔다). ⬜ 파생 누적값이 정확히 만 단위로 끝남(표기 변화? 1회 관측, 보류) *(UPDATED 2026-09-24)*
- [[Ternary-Bonsai-2-27B]] — 🔄 갱신. v2 **2,815,979**(창=누적, created 09-16 = 8일) · 좋아요 1,976 · trendingScore **1,806**(트렌딩 3위). 🔴 **볼트 기록 정정**: 09-22 *"대조군 v1 662,554"* 는 **창값**이었고 **누적은 1,879,078**(2.84배). 🎯 그 차이가 [[지표-창길이]] **창 길이 역산 ≈28.5일**의 근거가 됐다. **v2가 8일 만에 82일 된 v1 누적을 1.50배 초과**(일평균 15.4배) — 🔴 **검사가능성 후퇴([[검사가능성-후퇴]] 사례 1)가 채택을 막지 않았다** *(UPDATED 2026-09-24)*
- [[Qwen-Image-2.1]] — 🔄 갱신. 원본 **28,407** vs `Comfy-Org` 재포장 **2,220,609**(**78.17배**) vs `abenzerps` GGUF **350,678**(12.34배) — 셋 다 생성 10일 이내라 **창=누적 확인**(배수는 창 아티팩트 아님). 🆕 **trendingScore가 좋아요 편에 선다**: 원본 trend **1,970** vs 파생 622(**3.17배**), 좋아요 2,097 vs 648(3.24배) — **정체 축에 독립 지표가 하나 더**. 🎯 **주목과 채택이 78배 갈린다.** ✅ `abenzerps` 는 09-22 *"DL 0 제외"* 대상이었는데 **2일 만에 350,678** — 리다이렉트 추적 규약이 수치로 정당화 *(UPDATED 2026-09-24)*

**보너스 갱신 1 (배치 외 · 교차조회 중 발견)**
- [[TradingAgents]] — ★96,723(볼트 08-09) → **실측 108,376**(+11,653 · **10만 돌파를 볼트가 놓쳤다**) · forks 20,752 · open issues 167. 🔴 **낡은 주장 만료**: 이 페이지가 *"최종 푸시 2026-07-18로 5주 정체 … 스타를 활성도 지표로 쓸 수 없다는 볼트 누적 관측의 **가장 큰 사례**"* 로 세워 뒀으나, **`pushed_at` = 2026-09-24T07:51:55Z(오늘)**. **5주 공백은 은퇴가 아니라 휴지였다.** 일반 규칙은 유효하나 최대 근거는 [[agent-browser]](★+62 vs npm 532만)로 **교체** — 단일 레포 커밋 공백보다 **두 독립 소스 수치 대비**가 반박에 강하다. 🎯 교훈: **시점 의존 관측을 인사이트의 토대로 쓰면 만료된다** *(UPDATED 2026-09-24)*

### 2026-09-23 배치 (13건 · 신규 9 · 갱신 4 · 도메인 재판정 1 · 🎯 볼트가 수집기 "최대 발견"을 반박)

**GitHub 5 — 상위가 전부 "에이전트 인프라"로 채워졌다**
- [[ax]] — 에이전트를 쿠버네티스처럼 선언하는 오케스트레이터(`ax.io/v1alpha1` YAML · 프리미티브 4개 Task/Workspace/Gateway/Model · `suspend/resume`). ⭐8,140(**당일 +2,305 = 배치 최대 급상승**)·Go·Apache-2.0. 🎯 **진짜 뉴스는 의존 사슬** — `google/` 레포가 *"구글 공식 아님"* 을 명시한 [[substrate]] 위에서 돈다. 🔴 **성능 수치 0개**(확인 범위: README **185행 전문**, benchmark/throughput/latency 키워드 소개문 외 0건) *(NEW 2026-09-23)*
- [[substrate]] — "에이전트는 대부분 유휴하다"에 베팅한 밀도 런타임(actor 다수 → worker 소수 다중화 · microVM/gVisor). ⭐3,171·**오픈이슈 525 = 스타의 16.6%**(ax 0.44% 대비 이례적). 🔴 자기보고 3종(*10배 밀도·sub-500ms resume·500+/s*) **근거표 0** · 데모는 ~250 actor/8 pod(≈31배)로 "10배"와 관계 불명. ⬜ README 252행 전문 미열람 *(NEW 2026-09-23)*
- [[univer]] — 오피스 런타임 SDK(스프레드시트·문서·슬라이드, 단일 Facade API). ⭐15,934·TS·Apache-2.0. 📌 **생성 2022-09-29 = 4년 된 SDK의 리포지셔닝** — 코드가 아니라 포지션이 새 것. 🔀 [[docling]](문서→구조화)의 **반대 방향**(구조화→문서). ⬜ 에이전트 전용 기능 실재 여부 미확인(README 405행 미열람) *(NEW 2026-09-23)*
- [[treg]] — 에이전트 툴용 프록시(60+ 제공자·3,000+ 엔드포인트, **콜당 과금**, 자격증명 서버사이드 주입). 📌 과금 논거 구체적(Semrush $139/mo·Moz $99/mo·Apollo $59/seat 를 1회 실행분으로 분할). 🔴 **실단가 미공개**(*"from a cent"*)·라이선스 **NOASSERTION**·treg.to 결합. **reliability low** *(NEW 2026-09-23)*
- [[financial-services]] — ⭐19,584 → **36,623 (+17,039)** 갱신. 📌 **한 소스를 두 경로로 배포**(Claude Cowork 플러그인 ↔ Managed Agents API `/v1/agents`, 동일 프롬프트·스킬). ⚠️ README 강조박스: 산출물 **전부 인간 승인 전제**(거래 실행·원장 기표 불가) → [[자기제한-명시]] 상업판. ⬜ **직전 기록 시점 미상이라 증가율 미확정** *(UPDATED 2026-09-23)*

**HF 논문 5**
- [[RULER-SVG]] — 지시문 → **6항목 루브릭**(의미·시각·스타일) → 심판 VLM 항목별 채점 → 가중합을 GRPO 보상. **정답 SVG도 인간 선호 라벨도 불필요.** 📊 0.432→0.693 · 0.395→0.683(**절대 점수**, 상대 +60.4%/+72.9%). ✅ 한정어 보존: DeepSeek-V3와 *"대등(matching)"* — **상회 아님**. ⚠️ **볼트 기존 "RULER"(장문맥 벤치, [[Jet-Long]])와 이름 충돌 → 페이지 분리, `[[RULER]]` 단독 링크 금지.** HF 데일리 1위 ↑48 *(NEW 2026-09-23)*
- [[ScriptMoE]] — 229개 언어 단일 인식기(시각 인코더 공유 + dense 디코더를 희소 MoE로 · **이미지 단위** top-2 스크립트 라우팅). 🎯 라우팅 단위가 토큰이 아니라 이미지 = 과제 구조와 일치. TextMuSS-10M 동반. ⬜ *"by 1.31%"* **단위 미확정**(%p면 80.75→82.06, 상대면 +1.07%p) · 🔴 **스크립트별 내역 없음**(평균 82.06%가 어디서 무너지는지 불명) ↑33 *(NEW 2026-09-23)*
- [[BPO]] — critic-free RLVR. **종단 보상+자기회귀** 조건에서 PMD를 궤적 수준으로 재작성 → 중간 가치 추정 회피, **원 PMD와 동일 최적해 증명**. 🔴 **초록 성능 수치 0개**(✅ 볼트 실측: HF `summary` 전문 **769자** 파싱, 수치 토큰 0 / arXiv 본문 미열람) → **reliability low**. 📌 publishedAt **09-14 = 9일 차 재등장** → [[게시일-이중화]] *"최대 7일"* **3번째 반례** ↑19 *(NEW 2026-09-23)*
- [[HyperQ]] — 동결 확산LM + **토큰 조건부 양자 잔차 분기**(하이퍼네트워크). ⚠️ **실제 양자 하드웨어 미사용** — "정확한 고전 해석식"으로 고전 계산, 그래서 비용이 큐빗 수에 선형. 📊 47.65→54.30 · 백본 +4.71 · LoRA +3.67(**점수 차**). 💡 **진짜 주장은 표본효율** — 20,000쌍 vs 베이스라인 200,000쌍(**1/10**). ⬜ 벤치 개수·이름조차 초록에 없음 ↑18 *(NEW 2026-09-23)*
- [[GAE]] — 🔀 **도메인 재판정 `ai-news`→`slam-3dgs`**(포인트맵 디코딩·카메라 궤적 평가·RealEstate10K/DL3DV). *"3D 일관성은 모델링이 아니라 표현의 문제"* → 지오메트리 파운데이션 피처를 생성용 잠재로 재파라미터화, **외관·깊이·카메라·포인트맵 동시 디코딩**. ✅ **비교 설계가 무기** — 생성기·프로토콜 고정 후 잠재만 교체: FVD **−12.7%/−23.1%**(상대), 카메라 궤적 오차 **절반** → [[비매칭-비교]] 의 **반례(좋은 사례)** 로 등록. ⬜ 속도 수치 0 = 실시간성 불명 ↑34(볼트 실측, 수집 시 31→32 계속 상승) *(NEW 2026-09-23)*

**HF 모델 3 — 전부 기보유, 갱신. 이 배치의 지표 사건이 여기서 났다**
- [[Qwen3.8-27B]] — 네이티브 VL dense 27B(사고 모드 **기본 on**, `reasoning_effort`). DL **7,079,646(30일) / 9,681,199(누적)** ✅ 두 창 병기. 📌 Qwen Cloud 호스팅판 **1M 컨텍스트** *"coming soon"* = 오픈 가중치와 유료 API의 컨텍스트 차등 명시. ⬜ 벤치 미확인(카드 상단 ~2.2KB만, 582행 미열람 — *"없음"이 아니라 "안 봤음"*) *(UPDATED 2026-09-23)*
- [[Qwen3.8-27B-GGUF]] — DL **7,019,401(30일) / 13,993,047(누적)**. 🎯 **누적 기준 파생이 원본의 1.445배 → [[원본-파생-역전]] 반례 판정이 지지 사례로 뒤집혔다.** 🔴 `base_model_relation` 미설정 → [[파생표기-함정]] **6번째**. 🔴 *"top-1% 정확도 >10% 우수"* 비교표가 외부 블로그에만(카드 118행 전문, 수치표 0개) *(UPDATED 2026-09-23)*
- [[Ternary-Bonsai-2-27B]] — DL **2,569,604(30일) / 2,569,604(누적) — 정확히 일치.** 🎯 09-16 생성 = **7일 차**라 창보다 어리다 → 누적 축 정합성 확정. 📊 **이번 배치 유일하게 분포 지표 동반**(볼트 요청 2 충족): 14벤치 평균 84.78 → 수학 96.57 · 코딩 89.42 · **에이전틱 툴콜 74.92**(평균이 가린 최저점) *(UPDATED 2026-09-23)*
...
- [[TensorRT-LLM]] — [[NVIDIA]] 추론 런타임이 **영상 생성 가속까지** 범위 확장(★14,676). 🔴 **open issues/★ 10.23% = 배치 1위**(2위의 7.4배, [[harness-sdk]] 와 동일 자릿수) · ★ 상대속도 0.095% = 배치 최저. ⚠️ **API license=NOASSERTION ↔ 파일 본문=Apache 2.0**(복합 LICENSE) — 원인 규명. 🔴 영상 가속 주장은 **블로그 미열람, 분모 미상**
- [[higgsfield-repo]] — 🔴 **수집기 "동명의 다른 회사" 가설 기각 — 동일 법인**(`higgsfield-ai`.name = "Higgsfield Inc."). 볼트 [[Higgsfield]] 가 8개월 추적한 회사의 **폐기된 첫 제품**(기본 브랜치 최종 커밋 2024-02-13). 🎯 **진짜 발견은 볼트 [[Higgsfield]] 페이지의 5개월 공백** — CLI ★555 · `skills` ★1,070(**evals/** 포함) · JS/Python SDK · MCP 브리지. 🔴 ★+196(상대속도 3.85%, 배치 1위)의 **원인은 볼트도 모른다** · ⚠️ `pushed_at` ≠ 기본 브랜치 커밋
- [[Feyospace-v1]] `backlog` — 7인 팀 사이버 사후학습(업보트 **133**, 볼트 09-19 120 → 수집기 123). 궤적 164,269 · CyberGym **+23.76%** · s1 **63.24% / 10위**. 🔴 **초록의 중립 동사가 목차에서 바뀐다**: *bypasses* → **`2.3 Hongzwang: Jailbreak Tech`**, *analyzes* → **Signature Hack**, *reduces cost* → **Leverage Account**(5개 중 3개). ✅ Ethics Statement 실재(자체 샌드박스 한정) — ⚠️ 그 범위가 **제3자 교사 API 우회**까지 덮는지는 어디에도 없다. 🔴 코드·가중치 0
- [[StepAudio-3-Music]] `backlog` — **ABC 표기법으로 편곡 계획(ABC-CoT)을 먼저 쓰고** 음악을 생성(업보트 102). 50Hz 단일 코드북 65,536 + flow-matching DiT → 48kHz, 최대 **5분 30초**. 🎯 **[[m-a-p]] 와 결정적 차이 — 외부 아레나를 들고 왔다**(Music Arena Vocals *preliminary* Elo **1105**, Suno V5.5·Mureka 뒤 3위, 진 상대를 이름으로 적음). 🔴 *"among the evaluated systems"* 한정어 탈락 · 🔴 공개 아티팩트 0 · 🎯 **Mureka 가 두 논문에서 상위 — 볼트 미보유**
- [[Grouped-Value-Attention]] — **값만 저장하고 키는 학습된 선형 사상으로 복원**, 캐시 스칼라 **−45~47%**(업보트 76·저자 3). 🎯 KV 축 **세 번째 공격법**([[Random-Attention]] *고르지 마라* · [[Declarative-Attention]] *읽지 마라* · GVA ***두지 마라***). 🔴 **GQA에 0.18 뒤진다**(44.18 vs 44.36 · MLA 43.88) · 🔴 **속도는 저자가 "평가 중"** — 45~47%는 메모리 축이지 시간 축이 아니다 · 350M 단일 규모 · 코드 0
- [[ReactHuman]] — 급작 위험에 대한 **즉시 행동**을 재는 첫 물리 기반 벤치(17계열·1,000+ 장면·240Hz·주석 없음·**결정을 실제 실행**). MLLM 7종이 **약 3건 중 1건** 오처리, 외형>운동 신뢰, 요격점 **미터 단위** 이탈, 🔴 ***"none of these failures shrink with model scale"***. ✅ **초록 대조 불일치 0(수집기 정확)** · ✅ 데이터셋 공개(2,111파일) · 🔴 **업보트 52 ↔ 데이터셋 좋아요 1** = 주목·채택 분리 실측 · 🔀 `slam-3dgs` 재판정
- [[Agora]] — 연구 기록을 **Git append-only DAG**로(12일·워커 13·1,703 기여·**3.39 → 1.899 bits/byte**·재현 165건 실패 0·[[NVIDIA]] 포함 저자 8). ✅ **[[검사가능성-공사]] 3번째이자 가장 구조적**(미검증·방치된 가지가 **조회 가능한 인덱스 차원**, 자기인용 제외). 🔴 **그런데 레포에 코드가 0개**(LICENSE·README·assets·index.html) → [[검사가능성-후퇴]]. 🔴 **초록 말미 3항목 누락**: 사람 **1회 개입**(§4.7) · 입증 범위 · **대조실험 미실시**(부록 C가 *Proposed*)
- [[Ternary-Bonsai-2-27B]] — 전 구간 삼진 27B(DL **1,908,396** · Apache-2.0 · 14벤치 84.78 = FP16의 98.2% 주장). 🔴 **수집기 "실측 1.72bit/5.9GB"는 `(ideal)` 행** — 출하물은 **1.75bit/5.95GB(PTQ1_0)** 와 **2.13bit/7.21GB(PQ2_0)**, *"~47 tok/s"* 는 **PQ2_0 쪽**. 🔴 *"고정밀 예외 없음"* 정정(0.0976% 존재·포함 계수). 🔴 **stock llama.cpp 불가 — `Q2_0` 은 무경고 쓰레기 출력**(→ [[Bonsai-27B]] 옛 actionable 무효). 🔴 **v1의 `.eval_results/` 제거** → [[검사가능성-후퇴]]
- [[Swift-Qwen3.8-27B-GGUF]] — 사고 토큰 절감 파생 GGUF(DL 136,668). 🎯 **볼트가 09-19에 도착을 예고한 항목.** 🔴 **수집기 "GGUF 자체 측정치 아님" 정정 — 표 3개 중 3번이 GGUF 전용 24 tier KLD/Top-p 표**. 🔴 *"<1% loss"* 가 **9행 중 4행에서 거짓** · *"58.3%"* 는 **18셀 중 최댓값**(평균 37.2%) · *"x1.95 가속"* 은 **수집기 미전달 + 근거표 없음**. 🎯 **KV 64KiB/token → 262K에서 16GB** · **MTP 내장** · ✅ `base_model_relation: quantized` 명시

### 2026-09-19 배치 (13건 리터럴 전량 NEW · 5배치 연속 중복 0% · 🆕 7일 누적 업보트 창 첫 적용 · 서브에이전트 4병렬 본문 대조)

- [[agent-lightning]] — Microsoft 3,500행 하네스형 에이전틱 RL(★18,356): 에이전트 코드 그대로 프록시로 궤적 수집 → RL. Qwen3.5-9B SWE-bench V 41.8→56.4(6K 샘플) · 검색 +16.6 · 지시수행 +18.3(자체 검증셋). 🎯 롤아웃당 2.41 샘플 → 롤아웃 단위 정규화 → [[하네스형-에이전틱-RL]]. 🔴 절제 비단조(33.1<35.0<38.2) · ★는 v0.x 몫 · 수집기 "수치는 이미지뿐" 정정(본문에 텍스트) *(NEW 2026-09-19)*
- [[marin]] — Stanford CRFM·Open Athena 오픈 개발 LLM 플랫폼(★3,751, 이슈 500:PR 97 = 실험 장부). 🎯 32B 역방향 GSM8K 오염(−22점) → [[포맷-불일치-오염]] · Delphi 사전등록 333배 외삽 손실 0.2%(🔴 CI ±4%, 1차 발산). 🔴 "Llama 3.1 8B 이김"은 베이스·자체 19과제 — SFT 는 Tulu 에 짐(회고에만) *(NEW 2026-09-19)*
- [[gitdiagram]] — 레포 → LLM 그래프(AST) → 결정론 컴파일러 Mermaid(★16,579, hub→diagram 치환). 🎯 인제스트 "어디를 읽을지" 지도. 🔴 경로 오류 노드는 재시도 없이 링크만 떼고 남김(코드 확인) · 입력 트리 24k·소스 48k자 · 품질평가 0 *(NEW 2026-09-19)*
- [[tradingview-mcp]] — Claude Code ↔ TradingView Desktop CDP:9222 MCP(★6,540). 도구 **84**(README 78 오기) · Pine Script 컴파일-수정 루프. 🎯 [[Electron-CDP-브리지]] 설계도. 🔴 PR 195건 적체·7주 무병합 · 포크/스타 41.6% · 비공식 내부 API·약관 충돌 저자 명시 *(NEW 2026-09-19)*
- [[GLiNER2]] — [[Fastino]] 스키마 정보추출 레포(★1,977) = 컨테이너(아키텍처 2 ⊃ 체크포인트 9) → [[컨테이너-중복]] 사례 2. 🎯 논문 v1: 50라벨 CPU 208ms. 🔴 GLiNER2 클래스로 2.5 로드 불가 · 2.5 품질 수치 0 · 논문 서술 오류 1 · 기본 설치 = 클라우드 클라이언트 *(NEW 2026-09-19)*
- [[gliner2.5-multi-v1]] — Fastino 287M 다국어 boundary 추출(30일 DL 183,555). 🎯 287M 중 190M 임베딩 → 연산 86M급 · LLM 없는 로컬 추출기. 🔴 카드 594MB FP16 ↔ 실물 FP32 1.15GB · 한국어는 CC100 사전학습까지만 · 청크 경계 넘는 관계 삭제 · 벤치 0 *(NEW 2026-09-19)*
- [[LimiX-2]] — [[Stable-AI]](🔴 Stability AI 아님)·칭화대 표형 FM(업보트 261·저자 60): p(y|x) → p(x,y) 빈칸 채우기로 분류·회귀·보간 통합. 🎯 TabArena Elo 1935(TabFM+ 1818). 🔴 CI 겹침 · BCCO 자사 벤치 · 결론에서 "골격"→"인과 추론" 확대 · v2 비상업 *(NEW 2026-09-19)*
- [[ModularRSI]] — 하네스 5모듈 독립 진화 + 벤치 분리 2,000과제(업보트 171·★28, 코드·데이터·궤적 전부 공개). 🎯 예산 맞춘 AHE·Meta-Harness ≈ 기준선+1 vs 본 방법 TB2.0 61.79→67.42 = "벤치 위 진화 = 벤치 적응" 첫 통제 증거 · **루프 16 에폭·14세대 = RSI 최소조건 첫 충족**. 🔴 수 과제 단위 이득 · 분산 0 · 표4 예산 5배 · SWE→TB Pass3 변화 0 *(NEW 2026-09-19)*
- [[RSIAgent]] — 학습 없는 3에이전트 탐색으로 메모리 파일 축적(업보트 74·★343). 🔴 폐쇄 모델은 보고치 복사(저자 부록 인정) — 메모리 없는 하네스만으로 ALE 83.75 > GPT-6 82.26, RSI 순기여 OSWorld +7.01 · ALE +1.07 · ALE Binary 는 GPT-6 에 짐 → [[비매칭-비교]]. 📌 verifier `UNVERIFIED` = [[검사가능성-공사]] 4번째 수렴 *(NEW 2026-09-19)*
- [[PhysBrain-1.5]] — [[DeepCybo]] 이해·행동·예측 단일 토큰열(Qwen3-VL-8B + [[ActionPiece]] 어휘). 28벤치 72.5 · 오픈 1위 14. 🎯 지는 14개가 포인팅·궤적에 몰림. 🔴 폐쇄 모델 최저 설정·자체 재평가 · 13벤치 학습분할 · 제어/예측 정량 0(1.0 의 LIBERO 평가 사라짐) · 🔀 slam-3dgs 재판정 *(NEW 2026-09-19)*
- [[MiniMax-H3-Physical-Reasoning]] — NUS·Fudan·Tencent 의 [[MiniMax-H3]] 외부 평가: 517건 사람평가 SR 41.97%(VDR 56.00 · ADR 27.40). 🎯 벤더 정량 벤치 0 → 볼트 유일 측정치. 🔴 비교모델 0 · 오픈가중치/API 경로 미기재 · 데이터 미충전 · 저자가 모달리티 간 비교 금지 명시 *(NEW 2026-09-19)*
- [[K2-Horizon-7B]] — [[MBZUAI]] IFM dense, "7B-core" 라벨이지만 **9.0B**(비임베딩 6.95B + untied 2.05B, 볼트 산술 정확). 🎯 중간 체크포인트 69 태그 실존 · [[Uno]] 베이스 확정. 🔴 카드 8행 전승 ↔ 블로그 표 SciCode 패배(Gemma 38.2) · 블로그 "SWE 답안 다운로드 → 82점" · 학습코드 빈 레포 · 🔀 local-llm 재판정 *(NEW 2026-09-19)*
- [[Xing4.0-29B-A4B]] — [[China-Telecom]] 29B-A4B MoE(구 TeleChat). "Ascend+MindSpore 첫 모델"은 크기 한정어로만 성립(전작 105B~1T 이미 Ascend). 🎯 29B vs 31.2B = MTP + 중복 임베딩/헤드(볼트 산술). 🔴 9행 중 3승(Qwen 4) · SWE-V 굵은 글씨 오류 · 추론 PR 5건 전부 미병합 · 🔀 local-llm 재판정 *(NEW 2026-09-19)*

### 볼트 백로그 소스 (2026-09-19) — 1건 (볼트 자체 발견 · 수집기 경로 아님)

- [[NeoHorse-1-Paper]] — [[TokenRhythm]] 모논문 `arXiv 2609.08183`(저자 37 · 09-08). 🎯 **배포 중인 라우팅 하네스가 곧 데이터 공장** — 매 턴의 예측 티어(C0~C3)·실제 서빙 티어·결과를 분리 기록해 **궤적 + 커리큘럼 순서 + 다음 혼합 결손 신호**를 동시에 얻는다. ✅ **최강 증거는 데이터 출처 대조**(같은 레시피, 하네스 궤적 vs 공개 Toucan → 5벤치 **+6.26**, 양 스케일링은 +2.14 → **출처 > 양**). 🎯 품질 게이트 `PASS/WARN/FAIL/NOT_EVALUATED` = [[검사가능성-공사]] **3번째 수렴**. 🔴 **루프 1회(single pass) 저자 확정** → [[RSI-프레이밍]] · 커리큘럼 절제 없음 · 교사 비공개 · **레포 ★599에 학습코드 없음**(추론 예제 2개) · 초록 "eleven"은 오기(표는 10벤치). 🔴🔴 **볼트 자기 정정: 업보트 421 → 170**(동일 API 하루 차이) → [[선발창-누락]] 배수 하향. ✅ 조직 소속 확인(TokenRhythm Technologies·Infinigence AI·칭화대·[[Alibaba]] 등 8기관) *(NEW 2026-09-19)*
...
**GitHub 5 — 🎯 5건 중 3건이 "하네스/에이전트를 감싸는 층"** (09-17 *"실행 통제"* 가 **통제 → 설계**로 한 단계 더 올라갔다)

- [[BrowserSkill]] — [[Tencent]] ★**4,740**(+5 드리프트)·fork 335·MIT·**당일 커밋 09:09**. 🎯 **상대속도 27.5% = 볼트 관측 최상위권**(09-17 최고 [[Octop]] 12.61%의 **2.2배**). 발명은 자동화가 아니라 **소유권 분리** — CLI→데몬→확장 3단 경유로 에이전트를 **별도 Agent Window에 격리**, 사용자 탭은 **빌린 뒤 반납**. 🎯 [[에이전트-웹접근]] 3노선 중 **지문 위조가 원리적으로 불필요한 유일한 노선**. 🔴 *"expected to work"* = 지원 아닌 예상 · 벤치 0개 · **탭 반납 강제 미확인** *(NEW)*
- [[opensre]] — ★**11,124**(**드리프트 0**)·fork 1,623·Apache-2.0. 🎯 **저자가 *"AI SRE ... remains **unsolved**"* 를 적었다** → [[자기제한-명시]] **최상위 형태**(기능이 아니라 분야 전체의 미해결성). ✅ **그리고 이 선언이 벤치 부재를 설명한다** — 📌 **볼트 규칙 신설: "벤치 0개"는 *주장 있음+근거 없음*(감점)과 *주장 없음*(성숙도)으로 갈린다.** [[MiroFish]] 와 표면상 같고 결정적으로 다르다. 🔴 상대속도 **0.18% = 배치 최저**([[BrowserSkill]] 과 153배) *(NEW)*
- [[harness-sdk]] — ★**7,345**(+1)·fork 1,153·Apache-2.0·**배치 최고령(1년 4개월)**. 🔴🔴 **볼트 최대 발견: open issues 763 = ★의 10.39%, 배치 5건 중 유일한 이상값**(중위값 1.08%의 **9.6배** · 최저 [[MiniCPM]] 0.16%의 **65배**) — **무설정 동작·`Guardrails catch mistakes` 주장과 부딪힌다.** 🆕 **`open issues/★` 척도 신설**(정상 0.16~1.1% · 4%+ 확인 대상). 🔴 `any model, any cloud` ↔ **기본 프로바이더 Bedrock** = [[한정어-탈락]] **역방향** · 벤치 섹션 자체가 없음 *(NEW)*
- [[FastVideo]] — ★**4,467**(**드리프트 0**)·fork 465·Apache-2.0. DMD2 증류 × Video Sparse Attention **곱셈 구조**. 🔴 **볼트 검산: 50배 산술이 닫히지 않는다** — 8-step은 50-step 대비 **6.25배**이므로 VSA가 **8배**를 내야 하는데 **그 수치가 없다.** 🔴 **README 208행에 성능 대조표 0개**(근거는 외부 블로그) · *">50x 디노이징"* 과 *"5s→1.8s E2E"* 는 **분모가 다르다** → [[단위-불일치]] **영상 도메인 첫 사례**. 🔴 ★ 증분 결측 = **상대속도 계산 불가**(배치 유일) *(NEW)*
- [[MiniCPM]] — [[OpenBMB]] ★**11,078**(**드리프트 0**)·**open issues 0.16% = 배치 최저**. 🔴 **레포 1개 ⊃ 모델 4계열** → **★11,078은 어느 모델의 지표도 아니다.** README **1,373행·표 19개·평가 섹션 4개**(각각 소유 모델이 다름). ⚖️ **판정 (c) 채택** + 🆕 **[[컨테이너-중복]] 신설**(파생 중복과 구조가 **반대**: 파생은 이름이 갈라져서, 컨테이너는 **이름이 하나여서** 못 잡는다 · 09-17의 2·3차 키는 **채널을 넘으면 무력**). ✅ 신규분 **SALA**: 희소 25%+선형 75%, **520K 학습으로 2048K에서 81.6**. 🔴 3.5배 일부는 **Qwen3-8B OOM 대비** *(NEW)*

**HF논문 5 — 🎯 4건이 "하네스"이고, 그중 하나만 대조 실험을 했다** (🔴 그리고 **원인 논문을 볼트가 열흘째 못 받고 있다**)

- [[SoL-Pi]] — 업보트 **35**·저자 14. 🔴🔴 **볼트 최대 발견: [[NVIDIA]] NVlabs이고 코드 ★2,185다 — raw에 "NVIDIA"가 없었다.** 하네스 자동연구 루프 → **선택압 통과 4개 메커니즘**(그중 2개가 컨텍스트). 51과제 EdgeBench **토큰 −44.7~49.0% · API 비용 약 1/3**, 정확도는 *"comparable"*. 🎯 **토큰을 줄이는 주장을 NVIDIA가 했다 = 자기 이해관계와 반대 방향.** 🔴 **기준선이 둘**($8.75~13.50 vs Pi 대비 **$4.36~5.71**) → [[표-부분인용]] **네 번째 축(기준선)** *(NEW)*
- [[Harness-Design-Empirical]] — 업보트 **31**·저자 9. 🎯 **배치에서 유일하게 대조 실험을 했다** — 루프 고정 + 3요소만 변주해 **176개 설정**(5전략×4예산+절제) × 4모델 × SWE-Bench V·TB 2.1. **결론 4개 중 셋이 "빼라"**: 복구 장치는 **모델이 안 써서 이득 0** · 사전정의 도구는 bash 능숙 모델에 무용 · 계획은 강한 모델서 **정확도 불변**. ✅ **결론 ②를 볼트 인제스트 규칙으로 승격**(오늘 raw.md 454KB 처리에 실제 적용). 🔴 **코드 없음 · 4개 모델 익명** *(NEW)*
- [[DeepSeek-V4.1-Flash-Paper]] — 업보트 **36 = HF 데일리 1위**. 🔴 **저자 592명**(배치 나머지 4건 합 45의 **13배**) → 📌 **저자 수는 신뢰도가 아니라 *종류* 지표: 592 = 기업 백서(외부 재현 불가) · 9~14 = 연구 논문.** 🔴 **초록에 비교 수치 0개**(*"substantially better"*) — **반면 볼트 [[DeepSeek-V4.1-Flash]] 카드 페이지는 HLE 36.8 vs 56.3 등 불리한 축까지 12개 축 보유.** 🎯 **역방향 [[한정어-탈락]]: 요약이 약점을 먼저 버린다** *(NEW)*
- [[When-EOS-Tokens-Disagree]] — 업보트 **28**·코드 `UNCSciML/opd-eos` **★1**. 🎯 **[[온폴리시-증류]] 3레버 밖 네 번째 층(어휘)** — Qwen3·Llama·Gemma에서 **선언된 정지집합이 같아도 확률이 다른 EOS에 실린다.** 기제: 학생 선호를 **억압하면서 교사 대안은 전달 못 한다 = 양쪽 다 잃는다.** ✅ **[[자기제한-명시]] 논문 채널 최강** — *"insufficient"*·*"important, but **not exhaustive**"* 를 초록에 적었다. 🔴 ★1에 **72시간 규칙 적용 = 지표 아님** *(NEW)*
- [[JEPA-Anything]] — 업보트 **21 = 배치 최하위**·저자 13·코드 ★3. **직교 예측 인자분해(OPF)** 로 7도메인 확장. 🎯 **케플러 지수 -1.4991 복원**(이론 -1.5) = 정확도가 아니라 **옳은 법칙을 배웠다는 증거** · 개입 예측오차 **−34.8%** · **습식 실험 지지**(오가노이드·마우스) → [[검사가능성-공사]] **새 칸**. 🔴 **볼트 최대 지적: 비교군이 *"matched JEPA baselines"*** = 도메인 전용 SOTA 대비 아님 · 습식 검증은 **개입 *단수*** · **7도메인/13명 = 1.9 밀도** *(NEW)*

**HF모델 3 — 🔴 드리프트 0(3건 전부) · 그러나 `trendingScore: None` 도 3건 전부**

- [[NeoHorse-1-4B]] — [[TokenRhythm]] DL **19,789**·♥**2,354**(둘 다 raw 일치)·**4.21B** 실측·apache-2.0. 🔴🔴 **볼트 최대 발견: 모델 태그 `arxiv:2609.08183` 추적 → 모논문 업보트 421 · 코드 ★535 · 볼트 0히트.** 오늘 1위(36)의 **11.7배**이고 볼트 09-08 배치 1위는 [[Uno]] **19** 였다 → 🆕 **[[선발창-누락]] 신설.** 8열표 전건: **이기는 4축 전부 에이전트**, 🔴 **지는 6축**(BFCL·VitaBench·HumanEval·LCB·IFBench·IFEval). 🔴 **1회/3회 측정 혼재**(이기는 PinchBench가 1회) · 판정자가 [[DeepSeek-V4.1-Flash]] · DL:♥ **8.4:1** 이례적 저값 *(NEW)*
- [[NeoHorse-1-9B]] — DL **9,856**·♥881(raw 일치)·**8.95B**(🔴 "9B"는 반올림). 🎯 **[[에이전트축-분기]] 가 잠정을 벗었다 — 처음으로 *자기 베이스 대비 음수*다**: IFEval **89.09 vs 베이스 89.46 = −0.37** · IFBench **+0.00** · LCB v6 **+0.00** ↔ VitaBench **+11.00**. **4B는 10축 전부 +, 9B는 아니다 = 같은 레시피가 규모를 키우자 지시수행에서 비용을 치른다.** 📌 **평균 1위가 10개 축 3:3:4 분포를 덮는다**(Gemma-4-12B 3축·Muse-Glimmer 3축·NeoHorse 4축) *(NEW)*
- [[mms-300m]] — [[Meta]] DL **22,002**·♥565(둘 다 raw 일치)·**cc-by-nc-4.0 = 비상업 전용**·1,400+ 언어·50만 시간. 🔴🔴 **볼트 실측이 카드를 구조적으로 확증: `pipeline_tag: None`** — **HF가 과제를 배정하지 못한다.** *"should be fine-tuned"* 는 겸손이 아니라 사실. 🔴 **`lastModified 2023-06-05`** → `How to finetune` 이 *"Coming soon"* 인 채 **3년 3개월** = **포기된 문서.** 🎯 **그래서 재부상은 전적으로 수요 측**(공급 변화 0). 09-15 *"상시 인프라 바닥"* **4번째** *(NEW)*

**판정 1 + 🔴 볼트 역발견 1**

- [[컨테이너-중복]] — ⚖️ *"레포가 이미 보유한 모델들을 담고 있다"* 판정 요청 **3**에 대한 답: **(c)**. 🎯 **[[파생저장소-식별]] 의 *"새 축이 문서화됐는가"* 기준을 그대로 적용했고, 구조는 정반대다** — 파생은 **원본 1 → 파생 N**(이름이 갈라져서 못 잡음), 컨테이너는 **레포 1 ⊃ 모델 N**(이름이 하나여서 못 잡음). 🔴 **09-17의 2·3차 키가 채널을 넘으면 무력**(GitHub에 `base_model` 없음 · 부분문자열은 과탐). 📌 **탐지 경로는 자진신고뿐이고 수집기가 실제로 자진신고했다 — 이번 배치 최고의 기여.** 🆕 **컨테이너 페이지 규약: 기존 개별 페이지의 수치를 복제하지 않는다** *(NEW 2026-09-18)*
- [[선발창-누락]] — 🔴🔴 **볼트가 수집기 배달에서 역으로 찾아낸 구조적 누락.** *"HF 데일리 상위 5"* 는 **업보트가 며칠에 걸쳐 쌓이는 논문을 창 밖으로 버린다.** 🎯 **[[DeepSeek-V4.1-Flash]] 가 다운로드에서 확정한 72시간 규칙의 업보트 버전** — 지표마다 시간 상수가 다르다. ⚠️ **정직한 한정: 421은 오늘 측정값이고 당일엔 낮았을 수 있다. 그러나 열흘간 아무 채널도 되돌아보지 않았다.** 📌 **볼트 요청: 주 1회 "지난 7일 누적 업보트 상위" 별도 슬롯**(09-14 "중복 재배달" 실패와 달리 **미수집 확인 후 신규 배달**) *(NEW 2026-09-18)*


### 2026-09-17 배치 (13건 리터럴 전량 NEW · 3배치 연속 중복 0% · + 판정 1)

**GitHub 5 — 🎯 5건 중 3건이 "다른 에이전트를 감독하는 층"이다** (09-16의 "모델 위 계층"이 **실행 → 실행 통제**로 한 단계 올라갔다)

- [[security-audit-skill]] — [[Cloudflare]] ★**8,529**(+23)·fork 474·MIT. 🎯 **배치의 척추**: 판정 3분류를 스키마로 강제 — `needs_validation` = *"an exact unresolved fact and **no severity**"*. **심각도를 붙일 수 없는 칸을 따로 만든 게 설계의 중심.** ✅ 볼트 추가(README 90행): 샌드박스 없으면 *"keeps the lead as `needs_validation` **instead of executing target code**"* = **실패 모드를 미리 선언.** 🔴 탐지율 수치 없음 · `topics` 0개 *(NEW)*
- [[oh-my-hermes]] — ★**2,657**(+4)·fork **189 완전일치**·MIT. [[NousResearch]] Hermes 위 운영 계층. 🔴 **볼트 최대 정정: 수집기가 *"별도 문서 확인 필요"* 라 한 정의가 같은 파일 586행에 있었다**(README 660행 중 48행만 읽음). 4상태 어휘: `Plan · not run` / `Code · running` / **`Code · reported done`**(*"실행자가 끝났다고 말했다. 아무도 확인하지 않았다"*) / `Test · verified`. 🎯 **그리고 자기 벤치마크 칸에 *"No measured run has been published yet"* 을 적었다** = **자기 기준의 재귀 적용** → [[자기제한-명시]] **최강 사례** *(NEW)*
- [[Octop]] — [[Tencent]] ★**3,147**(+6)·MIT·**상대속도 12.61% = 슬레이트 최고**. 채널 7종·백엔드 4종을 **양쪽 다 교체 가능**하게. 🎯 `octop acp` 로 **OpenCode/Claude Code에 권한 게이트 걸어 위임** = 코딩 에이전트를 **피호출자로 격하.** 🔴 *"digital life form"*·MBTI 16 페르소나는 **프롬프트 템플릿** · 벤치 수치 548행 전체에 **없음** *(NEW)*
- [[cline]] — ★**68,506**·Apache-2.0·2년 2개월차. 🔴 **볼트 관측 사상 첫 음수 드리프트(−1)** — 당일 +112(**0.16%**)인 레포가 몇 시간에 −1 = **일간 신호가 언스타 노이즈와 같은 자릿수** → [[상대속도-가림]] 에 **측정론적 근거**. 🔴 헤드리스 전용 섹션이 **219행**(수집기 48행): *"Pipe input, get JSON output, chain commands"*. 성능 수치 235행 전체에 없음 *(NEW)*
- [[wshobson-agents]] — ★**39,749**(+1)·fork **4,235 완전일치**·MIT. 94플러그인/202에이전트/183스킬/105커맨드를 **6하네스로 변환 배포**. 🎯 *"idiomatic, harness-native artifacts — **not lowest-common-denominator translations**"*. 🔴 `topics` **20개 = 만석** → 09-16 [[omniget-재판정]] 패턴 **두 번째 확증**(부재가 정보 0). 상대속도 **0.10%** = [[Octop]] 과 **126배 차이** *(NEW)*

**HF논문 5 — 🎯 5건 중 4건(80%)이 "먼저 이름을 붙인다"** (개선 주장 전에 측정 대상을 재정의)

- [[ScienceIDE]] — 업보트 **63**(드리프트 0)·**HF 데일리 1위**·저자 **45명**. 과학 코드 레포를 에이전트 학습 환경으로 변환 — 병목을 **`scientific experience bottleneck`** 이라 명명. **전문가 정의 합격 기준**이 루프의 앵커(→ [[ProgramDistill]] 과 **반대 자동화 축**). 🔴 *"across **selected** general-purpose benchmarks"* · **구체 수치가 초록에 없다** *(NEW)*
- [[XConf]] — 🔀 raw `ai-news` → **`local-llm`**. 업보트 **44**·HF 2위. 🎯 **기존 신뢰도 추정 전부가 "지금 이 추론"만 본다는 전제를 부정** — 과거 채점 이력(태스크·반성·당시신뢰도·결과·교훈 **5필드**)에서 신뢰도를 끌어온다. **24개 비교 중 23개** AUROC 우위·ECE 낮음·**비용 1/10** · 10% 포기 시 **+8.7점**. ✅ **logit·가중치 업데이트 불필요 = API-only 적용 가능.** 🔴 볼트 발견: 9벤치×4모델=36인데 비교 **24** — 산술이 닫히지 않음 *(NEW)*
- [[ProgramDistill]] — 업보트 **40**. 지시문이 아니라 **동작하는 참조 앱을 조작해** 기능을 알아내는 벤치. `mine-craft-patch` 가 26앱에서 **replay-verified 행동 1,975개** → **사람 개입 없이 4,063 태스크**. 🔴 **지는 축을 저자가 앞세웠다**: GPT-6 Astra **49.2%** / Claude Opus 5 **28.8%** = **최고도 절반 미달.** 🎯 복원 깊이 1→8에서 **100%→64.0%·96%→32%** = 난이도 통제 증거 *(NEW)*
- [[SP3O]] — 업보트 **35**. PPO 크리틱의 **`Value Flattening`** 명명(실제 상태가치는 급변하는데 크리틱 예측은 평평). 원인 2개: 손실의 **암묵적 분산 페널티** + 시간 상관 상태의 **중복 업데이트**(→ [[선택비용과-중복성]]). 처방이 반직관: 감독을 **줄인다**(응답당 **3개 상태만**). ✅ 성립 조건: **상태공간이 커질수록 심해진다**(작은 태스크에선 안 보임). 🔴 개선폭 수치 없음(*"consistently improve"*) *(NEW)*
- [[ActionPiece]] — 🔀 raw `ai-news` → **`slam-3dgs`**. 업보트 **32**. 🎯 **[[측정도구-먼저-반증]] 교과서 사례**: MSE 같은 점별 오차로 액션 토크나이저를 재는 관행을 반증 — *"small individual errors do not fully characterize..."*, 압축 후 문맥별 조정이 *"diminished, distorted, **or even reversed**"*. 🆕 지표 **PRC**(거리 순위 보존 · **토큰 어휘·디코더 비종속**). LIBERO **94.8%** / 미학습 Plus **68.8%** = 🔴 **−26.0점** · VLA-Arena **51.5%** *(NEW)*

**HF모델 3 — 🔴 드리프트 0(3건 전부) · 그러나 결론 오류 2건이 여기서 나왔다**

- [[DeepSeek-R1]] — DL **740,958**(드리프트 0)·♥ 14,252·**DL:♥ 52:1**·MIT·**1년 6개월 미갱신**. 🎯 **볼트 최대 발견: 수집기가 뺀 2개 열이 결론을 뒤집는다** — `AIME cons@64` 에서 **7B(83.3) = 32B(83.3)** 로 완전 동일(`pass@1` 은 +17.1점). **32B의 우위는 능력 상한이 아니라 1회 시도의 신뢰성**이며 **파라미터 4.6배가 집계 예산으로 교환된다.** 🔴 6열 집계 시 **70B가 4열 최고**(카드 자체 볼드) — *"70B 하락"* 이 아니라 **"순위가 열마다 다르다"**. ✅ `arxiv:2501.12948` 본문 대조 완료(정상 사례) *(NEW)*
- [[Qwen3.8-27B-TWIN-TURBO-709-GGUF]] — DL **114,335**(드리프트 0)·♥150·**DL:♥ 762:1**·[[DavidAU]]. 🔴🔴 **볼트 최대 정정: 이 레포의 거부율은 0/100이 아니라 68/100이다** — 카드가 `[ THIS REPO/ MODEL ]` 이라 명시한 줄이 **`68/100 refusals // KL 0.0025`** 이고, 수집기가 옮긴 **0/100 은 STAGE 1 = 타인(trohrbaugh) 작업**이다. 🎯 **4점을 읽으면 축이 드러난다: KL이 낮을수록 거부가 남는다**(거부는 원본 분포의 일부) — 저자도 *"ultra low KLD **first**... low refusal rate **second**"* 라 적었다. 🔴 arc-c 709/701/591 **분모·하네스 미공개 = 확인 불가** · `pipeline_tag: image-text-to-text`(텍스트 카드인데 비전) *(NEW)*
- [[Comfy-Org-YuE2]] — DL **59,231**(드리프트 0)·**CC-BY-NC-4.0**·파일 5개. 카드 전문 29행 = *"Repackaged model files for ComfyUI"*. 🔴 **볼트 발견: 채택 근거였던 `int8_convrot` 이 카드에 미문서화** — 파일은 실재하나 **배치 위치·의미·손실이 한 글자도 없다** → *"bf16/int8 두 점 비교"* 불성립(**파일 존재 ≠ 사용 가능한 축**). 🔴 `tags` 에 [[YuE2-3B]] 누락 + `finetune` **오라벨** → [[파생표기-함정]] **5번째**. ⚖️ **(b)→(c) 병합 정정**(페이지는 파생 추적 증거로 보존) *(NEW)*

**판정 1 — 수집기 질문에 대한 답**

- [[파생저장소-식별]] — ⚖️ *"재포장·양자화 파생을 중복으로 볼 것인가"* 에 대한 볼트 판정. 🔴 **문제 크기를 먼저 쟀다: 트렌딩 상위 200의 14%(28건)가 단일 베이스 가족.** 2차 키는 `tags` 아닌 **`cardData.base_model`**, 단 **1홉만** 잡히므로 **이름 부분문자열 3차 키 + 본문 사슬 확인** 필요. 🎯 **판정 기준: 새 축이 *문서화*됐는가** *(NEW)*
...
**GitHub 5** — 🎯 에이전트 축이 **모델 위 레이어**로 올라갔다(셋 다 모델을 바꾸지 않는다)
- [[firstmate]] — ★6,003(+978·**16.3%**)·MIT. 단일 창구 에이전트가 crew를 스폰, 각자 **독립 git worktree**. **설치할 앱이 없다** — `AGENTS.md`+스킬+스크립트 디렉토리이며 하네스 클론 안에서만 인스턴스화. 🔴 미결이슈 **23.2%**(배치 최고, 판정 보류) · macOS/Linux 한정
- [[all-agentic-architectures]] — ★4,507(+217)·MIT. 35개 에이전틱 패턴을 **동일 `.run(task)` 계약**으로 통일 → 클래스 교체만으로 패턴 교체. deterministic-picker **13/35 적용·9개 면역 = 나머지 13개는 방어 없음**. 🔴 배지 전부 자체보고 · 85일 정체
- [[ai-engineering-hub]] — ★37,549(+134)·MIT. 노트북 93개+. 🔴 **재사용성이 all-agentic과 급이 다르다**(복사 구조 vs 통일 계약) — **스타는 8배 많은데 역방향**. README 상단이 뉴스레터 유입
- [[MiroFish]] — ★**73,504**(+560)·**AGPL-3.0**. GraphRAG로 평행 세계 구성 후 수천 에이전트로 미래 예측. 🔴 **예측 정확도 벤치 0 · 정성 데모뿐** → reliability **low**. 배치 스타 2위인데 신뢰도 최저
- [[huggingface-transformers]] — ★166,137(+536)·Apache-2.0. **제품이 아니라 호환성 계약.** 🔴 **상대속도 0.32%로 배치 최저** — "급상승"이 아니라 8년차 기저 유입. 📌 1,043소스 중 페이지 부재였음(중복 오탐 38히트 당사자)

**HF논문 5** — 🎯 업보트 순서와 검증가능성 순서가 거의 반대다
- [[DataFlex-RL]] — 업보트 **108(꼴찌)** · 신뢰도 **high(배치 최고)**. 🎯 **이 배치 최대 수확.** 동일 9런을 수학편중 6벤치 vs 균형 12벤치로 재채점 → **순위 상관 -0.33**. 롤아웃 선택·재가중 **8기법 전부 95% CI가 0을 포함**, 적응혼합 3종도 균등혼합을 못 이김. 균일 GRPO만 +7.76%p. → [[측정도구-먼저-반증]] 최강 실증
- [[Atria-Dawn]] — 업보트 221 · 저자 **143인**. 16벤치 competitive, **그중 5개** 최고(나머지 11개 순위 미기재). 769건 작업기록/56명 분석: **1/3이 "AI 없이는 불가능"**(🔴 자기보고 평정, 통제실험 아님). 🔴 **제목만 "Superintelligence"** — 초록은 전부 한정적 → [[한정어-탈락]] 새 양식(저자가 제목에서 일으킴)
- [[Dream-RSI]] — 업보트 164. 발견 트리를 **리플레이 시뮬레이터**로 재사용해 off-policy로 탐색 정책 개선. 📌 **개선 주체는 모델이 아니라 탐색 정책**(기반 코딩 에이전트 불변). 🔴 *"competitive **or** improved"* · 비용절감 *"several settings"* 한정 · **초록에 수치 0**
- [[ZGCM-1]] 🔀 **→ local-llm 재판정** — 업보트 160. 7B 밀집·전 단계 공개(가중치+중간체크포인트+코드+**데이터 레시피+W&B 로그**)·경험적 발견 8건. **압축이 아니라 "도구로 용량 한계 우회"** 설계. 🔴 235B급 competitive는 **일부 수학·에이전틱 스위트 한정** · 4.2×는 **16K time-to-loss 기준**
- [[Vidu-S2]] 🔀 **→ video-saas 재판정** — 업보트 150. S2-Avatar(540p→**720p** 실시간·동적 레퍼런스)+S2-Editing(**스트림 위** 의상·인물·배경 교체)+spatial. 🔴 **성립조건 부재 — 배치 유일.** 초록 전부가 *"outperforms all baselines"*, baseline·벤치·수치 **전무**. **S1(medium)보다 신뢰도 낮음(low)**

**HF모델 3** — 🔴 전부 신규 릴리스가 아니다(다운로드 절대량 정렬의 결과) → [[상대속도-가림]]
- [[clip-vit-base-patch32]] — DL **2,150만/월** · DL:♥ 14,046:1 · trending 179. 🔴 **라이선스 미선언**(API 실측 확인) · 🔴 카드가 **"일반 배포용 아님"** 을 명시하는데 월 2,150만이 다운로드된다. 2024-02 이후 갱신 없음
- [[distilbert-base-uncased]] — DL 731만/월 · apache-2.0. GLUE test 8축 기재. 🔴 **CoLA 51.3 · RTE 59.9 — 8개 중 2개가 50%대**인데 단순평균 79.5가 덮는다. 🔴 BERT-base 상대 비교표가 카드에 없어 *"더 작고 빠르다"* 의 정량 근거 부재
- [[Llama-3.1-8B-Instruct]] — DL 571만/월 · **`gated: manual` → 카드 raw 401 차단**. 🔴 **벤치 수치 미기재(확인 불가 · 추정으로 채우지 않음)**. DL:♥ **750:1**로 배치 최저지만 **게이트 DL은 승인분만 집계** = 분모 정의가 다름 → [[측정도구-먼저-반증]] **세 번째 실패 양식(모집단 불일치)**


### 2026-09-14 배치 (13건 = **신규 5 · 🔴 중복 8** · GitHub 5 · HF논문 5 · HF모델 3)

> 🎯 **배치 결론**: 이번 배치의 최대 발견은 소스가 아니라 **수집 절차 자체**다. raw는 볼트가 09-13에 요청한 단 하나(*"헤드라인 수치의 성립 조건을 같은 불릿 안에"*)를 **13/13 완벽 충족**했다. 그런데 **이미 2배치 연속 작동하던 중복 탐지가 조용히 무너졌다** — 배달 13건 중 **8건이 볼트 기보유**.
> 🔴 **원인이 데이터로 확정된다**: 탈락이 있었던 채널(GitHub 4탈락·HF모델 4탈락)은 **배달분 중복률 100%**, 탈락이 0인 논문 채널은 **중복률 0%**. **필터가 비운 자리를 채운 후보에만 필터가 적용되지 않았다** → [[백필-우회]] 신설.
> 📌 **[[AgentGrad]] 원칙의 사각지대다** — 한 번에 하나만 바꿔서 타깃 지표는 개선됐는데, **감시 목록에 없던 통제 지표가 회귀했고 로그는 정상으로 보였다**(*"중복 8건 탈락"*).
> 🔴 **볼트 자신의 오류도 2건 드러났다**: ① 09-10 [[MiniCPM5-2B]] *"관심 대비 실사용 적다"* → 4일 뒤 DL **×52**로 반증 ② 08-30 [[OpenMontage]] *"푸시 정체 2주 넘기면 유지보수 중단"* → 임계선 하루 넘긴 직후 푸시 재개. **임계선을 건 설계는 옳았고 임계선의 값에 근거가 없었다.**

**HF 논문 5건 — 이 채널만 전량 신규** (⚠️ 업보트 전건 상승 드리프트 +0~+4)
- [[Benchmark-Radar]] `arXiv:2609.11115` — 업보트 **46**(raw 45)·저자 7인. 벤치마크 **자체를 레코드로** 매일 수집, 모델카드 인용과 점수 이력까지 보관. **소스 37개**(직접 커넥터 13 + 자체 피드 24) · **1,283 레코드**(4개 카탈로그 기반) · **790 레코드에 12,916 관측**. 🔴 **1,283 중 점수 있는 건 790(61.6%)** — 나머지 493은 껍데기. 🎯 *"benchmark saturation... **the limits of score comparisons**"* 를 **자기 감사 대상**으로 명시 → [[측정도구-먼저-반증]] 의 정면 사례. 🔴 재현율·정밀도 수치 0개 = 커버리지 주장 인용 금지 *(NEW 2026-09-14)*
- [[SAS]] `arXiv:2609.13141` — 업보트 **44**(raw 43)·저자 9인 · **local-llm 재판정**. 하드 Top-K가 LM 손실 그래디언트를 끊어 **어쩔 수 없이 dense 어텐션을 증류**했고, 그 순위는 *"**not directly aligned with their impact under a fixed attention budget**"*. 해법은 선택기 연속 점수를 **소프트맥스 안쪽에 로그로** 주입. 실무 3원칙(게이트 위치·현재블록 캘리브레이션·연속 점수 보존) + Triton/FlashAttention 커널. 🔴 **초록에 절대 수치 0 · 베이스라인명 0 · 베이스모델명 0 · 벤치명 0** — **성능 우위 인용 전면 금지**, 메커니즘만 채택 *(NEW 2026-09-14)*
- [[LIT]] `arXiv:2609.12641` — 업보트 **44**(raw 40, **배치 최대 드리프트 +4**)·저자 4인 · **slam-3dgs 재판정**. 로봇 정책이 **과제 무관 시각 단서**와의 우연한 상관을 이용하는 걸 끊는다. 1단계 **이미지 없이** 언어·상태·**종료 SE(3) 자세**로 행동 사전분포 → 2단계 그 자세를 복원하는 **잠재 인터페이스가 유일한 시각 통로**. LIBERO-Plus **+3.87~10.70%p**(Pi0.5·MolmoAct2·FAST-WAM·ImageWAM) · 실로봇 **+13.30~16.70%p**. 🔴 **성립 조건이 같은 문장에**: 실로봇 이득은 *"**unseen camera configurations, lighting variations, and distractors**"* 조건이고 통상 LIBERO는 *"preserving or improving"* — **이득은 분포 외에서만 난다** → [[분포내-우위]] 의 **거울상**. 🔴 절대 성공률·과제명 없음(%p 증분만) *(NEW 2026-09-14)*
- [[COBRA-Skills]] `arXiv:2609.11682` — 업보트 **26**(raw 24)·저자 9인. 스킬 최적화를 **예산 제한 순차 최적화**로 재정의 — 컨텍스트 밴딧이 *"promising **or** informative"* 후보에 평가를 배분(탐색·활용 균형). 6벤치×3모델, **SkillOpt 대비 비용 55~58% 절감** · **벤치당 예제 50개** · **타깃 모델 자신으로 생성·정련해도 작동**. 🔴 *"strongest **average** performance"* — **평균 최고이지 전승 아님**, 절대 점수 0개, 비교군은 SkillOpt 외 무명. 🎯 [[OpenMontage]] 700+ · [[Claude-Red]] 78 = 전수 평가 불가 구간이 이미 왔다 *(NEW 2026-09-14)*
- [[PLC-DPO]] `arXiv:2608.30597` — 업보트 **22**(**드리프트 0 — 배치 유일 완전 일치**)·저자 3인·발행 08-31(**14일 후 재부상**). DPO의 *"관측된 선호는 전부 신뢰 가능"* 가정을 깬다. **캘리브레이션된 정책-레퍼런스 마진을 온라인 증거**로 각 쌍을 **clean/flip/tie** 라우팅 — *"**actively correcting** supervision direction and strength rather than **merely filtering**"*. **57셀**에서 승률 **60.5 vs 55.5**. 🔴 **승률 기준선은 DPO 맞대결**이고 **차선책 이름 없음**. 🎯 **자기확증 진단**을 검증 항목에 넣은 게 백미 — 모델 마진으로 라벨을 고치면 생기는 구조적 위험을 저자가 먼저 짚었다 *(NEW 2026-09-14)*

**GitHub 5건 — 🔴 전량 중복 재배달 (기존 페이지 갱신 처리)**
- [[colibri]] `JustVugg/colibri` — ⭐**30,734**(3일 **+3,117·+11.3%**)·fork 3,305·Apache-2.0·C·당일 푸시. **09-11 기생성**. 패밀리 8→**9개**. 🔴 raw 신규 한정어 채택: *"744B·4 tok/s·TTFT 1.6s·disk 0"* 은 **`6× RTX 5090` 전문가 전량 상주 조건**이고 디스크 스트리밍 수치가 아니다 · MTP가 **적중률 85%에서 32% 손실** 자기 반증. 🎯 09-11 판정(*"속도 SLA 없음, 시맨틱만 보장"*) 유지·강화 → [[자기제한-명시]] *(UPDATED 2026-09-14)*
- [[VoiceStudio]] `debpalash/VoiceStudio` — ⭐**27,850**(11일 **+12,396·+80.2% = 볼트 추적 중 기간대비 최고**)·AGPL-3.0·Python. **09-03 기생성**(당시 15,454). ✅ raw의 *"TTS 16 + ASR 11"* 이 09-03 기록 *"27개 엔진"* 과 정확히 일치. 🔴 *"646개 언어"* 는 **카탈로그 합계**이고 같은 표 옆 칸이 *"실제 커버리지·품질은 엔진 의존"* 이라 적는다 — **그런데 GitHub description은 한정어 없이 646을 광고한다**(같은 레포의 두 표면이 다른 강도로 말함) → [[한정어-탈락]] *(UPDATED 2026-09-14)*
- [[system_prompts_leaks]] `asgeirtj/system_prompts_leaks` — ⭐**66,373**(71일 **+17,148·+34.8%**)·CC0-1.0·JS. **2026-07-05 기생성**·**71일간 미갱신 40줄 최소 페이지였다**(갱신 주기 없는 페이지는 조용히 낡는다). 최근 추가분에 날짜: Gemini 3.8 Flash·ChatGPT Work Codex **09-13**(pushed 09-13과 일치) · Claude Code headless(Fable 5.1) 09-05. 🔴 *"verbatim"* 을 **레포 내부에서 검증할 수단이 없다**(추출방법·캡처시점 로그 부재). 🎯 **⭐66k를 기능 성숙도로 읽으면 안 된다 — 문서 아카이브에서 별은 다른 것을 센다** → [[단위-불일치]] *(UPDATED 2026-09-14)*
- [[open-code-review]] `alibaba/open-code-review` — ⭐**24,055**(46일 **+7,752·+47.6%**)·Apache-2.0·Go·당일 푸시·[[Alibaba]]. **07-26 기생성**(당시 16,303). 분업 기준이 명확하다 — **정답이 있는 것(파일선별·번들링·룰매칭·줄번호 보정)은 결정적 코드**, 없는 것은 LLM → [[요약자와-판정자-분리]]·[[MetroLLM-Bench]] Tier1/2와 동형. 저자가 겨냥한 실패 3종 명시(부분 리뷰·**위치 드리프트**·프롬프트 요동). OpenSSF Gold·Git≥2.41. 🔴 **벤치가 이미지 1장, 본문 수치 0개 — [[AuK]]·[[VibeVoice-ASR-Streaming-7B]] 에 이어 3번째, 이제 관행으로 본다** *(UPDATED 2026-09-14)*
- [[OpenMontage]] `calesthio/OpenMontage` — ⭐**58,859**(15일 +4,550)·AGPL-3.0·이슈 320. **06-23 기생성**. 🎯 **08-30 반증 예측의 판정**: *"푸시 정체 2주(09-05) 넘기면 유지보수 중단"* → 실제 **09-06 푸시 재개**(정체 15일). **관측 설계는 옳았고 임계선 값에 근거가 없었다** — 과거 푸시 간격을 안 보고 "2주"를 골랐다. 제작비 공개 **60초 $1.33 · 50초 ~$4 · 쇼케이스 ~$5**, 🔴 **단 fal.ai(Kling v3)·Chirp3-HD 외부 유료 API 사용분**이고 무료는 로컬 GPU 별도 설정. 🎯 차별점은 **무료 스톡/아카이브에서 실제 모션 클립을 검색·편집**하는 비생성 경로 *(UPDATED 2026-09-14)*

**HF 모델 3건 — 🔴 전량 중복 재배달** (⚠️ `trendingScore` 가 현재 API에서 `null` — raw의 트렌딩 순위·점수는 **볼트 재확인 불가**)
- [[DeepSeek-V4.1-Flash]] `deepseek-ai/DeepSeek-V4.1-Flash` — DL **244,457** · ♥**2,303** · MIT · [[DeepSeek]]. **09-11 기생성**. 🎯 **09-11 가설이 정면 확증됐다** — 당시 DL **6**:♥1,583(**0.004:1**)에서 *"이 비율 규칙은 공개 당일·초대형에 무효"* 라 적었고, 3일 뒤 **DL ×40,743 · ♥ ×1.45 → 106:1**. **두 지표의 시간 상수가 다르다.** **CED**: 40층=인코더20+디코더20, 디코더 전역 KV를 **인코더 최종 은닉상태에서 투영** → 552B에서 **prefill 8B·decode 16B만 활성**, KV **V1 대비 437배** 축소, 컨텍스트 100만. 🔴 **양축 병기**: TB2.1 **90.6**(Opus-5.0 89.1)·DeepSWE 74.2·Codeforces 3471 ↔ **TB3.0 30.0 vs 43.3 · TB4.0 31.2 vs 51.8 · HLE(무도구) 36.8 vs 56.3 · ExploitGym 15.3 vs 33.7** = **벤치 세대가 오를수록 격차가 벌어진다**. 🔴 전 수치 조건 **`reasoning_effort=100`** + **벤치마다 하네스가 다르다** *(UPDATED 2026-09-14)*
- [[MiniCPM5-2B]] `openbmb/MiniCPM5-2B` — DL **150,110** · ♥**1,362** · Apache-2.0 · [[OpenBMB]]. **09-10 기생성**. 🔴 **볼트 자기 정정**: 09-10 *"관심 대비 실사용 적다(2.9:1)"* → 4일 뒤 **DL ×52 → 110:1**. **판정이 틀린 게 아니라 측정이 일렀다** → **공개 후 72시간 DL은 지표가 아니다**를 볼트 규칙으로 채택. 🔴 *"2B급 SOTA 평균 53.9·4B급 51.1 상회"* 의 성립 조건을 **같은 표가 반증** — Qwen3.5-4B 대비 **7축 중 6축에서 진다**(MMLU-Pro 70.8<78.0 · GPQA-D 70.2<77.1 · TB2.1 8.6<25.8 · LongBenchPro 44.8<58.4). 🎯 **진짜 우위는 장문맥·도구에 몰려 있다** — NoLiMa **68.1**(2B급 경쟁 **0.7~17.1**) · τ²-Telecom 97.1 · GAIA-103 88.7. **이 축을 헤드라인으로 걸었어야 한다.** ⚠️ †점수는 **Artificial Analysis 공개값**(재현 경로 다른 수치를 같은 표에서 비교) *(UPDATED 2026-09-14)*
- [[Qwen3.8-27B]] `Qwen/Qwen3.8-27B` — DL **7,768,964**(8일 **+2,808,481·+56.6%**) · ♥**15,061**(볼트 추적 HF 모델 **최다**) · Apache-2.0 · [[Alibaba]]. **08-18 기생성**, 🔴 실제 `createdAt` **2026-08-05**(볼트 인지보다 13일 앞섬). 64층 **16×(Gated DeltaNet→FFN×3 → Gated Attention→FFN×1)** 하이브리드 27B dense VLM · 네이티브 262,144→**100만** · `preserve_thinking`. 🔴 **최대 격차가 사내 벤치에서 나온다** — SWEBench **79.0 vs 49.3/59.2/63.8** 인데 **각주가 *"In-house coding benchmark"***. 외부 벤치는 혼전: SWE-bench Pro **61.7**(Opus4.6 Max 53.4 ✅) ↔ **TB2.1 73.0 < 78.2** · NL2Repo 42.3 < 47.6 *(UPDATED 2026-09-14)*

### 2026-09-13 배치 (13건 = **전량 신규** · 중복 0 · GitHub 5 · HF논문 5 · HF모델 3)

> 🎯 **배치 결론**: 오늘 소스들은 **「셀 수 있게 만들어 뒀는가」**로 갈렸다. [[Claude-Red]] 는 배지 78/23이 git tree 실카운트와 정확히 일치했고, [[CloddsBot]] 은 121+/118+/1000+ 가 **세는 대상부터 다르다**. 같은 형식·같은 자릿수인데 갈린다 → **단위 불일치는 규모가 아니라 습관의 문제**.
> 🔴 **그리고 이번엔 볼트 자신이 걸렸다** — 09-02 [[Qwen-Drive-1.0]] *"수치 0개"* 판정은 틀렸다. 가중치 레포가 **6일 앞서** 22개 벤치 표를 달고 존재했다. → [[짝-아티팩트-조회]] 신설.
> 📌 **이번 배치부터 `org/repo` 원본 식별자를 병기한다**(raw의 09-13 요청 반영 — `microsoft/VibeVoice-ASR-Streaming-7B` 전체 ID grep이 0건을 반환한 위음성 문제).

**GitHub 5건**
- [[pentagi]] `vxcontrol/pentagi` — ⭐**23,624**·Go·MIT·[[vxcontrol]]. Docker 샌드박스 + 20여 보안툴 위임, LLM 공급자 10종+ 교체. 🔴 **63행 *"Fully Autonomous"* 를 82~85행이 스스로 부인**(*"and assistant-guided"·"not BAS"·"conceptual or future work"*) — **주장과 부인이 19행 거리**. 1067행 *"explicitly authorized"* 한정 *(NEW 2026-09-13)*
- [[no-ai-slop]] `petergyang/no-ai-slop` — ⭐**8,865**·MIT·**총 14파일**(SKILL.md 1장 실확인). 20+ AI 상투 패턴 규칙 검출. ✅ *"**without guessing whether AI wrote the text**"* — **판별기가 아님을 저자가 먼저 명시** → [[자기제한-명시]]. ⚠️ README는 20+ 중 **10개만 열거** *(NEW 2026-09-13)*
- [[YuE]] `multimodal-art-projection/YuE` — ⭐**7,460**·**Apache-2.0**. 가사→편집가능 악보(ABC)→완성곡 2단계. 🔴 **가중치 [[YuE2-3B]] 는 CC BY-NC** = 코드만 보면 상업 판단이 정반대. 🎯 **best-of-8이 기본판에 지는 축 2개**(MuLan 0.5051<0.5068 · PER 9.79%>8.44%). 🎯 **커버 0.647 vs 0.006(108배)** 가 raw가 통째로 놓친 최대 수치 *(NEW 2026-09-13)*
- [[Claude-Red]] `SnailSploit/Claude-Red` — ⭐**3,788**·MIT·[[SnailSploit]]. 78개 SKILL.md × 23 카테고리 온디맨드 로딩. ✅ **배지 = git tree 실카운트 정확 일치**(이 배치 유일). ⚠️ 푸시 08-30 = **2주 정체**, 공격보안은 정체가 곧 감가 *(NEW 2026-09-13)*
- [[CloddsBot]] `alsk1992/CloddsBot` — ⭐**2,613**(당일 +376·**16.8%** 배치 1위)·TS·MIT. 🔴 **수익화 3층**: README 12행 **자체 토큰 CA**(pump.fun) + 마켓 **5% 수수료** + **폐쇄 등록**(verified 인스턴스만). 🔴 **121+ skills vs 118+ strategies vs 1000+ markets**, 실제 명시 전략은 **4개**. **reliability medium** *(NEW 2026-09-13)*

**HF 논문 5건** — ⚠️ 전건 **발행일 정정**: raw *"09-11 발행"* → API `publishedAt` **09-09~09-10**(09-11은 데일리 게재일)
- [[MaP-WAM]] `arXiv:2609.11561` — 업보트 **35**·저자 8인. 기억을 *"되돌아볼 기록"* 이 아니라 **"다음 계획"** 으로 변환, 실행기 컨텍스트 **고정**. 🎯 핵심 효과는 성공률이 아니라 **지연 상수성**. RMBench **83.3%** / 실로봇 **78.0%**(5.3%p 격차 병기) *(NEW 2026-09-13)*
- [[FreeFlow]] `arXiv:2609.11486` — 업보트 **32**·저자 4인 · **slam-3dgs 재판정**. 상관볼륨·워핑·반복정제를 **전부 빼고** 어텐션 3종만. 🎯 대가로 얻은 것이 **스케일링**. Sintel **0.68/1.48** · KITTI **3.23** · Spring **3.192**. 🔴 **비교 대상 모델명이 초록에 없다** *(NEW 2026-09-13)*
- [[MetroLLM-Bench]] `arXiv:2609.10016` — 업보트 **29**·**저자 1인**([[Remco Hendriks]]) · **local-llm 재판정**. 4B PEFT 학생이 Tier1 **91.3 > GPT-5.6 90.6·90.0**, **Q4_K_M 2.6GB**. 🔴 **저자가 반증 5개를 같은 초록에** 적는다(27B에서 **-0.91** · 규칙엔진 **84.6** · 종합 1위는 Muse Glimmer 30B · 서빙설정만으로 2.7점). 🎯 **볼트 추가**: 4B는 벤치 자신의 **717 케이스로 학습**됐고 GPT-5.6은 아니다 → [[분포내-우위]] *(NEW 2026-09-13)*
- [[RCWM]] `arXiv:2609.11499` — 업보트 **28**·저자 3인 · **slam-3dgs 재판정**. *"코드 표현은 있었고 없던 것은 **짓는 절차**"*. 전체→국소→**전체 재방문** 재귀. 🎯 [[국소-수리-원리]] 에 빠진 조각(**전역 정합의 책임 주체 = 부모 호출**)을 채운다. 🔴 **정량 수치 0개** — 수치 인용 금지 *(NEW 2026-09-13)*
- [[World-in-World]] `arXiv:2609.11548` — 업보트 **28**·저자 3인 · **slam-3dgs 재판정**. **동결** 비디오 모델에 어댑터 없이 *"카메라·시간 라벨 붙은 깨끗한 시각 상태"* 로 **입력 형식만 번역**. 🔴 **초록이 결과를 안 적는다**(*"assessing..."* 으로 끝) — 우위 주장 인용 금지 *(NEW 2026-09-13)*

**HF 모델 3건**
- [[Edge0-35B-A3B-preview]] `Edge0/Edge0-35B-A3B-preview` — DL **1,596**·♥626·트렌딩 **3위**·**apache-2.0**(raw 누락)·[[Edge0]]. 4비트 전문가를 스토리지에 두고 **prerouter 선예측 스트리밍** → 활성 **3GiB**. 🔴 *"phone-class memory"* 실측은 **Mac mini M4 Pro 24GB** · *"3 GiB"* 는 **짧은 컨텍스트 전제**(각주+Limitations 2곳) · 🔀 *"15 tok/s"* 는 표(**14.9–17.7**)보다 **과소**. 🎯 **IFBench 57.9(최하)와 "에이전트 약함" 자인이 같은 능력을 가리킨다** *(NEW 2026-09-13)*
- [[Qwen-Drive-1.0-4B]] `Qwen/Qwen-Drive-1.0-4B` — DL **3,761**(배치 1위)·Apache-2.0·**4.54B**(이름 4B 대비 +13.5%)·[[Alibaba]] 확정. 🎯 **09-02 [[Qwen-Drive-1.0]] 의 "검증 불가"를 해소** — 가중치가 **6일 앞서** 존재했다 → [[짝-아티팩트-조회]]. 🔴 **일반 VLM 15개 중 10개에서 베이스보다 낮다**(초록의 *"largely preserving"* 반증). 🔴 **RL val 1.27 → test 2.67(2.1배)** = 보상 최적화 벤치에서 과적합 서명. 폐루프 at-fault **0.27 표 내 최하** *(NEW 2026-09-13)*
- [[AuK]] `tencent/AuK` — DL **1,202**(raw 907 대비 **+32.5%**)·♥157·MIT·[[Tencent]]. 자연어 지시 하나로 **5범주 16태스크**(생성2·내용편집2·음향편집3·준언어5·향상분리4, 합 일치). 🔴 *"1.5B"* vs **실측 6.76GB + Qwen2.5-Omni-3B 별도 의존**(config.yaml 실조회). 🔴 **`## Performance` 가 이미지 1장**([[VibeVoice-ASR-Streaming-7B]] 패턴 재현). ⚠️ 생성일 실제 **08-18**(raw 09-09), **24kHz**(YuE는 48kHz), `AuK-Flash` 별도 레포 *(NEW 2026-09-13)*

### 2026-09-12 배치 (13건 = **전량 신규** · GitHub 5 · HF논문 5 · HF모델 3)

> 🎯 **배치 결론**: 오늘 소스들은 *「무엇을 만들었나」*보다 **「그것이 맞는지 누가 검사하나」**를 말한다. 검사를 설계한 쪽([[SWE-Bench-Pro-Verified]]·[[T1]]·[[hyperresearch]]·[[EvoSafeHarness]])과 생략한 쪽([[VibeVoice-ASR-Streaming-7B]]·[[Mi-Ripple]]·[[YuE2-3B]])이 한 배치 안에서 갈렸다.

- [[hyperresearch]] — ⭐**2,858**·MIT·Python. Claude Code용 **16단계 딥리서치 → 영속 볼트**. 🔴 볼트가 없는 것을 갖고 있다: **출고 전 검증 배터리 6종**(quote-integrity·retracted-citations·numeric-consistency·cite-check·locus coverage·**patch-only 툴 잠금**). ⚠️ *"리더보드 1위"* 는 **층화 파일럿에서의 외삽**(20행 캡션, raw는 14행만 인용) *(NEW 2026-09-12)*
- [[WeKnora]] — ⭐**22,480**·[[Tencent]]·Go. 문서→RAG·에이전트·**Auto-Wiki** 3출구. **Wiki Mode v0.5.0 GA → v0.5.2 4만 문서 → v0.7.2 리비전+행단위diff+원클릭롤백**. 🪞 **볼트에 없는 「되돌리기」**. ✅ `NOASSERTION`이지만 LICENSE **8행 MIT**(Tencent 3일새 2번째→벤더규칙 승격) *(NEW 2026-09-12)*
- [[OpenResearch]] — ⭐**1,469**·Rust·MIT·[[alphaXiv]]. **모델뿐 아니라 하네스까지 세션별 교체**(Claude Code/Codex/OpenCode/Cursor), 로컬·SSH·Slurm 동일 실행. 🔴 raw의 *"초기 단계"* 판정은 오류 — `beta` 의 주어는 **「Windows 지원」**이었다 *(NEW 2026-09-12)*
- [[MathModelAgent]] — ⭐**4,976**·Python. 수학 모델링 3역할 에이전트, 데스크톱앱이 **Claude Code + SKILLS 번들**. 🔴 **LICENSE 파일 자체가 없다**(루트 14파일 실조회) = 저작권 유보. `null` 은 `NOASSERTION`(판정 유보)과 **반대로** 취급해야 한다 + 인스톨러 코드 미서명 *(NEW 2026-09-12)*
- [[DeskcommCRM]] — ⭐**1,509**·TS·MIT. WhatsApp 영업 에이전트 내장 자가호스팅 CRM. 🎯 **fork/star 1:2.94 배치 1위**(다음이 1:6.96) = 실배포 신호, 이슈 95 최다. ⚠️ 포르투갈어가 1차 언어(EN/ES는 번역본) *(NEW 2026-09-12)*
- [[SWE-Bench-Pro-Verified]] — 업**22**(배치 최저인데 **실무 영향 최대**). 표준 벤치가 **정답 유출·리워드 해킹·잘못된 테스트 범위**로 오염됐고 교정 후 *"some models perform **substantially worse**"*. ⚠️ 볼트 기존 수치가 자동 무효는 아니다 — [[MiniCPM5-2B]] 는 **SWE-bench *Verified*** 로 **다른 벤치** *(NEW 2026-09-12)*
- [[T1]] — 업**47**·122B MoE. 실제 셸 **300+ 툴콜 턴**, 보상 = **작업 자신의 검증기 실행**. Terminal-Bench 2.1 **43.8→64.0**, LHTB **27.9**(GPT-5.4·[[Zhipu AI]] GLM-5.1 상회). TITO+R3로 로그확률 괴리 0.021→0.013. 학습코퍼스 벤치 분리 **명시**(단 자기신고) *(NEW 2026-09-12)*
- [[EvoSafeHarness]] — 업**47**. 보안 하네스를 전문가가 아닌 **탐색으로 합성**(자연어 정책+실행 코드 동시). DecodingTrust ASR **45.6→10.0**(유틸 3.3p), AgentDojo **ASR 0.0에서 유틸 82.8**(CaMeL 2배). 볼트 추가발견: **미지 AgentDyn 무수정 전이** = 과적합 반론에 대한 직접 답 *(NEW 2026-09-12)*
- [[Mi-Ripple]] — 업**33**·저자 3인(배치 최소). 반복 AI 편집이 남긴 **digital ripple** 을 격자/입자로 **분리해 분기 처리**(노칭 vs 재생성). ⚠️ 근거가 14회 실행 + 페어 1건 = **비교군·유의성 없음**. 실질 기여는 방법보다 **진단명과 2분류** *(NEW 2026-09-12)*
- [[X-AuT]] — 업**26** → 🔀 **local-llm 이관**. 18→16층 **5.61%→5.27% 개선**, 14층은 **5.75% 악화** + 파라미터 20.7% 절감. 🎯 **압축은 단조 이득이 아니라 운용점 2개**. 볼트 추가: 교사1.7B 5.55 vs 자기증류 8.45 · 점진 5.75 vs **직접 6.73** *(NEW 2026-09-12)*
- [[MiniCPM5-2B-GGUF]] — DL **70,755**(모델 3건 중 1위, 2위의 31배)·Apache-2.0 → 🔀 **local-llm 이관**. 🔴 **카드가 원본 BF16 카드와 사실상 동일**(md5 `fee94d2…` vs `efedd19…`, diff = `👈 you are here` 마커 이동뿐) → **4비트 양자화 성능은 측정치 0** *(NEW 2026-09-12)*
- [[VibeVoice-ASR-Streaming-7B]] — ♥**213**·MIT·[[Microsoft]]. 화자귀속 **스트리밍** ASR, 언어 10종 **한국어 포함**. 🔴 카드 **2,236B에 수치 0건**(Evaluation=이미지 1장) 🔴 이름 **7B인데 safetensors 8.67B(+24%)** → VRAM 17.3GB *(NEW 2026-09-12)*
- [[YuE2-3B]] — ♥**271**(모델 최다)·[[m-a-p]]·**CC BY-NC(상업 금지)**. 4090서 3.58분 곡 **71초**·11.18GiB. 🔴 카드가 Suno만 지목했으나 **실제 2위는 Mureka 9(6.9377)로 격차 0.0255** 🔴 **Q3O·PER은 진다** *(NEW 2026-09-12)*

### 2026-09-11 배치 (13건 처리 = **신규 11 · 갱신 2**)

**GitHub (3신규 + 2갱신)**
- [[colibri]] — ⭐**27,617**·Apache-2.0·**순수 C**. VRAM·RAM·NVMe를 단일 계층으로 **전문가를 디스크 스트리밍**해 744B~2.8T MoE 구동(GLM-5.2 744B: 상주 **9.9GB**/디스크 ~370GB). 🎯 *"**속도에 SLA 없음, 의미론에 하드 개런티**"* 를 저자가 먼저 선언 → [[자기제한-명시]] **성숙 형태**(개런티를 **비트 동일성 테스트**로 검증 가능하게 냄). ⚠️ 실측 하한까지 공개: 6×RTX 5090 **5.8~6.8 tok/s·TTFT 13s** / 25GB 박스 **0.05~0.1 tok/s**
- [[llm_wiki]] — ⭐**18,381**·TypeScript. 🪞**이 볼트의 직접 대조군.** 4-신호 관련성(**Adamic-Adar ×1.5**·타입친화도) + **Louvain 커뮤니티 탐지 + 응집도 <0.15 경고** + 교차관계 탐지 → **볼트는 소스 1,012개를 쌓는 동안 링크 구조를 정량 측정한 적이 0회**(현 lint는 차수 0만 잡음, `synthesis/` 2건인 이유). 🔴 `NOASSERTION` 이지만 **LICENSE 원문 1~4행이 GPL-3.0**(어제 [[teamai-cli]] 와 동일 원인, [[파생표기-함정]] **이틀 연속 적중**)
- [[Clodds]] — ⭐**1,884**(당일+277)·MIT. 예측시장 10 + 선물 7 + Solana/EVM DEX를 자연어로 다루는 로컬 트레이딩 터미널(**119 스킬·21 메신저**). 🔴 **README 12행에 자체 토큰 컨트랙트(`2puc…pump`)** — **남의 러그풀은 GoPlus로 탐지하며 자기 토큰은 런칭**. reliability **low**(근거는 코드가 아니라 **인센티브 구조**) → 규칙 신설: *인센티브 있는 저장소에서 스타 급상승은 품질 신호가 아니다*
- 🔄 [[vercel-skills]] — ⭐15,695(04-23) → ⭐**31,337**(**2.0배**). **레지스트리 → 패키지 매니저**: `skills use`(**무설치**·프롬프트만 stdout → `| claude` 파이프) · `--agent '*'` 와일드카드 · 79개(자기보고) 에이전트. 🔴 **raw "중복 0건" 보고를 볼트가 반증** — 4.5개월 전 페이지 존재 → [[요약자와-판정자-분리]] **탐지 단계 오류 신규 유형**
- 🔄 [[i-have-adhd]] — ⭐7,400(07-22) → ⭐**39,550**(당일 **+3,882, 트렌딩 1위**). **코드는 그대로**(README 3,461B) → **도구 개선이 아니라 "AI가 말이 너무 많다"는 불만의 누적을 측정**. 🪞 볼트의 07-22 액션 **7주 미이행**(그사이 소스 페이지는 평균 6.3KB로 길어짐)

**HF 논문 (5신규)**
- [[WMRL]] — 업보트 **432**(데일리 1위)·저자 11. AutoResearch RL의 병목을 **계산량이 아니라 "공유 불가능성"으로 재정의**(*생성은 배치로 공유, 실행은 샌드박스 독점*). 환경 실행을 월드모델로 대체 + **Online Debiasing / Inverse-Variance Denoising**(수렴 보장 개선 **증명**). **학습 3~4배 가속**, 🔥**4B·9B가 48B·120B를 홀드아웃 벤치에서 상회**, VLA 전이
- [[AgentGrad]] — 업보트 **89**·저자 7. 텍스트 그래디언트의 두 결함 지목: **고친다고 실패가 해결되는지 검증 없이 대상 선택** + **그래디언트 무작위 집계로 무관 실패모드 혼합**. 해법은 **한 번에 한 에이전트만 바꾸는 순차적 개입**(=ablation을 최적화 루프에 넣음) + 의미 군집화. **5개 MAS SOTA·벽시계 2.5배 단축**. 🪞**볼트가 4배치째 저질러온 오류가 정확히 ①번**
- [[NCP-ArchPreview]] — 업보트 **102**·저자 28. NTP에 **다음 개념 예측(NCP)** 병행 — 은닉상태에서 PQ 개념 어휘 구성, Concept Module이 미래 개념 예측 후 **토큰 레벨로 되먹임**. **8.9B/Dolma-3 5.73T = 잠재공간 LM 최대 규모**. **토큰 51.3%로 OLMo-3-7B 최종 손실 도달** · 매크로 **+2.45**(GSM8K **+5.99**) · ⚠️**공정 비교는 "연산 85%"**(51.3%는 7B 대비) → [[단위-불일치]]. **17M VQ 모듈만 갱신해 도메인 적응**
- [[SenseNova-U1.5]] — 업보트 **78**(U1은 1,580)·저자 **65**·[[SenseTime]]. **encoder-free·VAE-free 8B-MoT**, 공간일관 패치 재구성, **4K 네이티브**. 4전문가(미학·이중언어 텍스트·인포그래픽·편집) 개별 최적화 후 **멀티전문가 온폴리시 증류로 통합**. 🪞**볼트 예측 빗나감**(*"SenseTime 특성상 오픈소스 가능성 낮음"* → **학습 코드 공개 예고**). ⚠️ 정량 벤치 거의 없음 · 업보트 20배 하락은 **신규성 지표지 성능 지표 아님**
- [[SpatialBlock]] — 업보트 **35**·저자 3. 실장면 공간 QA 라벨이 *"**외부 인지 모듈 의존으로 노이즈**"* 라는 급소를 **합성 블록쌓기**로 우회 — **정답이 생성 규칙에서 직접 나온다**([[검사가능성-공사]] 외부 정답 고정의 새 형태). **SpatialBlock-15k**(3D→2D 투영·시점 변환·구조 결합 + 앵커 유도 색 변조) → **합성·소규모인데 실세계 일반화**. ⚠️ 정량 수치 부재. 도메인 `ai-news` 유지 + [[slam-3dgs]] 교차참조

**HF 모델 (3신규)**
- [[DeepSeek-V4.1-Flash]] — 다운로드 **6**/♥**1,583**(공개 당일)·MIT·**552B 백본 MoE·1M 컨텍스트·멀티모달**. CED로 **프리필 8B/디코딩 16B만 활성**, CSA2+**FP4 KV**로 **토큰당 890바이트**(V4-Flash 대비 ~1/4·V1 대비 **437배**). ⚠️**전영역 우위 아님**: TB2.1 **90.6(1위)**·CyberGym 88.1·AutomationBench 54.8 ↔ **HLE 36.8**(Opus 56.3)·**TB4.0 31.2**(Opus 51.8). 🔴 카드 "552B" vs safetensors **763,205,315,794**(차이=Engram 196B) → [[단위-불일치]]
- [[Nex-N2.5-mini]] — 다운로드 **2,444**/♥667·Apache-2.0·**35B**(`qwen3_5_moe`). *"비전은 입력 양식이 아니라 **환경을 인지·검증하는 인터페이스**"*. **BrowseComp 83.4**(397B Pro 89.7에 6.3점 차) ↔ SWE-Bench Pro 43.8(Pro 61.2) → **브라우징은 절차적이라 체급이 덜 먹히고 코딩은 그대로 난다**. 🔴 `pipeline_tag`=`text-generation` vs 태그=`image-text-to-text` **자기모순** → **어제 처방("pipeline_tag를 보라")이 오늘 함정이 됨**
- [[Nex-N2.5-Pro]] — 다운로드 **3**/♥589·Apache-2.0·**397B**. TB2.1 82.7·SWE-Bench Pro 61.2·**BrowseComp 89.7**·OSWorld-V 82.2. 🔥**이 배치 최대 발견** — 같은 표에서 **Max(1.6T, 텍스트 기반)가 OSWorld-2에서 22.3 < Pro(397B, 멀티모달 기반) 56.4 = 34.1점 차**. **기반이 크기를 이긴다** → [[에이전트축-분기]]. 🔴 **mini와 카드 md5 동일**(`f706fcb33c93`) = 패밀리 공용 카드 → 열 매핑 없이 인용하면 틀린다


### 2026-09-10 배치 (13건)

- [[teamai-cli]] — ⭐**3,387**(당일+556)·[[Tencent]]·TypeScript. skills/rules/MCP/docs를 **공유 git 레포 → MR 리뷰 → SessionStart 훅 pull** 로 배포. **에이전트 설정에 코드리뷰를 붙인다**. 지원 11종이나 **전체 커버리지는 5종뿐**, 3층 중 **2층 beta**. 🔴 **볼트가 raw 판정 반증** — raw *"Tencent 자체 라이선스"* → **LICENSE 원문은 MIT 전문 수록**, `NOASSERTION`은 인사말 2줄이 자동탐지를 깬 것 → [[파생표기-함정]]
- [[Pascal-Editor]] — ⭐**23,176**(이 배치 최다)·MIT·R3F+WebGPU·[[pascalorg]]. `npx` 한 줄이 에디터+**인증 MCP 서비스** 동시 기동, `~/.pascal/data/pascal.db` 로컬 보관. 🎯 `furniture-fit` 이 **하지 않는 것을 먼저 못박고**, **MCP 툴 스키마를 런타임 선조회**해 없으면 좁은 결과 보고 → [[자기제한-명시]]. ⚠️ 서비스당 **활성 클라이언트 1개**(병렬 불가) · npm beta가 레포보다 뒤처짐
- [[text-to-cad]] — ⭐**15,249**·MIT·Python. **스킬 11종**(CAD·CAD Viewer·step.parts·DXF·URDF·SRDF·SDF·SendCutSend·DfAM Check·G-code·Bambu Labs). STEP 주출력, **실제 슬라이서 CLI로 프린터 프로파일 검증**, Bambu 실기 출력까지. → **에이전트 스킬이 물리적 결과물로 넘어간 경계 사례**. ⚠️ **조용한 실패 3종**(raw는 2종 — 볼트가 1건 추가): `update`가 신규 스킬 누락 · 폐기 스킬 미제거 · **구버전 Codex(<0.142.0)가 플러그인을 조용히 건너뜀**
- [[openai-plugins]] — ⭐**6,322**(당일+498)·**라이선스 없음**·[[OpenAI]]. 프레임워크 아닌 **예제 저장소**. 값은 **매니페스트 규약**에 있다 — `.codex-plugin/plugin.json` 필수 + skills/·.mcp.json·agents/·hooks.json 선택 = **한 플러그인이 스킬+MCP+서브에이전트+훅을 묶어 배포**. 🎯 **API키 로그인 사용자는 `api_marketplace.json` 로 분리** → 인증 경로별로 보이는 플러그인이 다르다. **remotion 예제 포함**. ⚠️ README **1,283바이트**·수치 전무·**재사용 권리 불명**
- [[PI-Desktop]] — ⭐**1,971**(당일+417)·LGPL-3.0. Electron 프런트 + **Rust 호스트 코어**(권한·FS·SQLite·시크릿). 계정·릴레이·에디터 종속 없음, **로컬 모델 세션별 교체**. 🎯 **격리 주장에 상한 명시** — *"완전한 OS 샌드박스가 아니라 사용자가 신뢰하는 코드"* → [[자기제한-명시]]. ⚠️ Early Preview 자인 · 🔴 **생성 2023-03-22인데 당일+417 → 레포 나이가 성숙도를 과대표시**(피벗 가능성)
- [[Show-Harness]] — 업**48**(HF 데일리 **1위**)·저자10·arXiv 2609.10522. VLM이 추론 가능한 **이산 의미 액션 단위**를 노출하고 임베디먼트별 인터프리터가 **결정적으로** 접지. **미세 물리 판단 책임을 VLM에 남긴다**. 같은 인터페이스로 ①프런티어 VLM **제로샷** ②소형 오픈 VLM **수 GPU-시간 파인튜닝** 둘 다. GUMI로 텔레오퍼레이션 하드웨어 없이 시연 수집. 🔴 **초록에 정량 수치 0건**(볼트 원문 확인) — *"상회"* 서술뿐, **인용 가능한 숫자 없음**
- [[Programmable-World-Model]] — 업**36**·저자11·arXiv 2609.10540 · **도메인 재분류: ai-news→video-saas**. 월드 상태 진화를 **시각 생성과 분리**, 자연어를 **실행 가능 프로그램**으로 번역, **화면 밖 엔티티·비시각 속성까지** 영속 유지, **상태증강 3D OBB**를 중간표현으로 결정적 컴파일, 비디오 모델은 **렌더러로 강등**. 🎯 **내 reat-* 구조와 동일** — 차이는 **영속 전역 상태만 없다**. 📊 CombatStateBench 94%/98%. ⚠️ **저자 자체 신설 벤치·비교 모델명/수치 없음** → 자기 시험지의 자기 점수
- [[WearableQA]] — 업**24**·저자8·arXiv 2609.05405. 실제 **200명**·1인당 **최대 500일** 웨어러블+혈액바이오마커로 만든 **4,084문항 10지선다**. 🎯 **노이즈를 제거하지 않은 것이 설계** — 실제 분포 보존. 질문 16종을 **두 축 직교 조직**(데이터추론/건강추론 × 단일/교차신호), **이중 접지**(문헌 생리학 + 통계검증 집단패턴). 📊 **14종 LLM 19.6~72.9%**(기준선 10%), **대부분 60% 미만**, 저자 자인 *"far from solved"*. → **이 배치 최고 설계 벤치마크**
- [[SAEScientist-Bench]] — 업**14**·저자4·arXiv 2609.09113·코드공개. 목표 개념 주면 에이전트가 **대조 프로브 설계** + Gemma Scope **13.1만+ 특징** 탐색. 평가 3축(활성화순위·개념선택성·**인과적 스티어링**), Neuronpedia **외부 앵커**. 📊 **10구성×20과제** — 프런티어도 **전문가 기준선에 크게 미달**, 개념 분리는 근접하나 **인과적 스티어링은 크게 뒤처짐**. 🎯 *"에이전트는 대조는 설계하지만 **실험 측정치를 빈번히 오해석한다**"* → [[요약자와-판정자-분리]] 의 외부 증거
- [[Discovery-Certification-Protocol]] — 업**13**(이 배치 최저이나 **방법론은 최상**)·저자4·arXiv 2609.09219. 성과 주장을 **실행 가능한 복구·피드백 테스트로 변환**. Gate1 봉인평가 · Gate2 **목표 연구 이력 차단**한 대조 에이전트(모든 유효 경로가 **복구 증인** 제출 → Core 거부권) · Gate3 진실피드백 효과. 📊 감사 2건 **96에피소드 복구 0건·상한 0.0468** · **진실 30 vs 중립 0** · 60쌍 널 통과 · **LLM 없는 결정적 검증기**가 동결 증거로 판정 재현. 🎯 **"발견의 증거는 결과가 아니라 대안 경로의 부재"** — 볼트 [[측정도구-먼저-반증]] 의 프로토콜화
- [[MiniCPM5-2B]] — [[OpenBMB]]·**밀집 2B**·Apache-2.0·다운로드 2,879/♥1,007. 카드 평균 **53.9** 로 2B급 3종뿐 아니라 **4B급 5종 전부 상회**. SWE-bench V **46.4**(차상위 36.8) · **NoLiMa 68.1**(차상위 43.5, 나머지 0.5~17.1 붕괴) · GAIA **88.7**. **코퍼스 실제 공개**(Ultra-FineWeb·UltraData + 후학습 SFT 400B)·배포백엔드 9종. ⚠️ **지시이행이 더 작은 모델에도 진다**(IFEval 86.7 vs LFM2.5-**2.6B** 93.4)·SWE Pro 14.4 vs 28.2. 🔴 **볼트 발견: "이기는 축=자체측정, 지는 축=제3자(†)측정" 상관** · 비율 2.9:1로 **커뮤니티 검증 없음**
- [[Qwen3.8-Flash-Next-NVFP4]] — [[NVIDIA]]가 [[Alibaba]] Qwen을 **NVFP4 4비트** 양자화·다운로드 26,302/♥185. 📊 **9개 중 5개에서 4비트가 FP8을 이겼다** — SciCode +2.5·AA-LCR +2.2·MMMU Pro +1.2·HLE +0.7·IFBench +0.5, **하락 4개는 전부 1점 미만**(최대 −0.7). → **"4비트는 손해" 통념이 성립하지 않는다**, 트레이드오프가 **하드웨어 세대(Blackwell)** 로 이동. 🎯 **표는 FP8 행을 볼드로 강조하나 숫자는 반대** → [[파생표기-함정]]. ⚠️ 날짜 불일치·**분산/시드 정보 없음**
- [[Qwopus3.8-27B-Flash-GGUF]] — Qwen3.8-27B 파인튜닝·Apache-2.0·다운로드 **113,295**/♥176(**비율 644:1 — 이 배치 최다 실사용**). **능력이 아니라 지연을 산** 파인튜닝. 근거: *에이전트 루프가 호출을 수십~수백 회 증폭(턴당 5초 → 50턴에 250초)*. 📊 디코딩 **+12.8%** · MTP 수용률 **+14.6pp**(과목별 균일) · 출력량 **−9.9%** ↔ **MMLU-Pro −1.45pp**(카드가 트레이드오프 **선공개**). 🔴 **볼트 발견: 멀티모달**(`vision`·`multimodal` 태그) — raw는 텍스트 전용으로 기술, **단 비전 성능 측정이 카드에 없다**. ⚠️ Python 들여쓰기 결함 자인

<!-- ── 2026-09-07 자동수집 배치 (신규 **13** · **중복 0** / GitHub 5 + HF논문 5 + HF모델 3, **전건 GitHub/HF API 실호출 검증**). 09-06 도입한 URL 대조 필터가 처음 돌아 **중복률 42% → 0%**. 도메인 재분류 **5건**([[Motion-Omni]]→video-saas · [[WorldSculpt]]→slam-3dgs · [[Dont-Drop-Dropout]]·[[Huihui-Qwen3.8-27B-abliterated-GGUF]]·[[Tiel-Coder-35B-A3B-GGUF]]→local-llm). 배치 최대 사건 **다섯**: ①🔴**스킬 생태계에서 첫 폐기** — ⭐25,754짜리 [[openai-skills]] README 최상단이 **deprecated** 선언, `openai/plugins`로 이동하며 *"skill-only **plugin**을 만들라"*. **스킬은 사라진 게 아니라 플러그인 안으로 들어갔다.** 같은 날 [[humanlayer-skills]]는 그 이동을 **이미 마쳤다**(루트에 `skills/` 없고 `plugins/`·`.claude-plugin/`) → [[claude-plugins-official]]까지 **3개 벤더가 같은 배포 단위로 수렴**, [[에이전트-스킬]] 3층 재편. 부수 규칙: API `archived`가 **false**인 채 폐기됐고 **스타는 계속 붙는다**(+3) → *"스타는 영향력이 아니다"* 를 **"스타는 과거의 크기를 잰다"** 로 강화. ②🔴**볼트 자신의 실패 — raw 요약을 소스로 취급했다**: raw가 [[humanlayer-skills]]를 *"스킬 3종"* 이라 적었으나 **README 실측 5종**이고 **누락된 2종 중 하나(`design-control-loop`)가 그 레포의 핵심**이었다 → 09-06의 *"전건 대조"* 가 **지표에만 적용되고 내용에는 적용되지 않고 있었다.** ③🔴**중복 필터가 첫 회차에 빈틈**: [[timesfm-3.0-pytorch]]가 URL 필터를 **통과**했는데 [[TimesFM]]이 **09-03에 이미 그 모델을 실측 기록** — 원인은 [[TimesFM]]의 `- 출처:`가 **arXiv+GitHub URL**(같은 대상, 다른 표면). ④✅**09-03 미해결 질문 해소 + 라이선스 확정**: TimesFM 3.0 DL **46,862→144,455(+208%)** vs 2.5 **55,036→60,209(+9.4%)** → **비율 0.85배→2.40배, 세대 교체 4일에 완료**. 동시에 `license_name` **`timesfm-non-commercial-license-v1.0`** 실측 — **코드 Apache-2.0인데 가중치는 상업 이용 불가**(2층 라이선스 함정 4번째, **이번엔 가중치 쪽**). ⑤🔬**근거의 세 등급이 한 배치에**: [[Tiel-Coder-35B-A3B-GGUF]]는 자체 측정이나 **통제군 + 자기 열세 3분해** 공개(재배포자 층 첫 사례) / [[open-science]]는 **자체 배지**(*"BiomniBench-DA 1위"*) / [[Huihui-Qwen3.8-27B-abliterated-GGUF]]는 **벤치마크 전무한데 DL 219만**(배치 최대) → **다운로드는 수요를 재지 성능을 재지 않는다.** 부수: **논문 5건 중 4건이 평가 장치 동반 공개**([[검사가능성-공사]] 최대 밀도) · **잠정 축 신설** *변환 비용과 공유 표현*([[Motion-Omni]]·[[Enoki]]가 무관한 도메인에서 같은 처방, 2소스라 승격 보류) · **추적 공백 5건 등록**(openai/plugins · Pixal3D · Marble/HY-World 2.0 · Ornith-1.5/Sharp 템플릿 · Claude Science). 신규 entity 0 ── -->
- [[marketingskills]] — **1층(자산)이 비-코딩 직무의 주류로 넘어간 최대 사례.** CRO·카피라이팅·SEO·애널리틱스·그로스를 스킬로 담아 ⭐**47,746**(배치 최대·MIT·포크 7,405). 08-24에 [[pm-skills]]·[[scientific-agent-skills]]로 관측한 코딩 밖 확산을 *"소수 실험"*→**"주류 채택"** 으로 올린다. 구조적 신규성 2: **스킬 간 의존 그래프**(`product-marketing`이 루트 → 나머지가 먼저 읽음, [[Repo0]] Dual-DAG를 **파일 배치만으로** 흉내 → 1·2층 경계 흐려짐) · **자체 검증 스크립트 보유**(`validate-skills-official.sh` — 볼트 09-04 *"스펙 대조"* 액션을 **직접 구현 없이 해소** 가능). ⚠️**`partners.json`·`tools/` 실재**(유료 Verified Partners) → **추천의 중립성을 판단할 방법이 사용자에게 없다** — 기존 약점(*"품질 보증 부재"*)이 **이해충돌** 형태로 발현 (GitHub API 실측 · raw 47,743 대비 **+3**) *(NEW 2026-09-07)*
- [[openai-skills]] — 🔴**⭐25,754짜리 벤더 스킬 카탈로그가 폐기됐다 — 3층에서 처음 관측된 retirement.** README 최상단 원문: *"This repository is **deprecated**. … use the **OpenAI Plugins repository**. … create a **skill-only plugin**."* 배포 단위가 **스킬 레포 → 플러그인**으로 한 층 올라가고 스킬은 그 **내용물**이 됐다. ⚠️**폐기 신호가 기계 판독 가능한 곳에 없다** — API `archived` **false**·라이선스 `null`·push **2026-07-14(2개월 정체)** 인데 스타는 **+3** 계속 증가 → 볼트 수집기가 **폐기 레포를 신규 소스로 올렸다**(수집기 개선 항목). 폐기됐어도 남긴 관측: README가 [[agentskills]]를 *"Agent Skills **open standard**"* 로 링크 — [[anthropics-skills]]와 **동일한 외부 표준 참조**가 **폐기 후에도 유지**된다. ⚠️후속 `openai/plugins` **볼트에 없음(추적 공백)** (GitHub API + README 원문 대조) *(NEW 2026-09-07)*
- [[humanlayer-skills]] — 🆕**4층(진화)에 「사용자의 시스템을 설계하는 스킬」이 등장.** `design-control-loop`가 **센서·컨트롤러·액추에이터·외란**으로 인터뷰해 에이전틱 제어 루프를 설계하고 **스케줄된 CI 워크플로**까지 빌드한다 — 4층 기존 사례([[SkillEvo]]·[[JIT-Agent]])는 스킬이 **자기를** 고치거나 생성했는데 여기선 **사용자의 자동화 시스템 전체**. LLM 에이전트를 *"프롬프트 체인"* 이 아니라 **피드백 제어 시스템**으로 모델링하는 어휘를 스킬 층에서 **처음** 봄 → *"스킬 = 절차 지식"* 정의를 **"스킬 = 시스템 부트스트래퍼"** 로 확장할 후보. 🔴**raw 오류 정정: 스킬 3종 → 실제 5종**(`improve-claude-md`·`narrow-react-prop-types`·`build-iterated-agentic-loop`·**`design-control-loop`**·**`show-me`**) — **누락된 2종 중 하나가 이 레포의 핵심**이었다. ⚠️효과 수치 없음 · 제어공학 어휘가 **실제 폐루프인지 비유인지 미확인** (⭐3,326·MIT·GitHub API + README + contents 실측) *(NEW 2026-09-07)*
- [[openwhispr]] — **모델도 앱도 아닌 "런타임 배포 정책"이 최종 사용자 기능을 결정한 사례.** 핫키→커서 위치에 받아쓰기, 로컬 STT(whisper.cpp·NVIDIA Parakeet)로 **오디오가 기기를 안 떠나거나** 클라우드(BYOK) 선택. 🔴**Intel Mac에서 화자 식별·음성 지문이 사라진다** — **ONNX Runtime 1.24가 macOS x86_64 바이너리 배포를 중단**했기 때문이고, 노트 검색은 시맨틱→**키워드로 폴백**. → [[MiniMax-H3]] *"오픈 가중치 ≠ 오픈 시스템"* 의 **런타임판**이며, README **배지는 크로스플랫폼인데 각주가 예외를 밝힌다**(볼트 규칙: **배지가 아니라 각주가 사실**). 폴백 설계와 사전 고지는 [[anthropics-skills]]·[[VoiceStudio]] 계열 **주장 약화형 정직성**(가점). ⚠️*"완전 프라이빗"* 은 **로컬 모델 선택 시에만** 참 · 오픈이슈 335건 (⭐7,664·MIT·Electron 41) *(NEW 2026-09-07)*
- [[open-science]] — **로컬 우선·모델 비종속 AI 연구 워크벤치**(Apache-2.0·Electron·**mcp** 토픽·**Zenodo DOI 10.5281/zenodo.22252246 실재**·push 당일). 🔴**그런데 핵심 성능 주장은 자체 배지다** — *"BiomniBench-DA Public 50 **1위**"* 는 저자가 자기 레포에 단 것이고 방법·표본이 없다 → 볼트 규칙상 **인용 금지**, 신뢰도 **medium**. 📌 **한 레포 안에 검증된 주장(DOI 아카이빙)과 미검증 주장(성능)이 섞여 있다**는 것 자체가 판정 포인트 — **배지와 DOI를 같은 층으로 취급하면 안 된다.** 토픽에 **`claude-science-alternative`** 를 박아 자칭 대안인데 **볼트에 원본 페이지가 없어 비교 기준이 없다**(추적 공백). 참고 가치는 **프로버넌스 구현 방식** — 볼트 `log.md`가 사람이 읽는 산문인 데 반해 기계 판독 포맷이 있다면 로그 구조 개선 레퍼런스 (⭐4,019·생성 2026-07-03) *(NEW 2026-09-07)*
- [[Motion-Omni]] — **말하면서 움직이는 아바타를 캐스케이드 없이 한 번에.** 음성 대화 모델과 co-speech 모션 모델이 **다른 계열**에 살아 표준 해법이 캐스케이드(음성 완성→그 오디오로 모션)였는데 **두 번째 완전 추론 패스**와 **공동 최적화 차단**이 대가. **음성을 만드는 hidden state에서** 표정·손·상반신·하반신을 직접 출력해 그 패스를 제거. teacher 캐스케이드 대비 **2% 이내**(reference-free)·**5.4배**·**RTF 0.78(실시간 초과)**·**WER 2.62%**(omni-modal 최저)·의사라벨 **422,856쌍/1,402시간**. 🔴**값싼 경로가 막혔다** — *"**음성 경로를 얼리면 모션 정렬이 깨진다**"* → 기존 음성 모델에 모션 헤드만 붙이는 방식은 실패, LLM·Speech·Motion **공동 학습 필수**. **SwDA-500 + 자칭 최초 공개 평가 프로토콜**(시스템 간 **오디오를 맞춘 뒤** 비교) 배포 — **모델 없이도 프로토콜은 즉시 전용 가능**. ⚠️**teacher를 이긴 게 아니라 2% 이내로 맞춘 것** · 코드 공개 미확인 · 절대 지표명 없음 (업30·2609.04250·초록 대조) *(NEW 2026-09-07 · **video-saas 재분류**)*
- [[WorldSculpt]] — **밀집 장면을 생성하기 위해 밀집 장면을 학습할 필요가 없었다.** 수백 오브젝트가 서로 가리는 장면을 통짜 표현이 아니라 **개별 오브젝트 메시들의 집합**으로 생성 — 단일 오브젝트 prior(**Pixal3D**)에 **multi-view conditioning 경로**만 붙였고 *"**전부 단일 오브젝트 정규 공간에서만 파인튜닝**했는데 **장면 단위 학습 없이** 심한 가림의 대규모 장면으로 일반화"*. 주장의 형태가 좋다 — *"**장면 복잡도와 가림이 커질수록 격차가 벌어진다**"*(평균 우위가 아니라 겨냥한 실패 모드를 실제로 품). 🔗**[[월드모델]] 축과 [[slam-3dgs]] 축을 연결** — 3DGS 월드(**Marble**·**HY-World 2.0**)를 compositional 메시 장면으로 **변환** 시연. **UE-MeshyScene** 공개(수백 오브젝트·**오브젝트별 주석**·**GT 메시**). ⚠️**절대 수치 전무** · 실시간성 판정 불가 · 코드/소속 미확인 · 🔴**"grounded video"는 분할이 이미 주어졌음을 시사** → 난이도가 [[X2SAM]] 계열 전처리에 종속될 수 있음 (업15·저자 12인·2609.05416) *(NEW 2026-09-07 · **slam-3dgs 재분류**)*
- [[Enoki]] — **환각 탐지의 claim↔span 정렬 단계를 "없앤" 게 아니라 존재할 이유를 없앴다.** claim 수준은 해석 가능한 사실 단위를 주고 span 수준은 위치를 잡는데, 둘을 이으려면 LLM 다중 호출 또는 별도 **claim-to-span 정렬**이 필요했다. **Open IE로 텍스트에 앵커된 관계 사실**을 뽑으면 사실이 처음부터 원문 위치에 붙어 있어 **지지되지 않는 사실을 span으로 역투영**하면 끝. 추출기를 **LLM/인코더/규칙** 중 선택해 정확도-비용 조절(**LLM 없이 도는 경로 존재**). 🔬**같은 배치 [[Motion-Omni]]와 같은 처방**(무관한 도메인인데 *"두 단계를 잇는 비용이 크면 **같은 표현을 쓰게 하라**"*) → [[선택비용과-중복성]]에 **잠정 축 「변환 비용과 공유 표현」** 신설. ⚠️**성능은 대등+우위의 조합** — claim 수준은 **대등**(자원 덜 씀), span·엔티티 위치추정에서 **우위**. *"환각 탐지 SOTA"* 로 뭉뚱그리면 틀림. 🔴**초록에 구체 수치 전무** · **추출기가 놓친 사실은 조용히 검증에서 빠진다**(재현율 미공개 — 최대 미확인 위험). **EnokiQA** 공개 (업12·저자 8인·2609.00581) *(NEW 2026-09-07)*
- [[Ask-Before-You-Optimize]] — 🎯**평가의 전제가 능력의 지도를 만든다.** *"기존 평가는 **완전한 명세를 전제**하므로 에이전트가 **모델링 전에 되물어야 할 때를 아는지**를 놓친다."* **OR-Clarify**는 문제 서술 일부만 공개하고 **구조화된 슬롯을 숨긴 뒤** 시뮬레이션 사용자와 제한 상호작용시켜 **슬롯 복구율·중단 행동·암묵적 가정(silent assumptions)·상호작용 비용**을 측정 — **묻지 않는 실패와 너무 묻는 실패를 동시에 처벌**한다. **InterOPT**(미해결 격차 식별 → 질문할지 멈출지 결정 2단): **선택형에서 전 베이스라인 크게 상회**, **개방형에서는 대등**(실무는 개방형에 가까움 → *"질문 잘하는 에이전트"* 일반화는 초록이 지지 안 함). 🔬**이 배치의 반대 극** — [[Motion-Omni]]·[[Enoki]]가 단계를 **없애서** 이길 때 이건 **추가해서** 이긴다. 가르는 변수는 **입력의 완전성** → *"추가 단계의 가치는 입력의 완전성에 반비례한다"*. 🪞**볼트에 직결**: [[humanlayer-skills]] *"3종"* 오류가 정확히 **silent assumption**이었다. ⚠️절대 수치 없음·공개 선언이 초록에 **명시되지 않음**(다른 3건과 대비) (업11·저자 6인·2609.05258) *(NEW 2026-09-07)*
- [[Dont-Drop-Dropout]] — **중복성 원리가 세 번째 층(깊이)에서 재현.** layer dropout(stochastic depth)은 *"정확도를 떨어뜨린다"* 는 통념으로 LLM 사전학습에서 사라졌는데, 지적이 정확하다 — *"**그 효과가 정량화된 적조차, 완화된 적은 더더욱 없다**."* 레이어 분포·시간 스케줄·옵티마이저 HP를 최적화하면 **동일 FLOPs에서 loss 더 낮고**, 동일 스텝 기준 **학습 FLOPs 최대 25% 절감**. 🔬**결정적 차이 — 이득이 미래로 전달된다**: [[Random-Attention]](축출)·[[Minima]](정밀도)는 추론 시점 절감인데, 이건 **학습 시 확률적 결손이 추론 시 구조적 유연성으로 전환**돼 early exit·중간 레이어 스킵·**self-speculative decoding**이 성립(**추론 1.5배**, 정확도 손실 거의 없음) → **중복성은 발견되는 성질만이 아니라 심어 넣을 수 있는 성질이다.** **2,400회+ 실험·271M~8.2B·최대 160B 토큰.** ⚠️**전 실험 Cerebras CS-3 단일 하드웨어** · **HP 탐색 비용이 25% 절감분을 상쇄하는지 미확인** · 볼트 운영자에게 **직접 실행 불가**(모델 생산자용) · 코드 미확인 (업**8**·raw 7 대비 **+1**·저자 9인·2609.05275) *(NEW 2026-09-07 · **local-llm 재분류**)*
- [[Huihui-Qwen3.8-27B-abliterated-GGUF]] — 🔴**이 배치 최대 다운로드(2,194,861)가 "능력"이 아니라 "거부 제거"를 판다.** [[Tiel-Coder-35B-A3B-GGUF]]의 **9.2배**·[[timesfm-3.0-pytorch]]의 **15.2배**인데 **모델 카드에 벤치마크 수치가 하나도 없다** — 저자 스스로 *"**crude, proof-of-concept** implementation"* 이라 부른다. **설계는 보존 지향인데 보존됐는지 아무도 재지 않았다**: ablate 범위가 전체→**18~51**→**23~51**로 좁아지고 *"원 모델 성능을 더 보존한다"* 고 하나 **측정 없음**, **MTP·비전 경로 미변경**(`pipeline_tag: image-text-to-text`와 일치). ⚠️**비표준 양자화** — ablate 대상 텐서(token_embd·output·ffn_down·ssm_out·attn_output)를 Q8_0/BF16로 올려 `_L` 명명 → 저자 경고대로 **`Q2_K_L`이 `Q3_K`보다 클 수 있다**(파일명으로 크기·품질 추론 금지). 📌**볼트 판정**: 생성 2026-08-16(3주)이라 *"공개 직후 DL"* 예외 아님 → **수요는 실제**. 다만 그 수요는 **경쟁이 약한 공백(거부 제거)** 에서 오므로 **낮은 품질로도 높은 DL이 가능한 시장 구조**. **다운로드는 수요를 재지 성능을 재지 않는다** (Apache-2.0·base [[Qwen3.8-27B]]·근거 구현 Sumandora/remove-refusals-with-transformers) *(NEW 2026-09-07 · **local-llm 재분류**)*
- [[Tiel-Coder-35B-A3B-GGUF]] — ⭐**재배포자 층에서 처음으로 통제군을 가진 자산.** 4bit **22GB** MoE(Ornith-1.5-35B-A3B 자체 imatrix 재양자화 + **Sharp 템플릿을 GGUF에 심음**). SWE-bench-Live **12/25 = Opus 4.6(medium) 동률**·Nail +3·Ornith +4·Sonnet 5(medium) +4, **속도가 진짜 무기**(중앙값 **8.6분** vs [[Qwen3.8-27B]] **50.2분/16문제** = **5.8배**). Claw-Eval 멀티턴 **67.2**(Ornith 65.3·Nail 60.5). 🔬**대가를 3분해해서 공개**: MMLU-Pro **73.7** vs Nail 84.0인데 **대부분이 베이스 상속**(Ornith 78.0 ← stock Qwen3.6-35B-A3B 85.3)이고 **양자화는 원인 아님**(*"같은 quant + Ornith 템플릿 = Ornith 점수 **그대로**"* ← **대조군**), 남은 **4.3점은 Sharp 템플릿이 답을 짧게 만든 트레이드**. 🔗**[[Ask-Before-You-Optimize]]와 직결** — Claw-Eval에서 **해명질문 -5.1·답변품질 +3.8**로 이겼고 점수가 **답변에 4:1 가중**이다 → **평가 가중치가 능력의 방향을 만든다**(카드가 *"vague request를 되묻길 원하면 베이스가 낫다"* 고 **스스로 예고**). 📌raw 누락: **`vision` 태그** · **KV 262k에 5GB 미만**. 🔴**n=25(1문제=4%p)·전 수치 저자 자체 측정·이미지 alt 수록** — *"Opus 4.6 급"* 일반화 금지 (DL 238,588·MIT·저자 권장 **UD-Q4 이상 + q8_0 KV**) *(NEW 2026-09-07 · **local-llm 재분류**)*
- [[timesfm-3.0-pytorch]] — ✅**09-03 미해결 질문 해소 — 세대 교체는 4일에 끝났다.** 3.0 DL **46,862→144,455(+208.3%)** · 2.5 **55,036→60,209(+9.4%)** · 1.0 490→636 → **3.0/2.5 비율 0.85배 → 2.40배**. 동시에 *"좋아요는 누적되고 다운로드는 이동한다"* 재확인(1.0은 좋아요 **835로 계열 최다**인데 DL **636**). 📌*"공개 직후 DL은 아직 없는 값"* 규칙이 **같은 모델 시계열 추적으로 4번째 근거**를 얻어 **"최소 2주 + 전 세대 상대비"** 로 강화. 🔴**도입 결정 변수는 라이선스** — `license_name` **`timesfm-non-commercial-license-v1.0`** 실측: **코드(google-research/timesfm ⭐30,397)는 Apache-2.0인데 가중치는 상업 이용 불가** → [[MiniMax-H3]]·[[anthropics-skills]]·[[VoiceStudio]] 2층 함정의 **4번째 사례이고 이번엔 가중치 쪽**. 🔴**중복 필터 빈틈 노출** — URL은 신규인데 [[TimesFM]]이 09-03에 이미 이 모델을 기록(**같은 대상, 다른 표면**). 별도 페이지 유지 근거는 **라이선스가 서로 다르다**는 것. ⚠️**성능 수치는 2회 연속 미확인**(카드에 벤치 없음) · 데이터 컷오프 **2023-11 / 2022년말** → 최근 시계열 드리프트 위험 (DL 144,455·좋아요 534) *(NEW 2026-09-07)*

<!-- ── 2026-09-05·09-06 자동수집 배치 (신규 **15** · 갱신 **11** / GitHub 5[전량 신규] + HF논문 7[전량 신규] + HF모델 3[전량 신규], 그리고 raw 26건 중 **11건이 중복**. **26건 전량 GitHub/HF API 실호출 재검증** — 09-05 raw 수집값과 09-06 실측을 전건 대조. 배치 최대 사건 **넷**: ①🎓**[[검사가능성-공사]] 잠정 → 확정 승격**(09-02가 명시한 승격 조건 *"다음 배치 2건 추가"* 충족) — [[Aspire]]·[[HarnessDev]]가 **히든 평가 분리**로 도달했고, 더 중요한 건 **급소가 논증에서 측정으로 바뀌었다**는 것: 09-02의 *"검사기가 피검사자와 같은 실패 모드를 공유하면 거울이다"* 를 [[Aspire]]가 **부정적 결과로 확인**(*좁은 자체평가를 신뢰해 국소 향상이 히든 평가로 전이되지 않는다*). 외부 정답 고정이 **다섯 형태**로 정리되고 마지막 둘이 신규 — **버전 태그 붙은 공식 문서**([[context7]])와 **히든 평가**(정답의 *출처*가 아니라 **볼 수 있는 시점**을 외부화 = 접근 통제도 외부화의 한 형태). ②🔴**자기개선 주장이 반증됨 — 볼트가 그 주장을 ⭐24만짜리로 보유 중**: [[Aspire]]가 *"루프가 돈다"* 와 *"루프가 개선한다"* 를 갈라 후자를 부정(**가중치 향상 희소·불안정**, **탐색 계속하면 앞선 개선이 지워짐**, 최강 진화 하네스도 Qwen-Agent 레퍼런스 미달) → [[hermes-agent]] 핵심 판매점에 직접 걸림. 짝논문 [[HarnessDev]]는 **하네스 개선의 모델 간 전이가 제한적**임을 보여 *"모델 바깥에 두면 재사용 자산"* 이라는 [[에이전트-스킬]] 전체의 암묵 전제를 반증 — 같은 배치 [[LatentPress]]의 **reader-matched 결박**과 **두 층에서 동시 관측**. ③🔬**한 모델(Qwen3.8-27B)에 원본·구현·원리 3층 동시 확보**(볼트에서 드묾): 원본 [[Qwen3.8-27B]] / 구현 [[Qwen3.8-27B-GSQ-RCO-GGUF]] **IQ3_S 11.8GB에 BF16 동률** / 원리 [[Minima]] **496개 선형층 전부 NVFP4에도 -0.52 · 게이트가 가장 덜 민감**(통념 반전). 그리고 GSQ-RCO는 **모델 카드가 [[Repo-To-Skill]] 기준(변수 하나만 토글)을 충족한 첫 사례** → [[Qwen3.8-27B-GGUF]]의 정확도 우위 주장 **반박됨**. ④🔴**볼트 주장 1건 정정 — [[MiniMax-H3]] 격차 "약 100배" → 실측 3.92배**(원본 59.4k→**498만**·84배 상승 / 재패키지 2.9배): *"오픈 i2v 실사용은 사실상 전부 ComfyUI 경로"* **완화**. [[BCIT]]가 지적한 **비율 주장이 관측 시점에 결박되지 않은 채 사실처럼 기능**한 첫 실제 피해. 부수: ⚠️**수집기 중복 검출 부재 — 중복률 42%**(09-04 인제스트분을 이틀 만에 재수집) · **담론은 추론 층에 몰림**(GitHub 9건 중 8건이 추론 도구, 유일한 학습 층 [[miles]] ⭐2,636으로 **최소 12배 열세**인데 [[On-Policy-Distillation-II]]는 *"데이터 과잉·알고리즘 부족"* 이라 함) · **프로세스 소유권 논쟁 양쪽이 같은 배치에**([[mattpocock-skills]] ↔ 그가 비판한 [[spec-kit]], issue/star가 사실상 동일 0.0018 vs 0.0024 → **판정 보류**) · raw 증분 불채택 **11회 연속**. 신규 entity 0) ── -->
- [[HarnessDev]] — 🔴**평가 단위를 태스크 출력 → 실행 가능 인프라로 이동.** *가중치 고정한 채 하네스만 바꿔도 성능이 상당히 달라진다*는 전제에서 Creation(최소 시드→완전 실행 시스템)·Evolution(실행 피드백으로 자기 하네스 개정) 2단계, **능력(홀드아웃 성공률)+효율(실행 토큰 비용) 이중 측정**. 창작자 LLM 6종·4도메인·**2,207 인스턴스**·**히든 평가 분리**. 결과가 영역별로 갈림 — **코드·검색/리서치는 사람 레퍼런스에 크게 못 미치고 글쓰기·ML 실험은 대등~상회** = *LLM이 하네스를 잘 만드는 게 아니라 사람이 안 만든 곳에서만 이긴다.* 🔴**Evolution은 불안정·부분 전이**하며 **이득이 하네스 실행 모델에 강하게 의존(모델 간 전이 제한)** → [[에이전트-스킬]]에 "모델 바꿔 재검증" 기준 신설 (업**240**·저자 19인·2609.01437·초록 대조 **high**) *(NEW 2026-09-06)*
- [[Aspire]] — 🔴**"자기진화"가 자기진화가 아니었다는 지적.** 기존 연구는 사람이 태스크·지표를 줘서 **명시적 목적함수 최적화로 축소**된다 → 자연어 능력 목표만 주고 **평가 태스크는 숨긴다**(전문가 작성 히든 **520문항**·6개 목표). 결과가 냉정: 학습·하네스 편집 루프는 **곧잘 완주**하나 **가중치 향상은 희소·불안정**, **좁은 자체평가를 신뢰해 국소 향상이 히든 평가로 전이 안 됨**, **탐색을 계속하면 앞선 개선이 지워짐**, 최강 진화 하네스도 **Qwen-Agent 레퍼런스 미달**. ⭐[[검사가능성-공사]] 급소의 **첫 실험적 확인** → 개념 **확정 승격**. [[hermes-agent]] 자기개선 주장의 직접 반증 (업210·저자 21인·[[HarnessDev]]와 **제1저자 3인 동일**·2608.31111·초록 대조 **high**) *(NEW 2026-09-06)*
- [[Minima]] — 🔬**통념이 정반대로 틀렸음을 보인 4비트 양자화 원인 분석.** 커뮤니티는 [[Qwen3.8-27B]]의 GDN 게이트를 8/16비트로 남겼는데(*"재귀에서 오차가 누적된다"*), **496개 선형층 전부 NVFP4 W4A4**로 내려도 BF16과 시드 노이즈 내 동등(5과제 평균 **-0.52**, perplexity 4K/32K·MMLU-Pro·GSM8K·AIME'25·GPQA-D·LCB·RULER 64K). **최소 17.5 GiB·프리필 +14~19%.** 원인: **게이트 투영이 가장 덜 민감**(GEMM 오차 ~11% → 출력 오차 **~2%**), 32K에서 주입 노이즈 **평탄 유지**. [[Random-Attention]]과 **다른 층에서 같은 원리**(중복성이 정밀함의 비용을 무효화) → [[선택비용과-중복성]] 근거 보강. ⚠️저자 **2인** · **NVIDIA 하드웨어 종속** · 하이브리드에서만 검증(어텐션 16층이 흡수했을 가능성 미배제) (업73·2609.04098·초록 대조 **high**) *(NEW 2026-09-06)*
- [[LatentPress]] — 🆕**[[에이전트-메모리-레이어]] 여섯 번째 축 「연속 잠재 메모리」.** 지적의 핵심이 기술이 아니라 **설계 관성** — *"압축 컨텍스트는 보통 **사람이 읽을 수 있는 텍스트**나 렌더링 이미지로 운반된다 — **소비자가 언어모델일 때조차도**"*. **동결 디코더가 입력 임베딩으로 직접 읽는 연속 메모리 토큰**(추론 시 텍스트 복원 없음), 어댑터만 학습 **4.2M~26.2M(디코더의 ~0.1%)**. LongMemEval **0.504 @ 7.70배 압축**으로 **무압축 원문 0.490을 상회** — 텍스트 요약 0.184·OCR 0.426~0.312 대비 큰 격차. 쓰기 43ms/대화·읽기 5~9배·압축 4~16배. ⚠️**reader-matched 결박**(디코더 교체 시 자산 재생성) — [[HarnessDev]] 모델 간 전이 한계와 **동형**. ⚠️저자 2인 (업108·2609.01507·초록 대조 **high**) *(NEW 2026-09-06)*
- [[Terminal-Universe]] — **희소한 것은 데이터가 아니라 환경이다.** *"궤적은 **얼어붙은 시연 1회**지만 환경은 **여러 검증 가능 과제로 재질의**되고 실행 피드백을 준다."* 환경을 새로 만들지 않고, **툴 실행 이력이 환경 구조를 이미 노출한다**는 관찰에서 **실행을 되감아** 파일을 수정 전 상태로 복원 — 버려진 로그가 자산으로 재분류. 🪞**볼트에 직결**: 소스 950건 · `queries/` **0건** = 시연은 쌓고 재질의 형태는 안 만들었다 ([[CoGR]]·[[prompts-chat]]에 이은 **3회 연속 지적**). ⚠️**복원 충실도·성공률·사후학습 이득 수치 전부 초록 범위 밖**이고 부분 복원 환경 과적합 위험 미처리 (업268·저자 14인·2609.04148·초록 대조 **high**) *(NEW 2026-09-06)*
- [[On-Policy-Distillation-II]] — **"데이터 과잉·알고리즘 부족"(data-overfed, algorithm-starved).** 볼트가 이 계보를 **2편째** 추적([[On-Policy-Distillation]] 2606.07082 → 본편). 질의 **1개**로 학습해도 수백 스텝 계속 개선되며 전체 데이터 OPD 이득의 **대부분을 회수**. 기여는 설명 메커니즘 — **상태 커버리지**(전체 OPD 방문 상태 중 도달 비율): **1개 71.5%**(대부분 첫 100스텝) → **16개 98.9%**(전체 데이터 동급). 실제 변수는 데이터셋 크기가 아니라 **상태 공간 도달률**. 같은 배치 [[miles]](대규모 인프라 전제)와 **정면 대비**. ⚠️*"1개면 된다"* 가 아니라 **16개가 실용 지점** · **교사 비용은 그대로** · "대부분"의 나머지 미기재 · 첫 100스텝 후 **효율 곡선 뒷부분이 나쁨** (업75·저자 13인·2609.04172·초록 대조 **high**) *(NEW 2026-09-06)*
- [[LLaDA-Image]] — **페어 데이터 의존을 학습 순서로 끊는다.** 6B DiT를 처음부터 학습하되 **이미지 단독 사전/중간학습으로 시각 생성 사전지식을 먼저** 세우고, 이해 모듈은 **동결**([[LLaDA2.0-Uni]] 계열 LLaDA2.0-Mini 백본) — 볼트의 LLaDA 계보가 **이미지 축으로 뻗은 첫 사례**. 파이프라인 **220M 샘플** · **파라미터 없는 RMSNorm + Muon 옵티마이저**(레시피 공개 논문이 옵티마이저까지 초록에 올림 = 상품이 모델이 아니라 **재현 가능한 절차**). Qwen-Image-Bench **영문 53.53 / 중문 53.38** 오픈소스 최고 주장, **Turbo 증류본 2~4스텝**. 가중치·학습코드·레시피 공개 → [[SolarWM]]과 같은 *"무엇을 공개하는가"* 노선, [[MiniMax-H3]]와 정반대. ⚠️자체 공표 · **벤치가 [[Alibaba]] 계열** · **53.53의 척도 불명** (업226·저자 **30인**(배치 최다)·2609.03796·초록 대조 **high**) *(NEW 2026-09-06)*
- [[Qwen3.8-27B-GSQ-RCO-GGUF]] — ⭐**배치 유일의 카드 내 제3자 대조 벤치마크 보유 · 로컬 배포 1순위.** GSQ(2604.18556)·RCO(2605.00649) 기반 **비균일 혼합정밀**. **IQ3_S(3.50bpw·11.8GB·4.6배 축소)** 가 AIME25 **100.00**·LCB v6 **85.71**로 BF16 **동일**, GPQA-D만 89.39(BF16 89.90)로 -0.51. **동일 파일 크기 8.4GB 고정** 후 Unsloth UD-IQ2_S 대비 **AIME25 +10.00 · GPQA-D +8.59 · LCB +4.57** → [[Repo-To-Skill]] 기준(변수 하나만 토글)을 **모델 카드가 충족한 첫 사례**이자 [[Qwen3.8-27B-GGUF]] 우위 주장의 **반박 근거**. ⚠️**AIME25 100.00은 포화 신호**(만점 구간에선 열화가 안 보임) · 벤치 3종뿐(추론 편중) · 대조군 1개 (DL **348,389**·6일간 **+68.6%**(배치 증가율 1위)·좋아요 433·gated=False·Apache-2.0·HF API **high**) *(NEW 2026-09-06)*
- [[context7]] — ⭐**이 배치 최저비용·최고확실성 즉시 활용 항목.** 라이브러리 문서를 **버전별로** 끌어와 프롬프트에 주입하는 MCP 서버 — 학습 시점 이후 API 변경 환각을 **외부 문서 대조**로 차단. [[검사가능성-공사]]의 **외부 정답 고정 네 번째 형태**(기하·실행결과·사람 유지보수 목록에 이어 **버전 태그 붙은 공식 문서**)이며, 문서는 모델이 쓰지 않았으므로 *검사기=거울* 급소를 **구조적으로 회피**. **issue/star 0.0011로 배치 최저 마찰.** ⚠️보장은 **문서 주입까지**이고 모델이 그걸 따르는 비율 수치 없음 · 버전 오판 시 **틀린 문서를 확신 있게 주입**하는 더 나쁜 실패 · 커버리지·셀프호스팅 미확인 (⭐61,684·fork 2,973·이슈 70·TypeScript·MIT·기본 브랜치 `master`·GitHub API **high**) *(NEW 2026-09-06)*
- [[spec-kit]] — **프로세스를 문서 권고가 아니라 CLI 절차로 강제한다.** 에이전트에게 코드를 시키기 전 **명세를 확정**시키는 SDD 툴킷, **GitHub 공식 조직(id 9919)**. 오픈이슈 **316 = issue/star 0.0024**([[hermes-agent]]의 **1/67**)이나 ⚠️관리 강도와 결함 밀도가 같은 숫자에 섞여 있어 **품질 근거로 직독 금지**. 🎯**[[mattpocock-skills]]가 명시적으로 비판한 대상**이 같은 배치에 들어옴(*"GSD·BMAD·Spec-Kit이 프로세스 전체를 소유해 프로세스 자체의 버그를 못 잡는다"*) — **양쪽 다 정량 근거 없고 issue/star도 사실상 동일** → [[에이전트-스킬]]에 **프로세스 소유권 축** 신설, 판정 보류. ⚠️급소 = **프로세스를 굳히는 도구는 잘못된 명세도 같이 굳힌다** (⭐133,645·fork 12,026·Python·MIT·GitHub API **high**) *(NEW 2026-09-06)*
- [[opencode]] — **에이전트 「본체」** — 이 배치 GitHub 9건 중 나머지 8건이 상부구조(스킬·하네스·문서주입·다이어그램)인데 유일하게 본체. 벤더 비종속이 유일한 차별점이고 ⭐20만은 그 포지션에 대한 표. ⚠️**소유자가 `sst`가 아니라 organization `anomalyco`(id 66570915)** — 인용 시 경로 고정. ⚠️오픈이슈 **5,704**(issue/star 0.0278) · **기본 브랜치가 `main`이 아니라 `dev`**(안정 릴리스가 아니라 개발선이 기본, 이슈 밀도와 정합) · **성능 근거 0** → *"많이 별 받은 에이전트"* 까지만 확정. 볼트 용도는 교체가 아니라 **하네스 변수 통제용 제2 환경** (⭐204,859·fork 26,718·TypeScript·MIT·당일 push·GitHub API **high**) *(NEW 2026-09-06)*
- [[DeepSeek-V4-Flash-Vision-Exp]] — [[DeepSeek]] 계열 **첫 멀티모달 실험 모델**(V4-Flash에 시각 모듈 얹어 연속 학습). 전작 V4-Flash-0731 대비 **ApexBench Pass@1 26.2→36.5(+39%)** · Agents' Last Exam 25.2→27.3, **텍스트 에이전트 성능 유지**가 판매점. 🎯**Opus-4.8 대비 승패가 축마다 갈림** — Agents' Last Exam **27.3 vs 25.7**·ZeroBench 35.0 vs 34.0으로 **이기고**, NL2Repo **57.7 vs 69.7**·DSBench-Hard 63.6 vs 71.7로 **크게 짐**(DeepSWE 59.3 vs 58.0). [[Qwen3.8-27B]]와 **동일한 형태의 분열** → *"에이전트 성능"을 한 덩어리로 인용하면 반드시 틀린다.* ⚠️자체 공표·재현 없음 · 모델명에 **`-Exp`**(벤더 스스로 실험 표시) · 🚨**배치 모델 6건 중 유일하게 파라미터 규모 미확인** (DL 209,191·6일 **+57.3%**·좋아요 698·gated=False·**MIT**·HF API **high**) *(NEW 2026-09-06)*
- [[diagram-design]] — **에이전트 출력물의 편집 가능성을 형식이 결정한다.** *"Self-contained HTML + SVG. No shadows. **No Mermaid slop.**"* — Mermaid는 렌더러가 있어야 그림이 되고 레이아웃이 렌더러에 종속되지만 **SVG는 그 자체가 최종물이고 노드 하나를 손으로 옮길 수 있다** → [[국소-수리-원리]]의 **출력 형식판**(Mermaid는 재생성만, SVG는 국소 수리 가능). 38종은 고정 목록이 아니라 버전 단위로 증가(2.5.10에서 Sankey·fishbone·**Wardley map**·kanban 등 10종) = 실제 자산은 SVG 코드가 아니라 **"어떤 그림을 언제 쓰는가"의 목록**. 📈**24시간 증가율 2.06%로 배치 1위**(절대 스타는 4위 = 아직 확산 중) ⚠️**2개 시점 관측이라 추세 확정 보류.** ⚠️owner가 organization이 아닌 **개인 user** · 품질 대조 없음(*editorial*은 미학적 주장) (⭐31,898·fork 2,051·HTML·MIT·생성 2026-04-16·GitHub API **high**) *(NEW 2026-09-06)*
- [[Qwen3.8-Flash-Next-GGUF]] — Unsloth Dynamic 3.0 GGUF 재배포, **MTP로 추론 1.3~1.7배** 주장. 6일간 DL **+17.3%**로 **양자화본이 원본 유입을 흡수**하는 구도 재확인. ⚠️**핵심 결함 — 카드에 자체 벤치마크 표가 없고 외부 문서 링크만**: *"타 양자화보다 정확도 우수"* 주장이 카드 안에서 검증 불가인데, **같은 배치 [[Qwen3.8-27B-GSQ-RCO-GGUF]]가 동일 조건에서 Unsloth를 이겼다고 수치로 반박**(AIME25 +10.00). → **정확도 우위 주장 미채택.** ⚠️라이선스 `other`(qwen-community-1.0)로 Apache-2.0 계열과 **갈림** · pipeline이 `image-text-to-text`(**VLM 계열**) (DL 823,733·좋아요 804·gated=False·HF API **high**) *(NEW 2026-09-06)*
- [[miles]] — **이 배치에서 유일하게 「학습하는 쪽」에 선 레포**(나머지 8건은 전부 추론 시점 도구). 그런데 ⭐**2,636** — 나머지(3만~25만)와 **최소 12배 차이**로, 이 격차 자체가 발견이다: **담론과 스타는 추론 층에 몰려 있고 학습 층은 같은 트렌딩 안에서도 두 자릿수 배 작다**([[시계열-예측-파운데이션-모델]]과 같은 구조). ⚠️**issue/star 0.368로 배치 최악**(970건)이나 fork/star **0.171**로 최고 = **소수가 깊게 쓰는 레포**. ⚠️*"forked from and **co-evolving with** slime"* 라 **miles 고유 기여 불명** · *"enterprise-facing"* 미정의 · 벤치·처리량 수치 **전무** → **관측만, 설치 안 함** (⭐2,636·fork 450·Python·Apache-2.0·당일 push·GitHub API **medium**) *(NEW 2026-09-06)*

<!-- ── 2026-09-03 자동수집 배치 (신규 7 · 갱신 6 / HF논문 5[전량 신규] + HF모델 3[신규 1·갱신 2] + GitHub 5[신규 1·갱신 4]. **13건 전량 API 실호출 검증** — HF `api/papers` 5편 초록 전문 대조 + `api/models` 3 + `google/timesfm-3.0-pytorch` 추가 조회 + GitHub REST 5 + **[[VoiceStudio]] README 원문 실열람**. 배치 최대 사건 **셋**: ①🚨**규칙 정정 — HF DL 카운터는 공개 직후 "비어 있다"**: 08-28에 [[GLM-5.3-Flash]] 페이지가 세운 *"트렌딩 순위는 사용량 지표가 아니다 · 순위축과 사용축이 애초에 독립"* 규칙을 **철회**한다. 6일 뒤 실측 [[GLM-5.3-Flash]] **34 → 517,902**(15,232배·DL/좋아요 0.024→**260.3**으로 기준선 [[Qwen3.8-27B]] 264에 착지) · [[Qwen3.8-Flash-Next]] **4,810 → 263,287**(54.7배) · `timesfm-3.0` raw *"DL 0"* → **46,862**. **3벤더([[Zhipu-AI]]·[[Alibaba]]·[[Google]])·3모달리티·3규모(321B/180B/331M)** 에서 동일 → **모델 특성이 아니라 플랫폼 집계 방식** 판정. **독립이 아니라 시차였다** — 트렌딩은 약 1주 뒤 사용량의 **선행 지표**. ②🎯**[[에이전트-스킬]]에 「생산 방식」 축 신설** — 같은 날 상위권에 **①수작업**([[ponytail]] 1개·⭐122,495·자체벤치) **②경험**([[hermes-agent]] 미공개·⭐240,408·**근거 없음**) **③증류**([[Repo-To-Skill]] **5,000+개**·**통제 실험 O**)가 동시에 있었고 **규모와 근거가 정확히 반비례**. 급소 = ***증류는 "하지 마라"를 뽑지 못한다*** (레포에 실패는 안 적힌다) → 08-28에 연 억제 축이 **왜 별도로 존재해야 하는지** 설명됨. **5층 후보(검색·선택) 잠정 등재** — Repo-To-Skill은 5,000개를 만들고 선택 메커니즘을 안 적었고 같은 배치 [[CoGR]]이 정확히 그 문제를 푼다(두 논문은 서로를 인용하지 않음). ③🪞**볼트 자신에게 걸린 발견 — `queries/` 0건**: [[CoGR]]이 *아이템 측 표현을 질의 성공 신호로 학습*시키자, 볼트가 **태그를 붙이기만 하고 유효성을 한 번도 측정한 적 없다**는 사실이 드러남. 스키마에 있는 절차가 **5개월간 0회 실행** = 인제스트만 하고 쿼리를 기록 않는 **半쪽 시스템**. 부수: 🔭**희소어텐션 3중 관측**(①[[Declarative-Attention]] 선언·학습불필요·**수치 O** / ②[[GLM-5.3]] `glm_moe_dsa` 자체표 / ③[[Qwen3.8-Flash-Next]] QSA 수치없음 — **공개된 건 ①뿐**) · ⚠️**HF 드리프트 大**(API>raw +60.0/+26.6/+17.3% vs GitHub +2~+14 ≤0.1% — 08-30 *"완전 일치"* 와 상반) · 관찰 2건 판정 보류(**이슈가 스타보다 빨리 증가** hermes 4.8배·ponytail 2.1배 / **"차세대는 덜 열어 둔다"** Zhipu 독자·Alibaba community) · ✅**raw 의심이 원문으로 해소된 드문 사례**([[VoiceStudio]] *"646개 언어"* 를 README가 스스로 한정) · raw 증분 불채택 규칙 **8회 연속**. 신규 entity 0) ── -->
- [[Repo-To-Skill]] — 🎯**[[에이전트-스킬]]에 처음 붙은 통제된 정량 근거.** ML 레포 **1,000개** → **검증된 스킬 5,000+개**(AREX-Skill Library · 20영역 · 178역량군)로 증류하는 연구 에이전트 **DisCo**. 빠진 층을 **운영지식(operational knowledge)** — *"방법을 아는 것과 작동시키는 것을 가르는 노하우"* — 으로 규정하고, 그 지식이 *없는 게 아니라 사람이 읽도록 쓰여 있고 한 태스크에 로드하기엔 너무 크다*고 진단. **GPT-5.5 백본·하네스·실행 예산 고정, 스킬만 토글** → MLE-bench **+134.3%** · PaperBench +34.4% · FrontierCS +9.2% · PassNet +14.0%. ⚠️**전부 상대 증가율·절대 점수 초록 미기재**(2→4.7%도, 30→70%도 +134.3%) · "검증됨"의 절차·**5,000개 선택 메커니즘** 미기재 (업**334**·데일리 **1위**·저자 11인·초록 대조 **high**) *(NEW 2026-09-03)*
- [[SolarWM]] — 모델이 아니라 **데이터 계약**이 기여. 10개 데이터셋 **143만 클립**을 **프레임 정렬 단일 계약**(시각관측·**미터법 카메라 기하**·캡션·품질메타·**선별 결정**·**출처**)으로 변환 — 뒤의 두 필드가 핵심이며 *무엇을 왜 뺐는지*가 함께 배포되면 **혼합 구성을 남이 다시 짤 수 있다**([[MiniMax-H3]]의 *"오픈 가중치 ≠ 오픈 시스템"* 과 **정반대 노선**, 그러면서 그 H3를 백본으로 사용). Wan2.2·[[LTX-2.5]]·[[MiniMax-H3]] 위 **5B~33B 4종**을 네이티브 표현 보존한 채 인스턴스화, 3단 레시피(양방향 적응→교사강요 AR 초기화→분포 정합 증류)로 **5초 학습 → 분~시간 롤아웃 실시간 상호작용**. 데이터·파이프라인·레시피·가중치·프레임워크 전량 공개 표방. ⚠️**초록에 벤치 수치 0** — *실시간*의 fps·GPU 미상, 4모델 비교 없음 (업105·데일리 2위·저자 **18인**·초록 대조 **high**) *(NEW 2026-09-03)*
- [[EarlyEval]] — 🎖️**이번 배치 근거 제시 최상.** 벤치마크 증류가 *태스크 수*를 줄인 데 비해 **각 태스크 안에서** 자르는 상보적 축 — 에이전트의 최종 성패는 **중간 행동에 이미 드러난다**는 관찰에서 **LightGBM 성공/실패 분류기 쌍**을 학습해 보정된 신뢰 임계 도달 즉시 중단. SWE-bench Verified·TerminalBench·Toolathlon에서 **스텝 13~26%·입력토큰 최대 44.1%·출력토큰 29.4% 절감**, 정확도 **89~97%**, **resolve rate 교란 1~2%p까지 자진 보고**(측정 대상을 훼손할 수 있는 개입의 훼손 크기를 스스로 적음). [[FrontierChallenge]]의 *"미통과 75.5%가 「완료」로 종료"* 에 대한 **외부 관찰자형 해법**이자 [[검사가능성-공사]]의 급소를 **검사기를 비-LLM으로 두어** 회피한 실증. ⚠️**참조해답 특징 사용 → 평가용이지 실행용 아님** (업76·데일리 3위·저자 6인·초록 대조 **high**) *(NEW 2026-09-03)*
- [[CoGR]] — LLM을 질의 확장기가 아니라 **질의·아이템 양쪽의 검색 표현(키워드) 생성기**로 직접 학습, 생성된 키워드가 **역색인으로 직접 매칭**돼 **기존 인프라를 그대로 유지**(벡터DB 전환 불필요). SFT로 정렬된 키워드 공간을 세운 뒤 **GRPO 공진화** — 질의 측은 F1 직접 수령, **아이템 측은 반사실적 한계보상**(*내 키워드가 질의 측 F1을 얼마나 바꿨나*). ⭐**raw가 "초록에서 잘림"으로 기록한 수치를 API가 복구**: baseline 10종 대비 **F1 +10.9%(사내 APP Marketplace)·+36.1%(공개 WANDS)**. 🪞**볼트 자신에게 직접 걸린다** — 922개 페이지를 grep/wikilink로 찾으면서 **태그 유효성을 한 번도 측정한 적 없음**(`queries/` 0건). ⚠️사내 데이터 재현 불가·**공개 벤치 개선폭이 3.3배 큰 이유 미설명**·비용/지연 미보고·공진화가 **사적 언어로 수렴하는 실패**와 *"점점 정렬되는 키워드 공간"* 이 겉보기 구분 안 됨 (업61·데일리 4위·**저자 4인 최소**·초록 대조 **high**) *(NEW 2026-09-03)*
- [[Declarative-Attention]] — ⭐**희소어텐션 3중 관측 중 유일하게 수치가 공개된 접근.** 실제 제목은 *"Language Models Can Control Their Own Attention"*(raw는 방법명 기재 — **정정**). 외부 프록시 점수로 토큰을 미리 고르는 방식이 **스텝당 O(N)** 을 못 벗어난다는 진단에서, *"모델이 이미 어디가 관련 있는지 알고 있지 않겠는가"* 라는 **내재적** 접근 — 모델이 사고사슬 안에서 `<global>`/`<focus>`/`<local>` 로 **참조 범위를 선언**하고 추론 엔진이 **툴콜처럼 파싱**해 KV 캐시 대부분을 스킵. **15개 롱컨텍스트 과제 제로샷·기성 모델**: Gemma-4-31B **-52.0%** 참조토큰(정확도 **-1.27%p**) · Qwen-3.6-27B -31.1%(-2.75%p), **손실이 규모와 함께 축소**. **학습 전혀 불필요.** → [[에이전트-메모리-레이어]]에 **"선택적 비독해"**(두지도 압축도 않고 **읽지 않기로 선택**·판단 주체가 **모델 자신**) 신설. ⚠️**-52.0%는 참조 토큰이지 wall-clock 아님** · **추론 엔진 수정 전제**(vLLM/llama.cpp 즉시 적용 불가) · 구현체 미확인 · 규모 효과는 **점 두 개로 그은 선** (업38·데일리 5위·저자 6인·초록 대조 **high**) *(NEW 2026-09-03)*
- [[GLM-5.3]] — [[Zhipu-AI]] 플래그십 **753B** MoE(`glm_moe_dsa`·fp8·`text-generation`·**멀티모달 아님**). [[GLM-5.2]]와 **동일 베이스에서 사후학습만으로** Terminal Bench 3.0 **4.6 → 28.3**(6.2배)·CyberGym 84.5·AutomationBench 48.2·GDPval-AA v2 1769 주장 → 같은 배치 [[Repo-To-Skill]](백본 고정·스킬만 추가)과 **"능력은 베이스 바깥에서 온다"** 는 같은 방향. 🚨**라이선스가 MIT가 아닌 독자 `glm-5.3`** — 볼트가 추적해 온 Zhipu의 개방 노선이 **계열 전체가 아니라 하위 모델에만** 적용됨이 확정([[GLM-5.3-Flash]]는 MIT 유지). **DL 151,021로 Flash(517,902)에 3.43배 열세**이나 좋아요는 1.29배 격차 = *다운로드와 좋아요는 다른 것을 잰다*가 **같은 벤더 플래그십/경량 쌍에서 재현**. ⚠️**벤치 대부분이 [[Anthropic]] Claude Code 하네스 위 벤더 자체 실행**(제3자 재현 0) · GPT-5.6 Sol 34.6에는 **여전히 패배** · raw 대비 DL **+60.0% 드리프트** · `arxiv:2602.15763` 미확인 (트렌딩 1위·HF API **high**) *(NEW 2026-09-03)*
- [[VoiceStudio]] — ⭐**배치 유일 즉시 활용 항목.** raw의 *"ElevenLabs 대체 스택"* 기록과 달리 실체는 **모델이 아니라 16 TTS + 11 ASR 엔진 위의 하네스**(단축키로 교체) — [[SolarWM]]이 비디오 백본에 한 것과 **같은 패턴의 음성판**. **인터페이스 4종: 데스크톱 앱 · 로컬 REST/SSE/WebSocket · OpenAI 호환 오디오 API · MCP 서버** → 내 [[reat-voice]] 파이프라인을 **엔드포인트 교체만으로 로컬 전환** 가능하고 ASR로 **SRT까지 같은 스택**. CUDA·MPS/MLX·ROCm·CPU·원격워커. ✅**raw가 의심한 "646개 언어"를 README가 스스로 한정**(*"actual coverage and quality depend on the selected engine"*) — 과장이 아니라 **명시적으로 한정된 주장**이며 베타 경고·**2층 라이선스**(앱 AGPL-3.0 / 모델은 각자 상위 조건)도 레포가 먼저 밝힘 = **배치 내 서술 정직성 최상**. ⚠️포크당 이슈 **0.0056(배치 최저)** 이나 **워치 73(최저)·Discord 채널 존재**로 *성숙의 증거로만 읽히지 않음* → 볼트 이슈 지표에 세 번째 단서(**트래커 밖 채널이 있으면 이슈 수는 사용량이 아니다**) (⭐15,454·포크 2,157·이슈 12·AGPL-3.0·당일 푸시·API+README 원문 **high**) *(NEW 2026-09-03)*

<!-- ── 2026-09-02 자동수집 배치 (신규 6 · 갱신 7 / GitHub 5[신규 1·갱신 4] + HF논문 5[전량 신규] + HF모델 3[전량 갱신]. **13건 전량 API 실호출 검증** — HF 논문 5편 초록 원문 대조 + **LICENSE 파일 원문 2건 직접 확인**. 배치 최대 사건 **셋**: ①🚨**SPDX NOASSERTION 2건 모두 LICENSE 원문에 실질 제약** — [[OpenClaude]](⭐31,655)는 README에 MIT 배지를 달았으나 LICENSE 파일에 *"does not have Anthropic's authorization to distribute their proprietary source"* 라고 **배포 권한을 자체 부인**(MIT는 *modifications only* + *where legally permissible* 조건부), [[academic-research-skills]](⭐45,261)는 실체가 **CC BY-NC 4.0 = 비상업**. → 신규 규칙 **NOASSERTION은 "정보 없음"이 아니라 "표준 식별자로 담기지 않는 사정 있음"이며 LICENSE 원문 확인 필수**. ②🔓**08-31 DL 동결 해제 — 그리고 철회가 옳았음이 확인됨**: [[Qwen3.8-27B]] 4,511,348→**4,960,483**(+449,135) · [[Qwen3.8-27B-GGUF]] 8,839,153→**9,354,057**(+514,904)로 두 모델 **동시 정상화**(파이프라인 진단 확증). 해제 후 증분이 **GGUF > 원본**이라 08-30의 *"원본 첫 추월"* 은 **재현되지 않음** → 08-31 철회는 정확한 판단. ⚠️단 해소 조건은 **연속 2회**이고 오늘은 1회차, 동결 누적분이라 일간율 분해 불가 → **판정 계속 보류**. ③🎯**새 개념 [[검사가능성-공사]] 신설(잠정)** — [[Qwen-Drive-1.0]](BEV 헤드를 *inspectable interface*로) + [[Safin-1]](메모리를 능동 기질로) + [[UI-Venus-2]](검증을 스케일링 축으로)이 독립 도달, [[국소-수리-원리]]의 **전제**를 짓는 층. 부수: [[국소-수리-원리]] **medium→high**([[DiagEvo]]가 첫 정량 근거) · 층위 3→**5** · 관찰 2건 판정 보류(**저자 수↔수치 제시 16인 경계 완전 분리** n=5 · **이슈 밀도는 도메인 특성을 먼저 반영**) · ⚠️**트렌딩 편향** 절대 스타 1위는 2024-07 생성 [[minimind]] · raw 증분 불채택 규칙 **7회 연속 재현**([[OpenMAIC]] +14로 유일하게 ±10 초과). 신규 entity 0) ── -->
- [[OpenClaude]] — 임의 LLM 백엔드용 코딩 에이전트 CLI(OpenAI 호환·Gemini·GitHub Models·Codex OAuth·Ollama를 **하나의 터미널 워크플로**로: 프롬프트·툴·에이전트·MCP·슬래시 커맨드·스트리밍). 모델 없이 **하네스 계층만** 제공 — *"하네스가 실질 자산"* 명제의 극단. 🚨**LICENSE 원문이 Claude Code 파생임을 밝히며 Anthropic 배포 권한 없음을 자체 고지**(README 배지는 MIT). 포크/스타 **0.284로 배치 최고**(2위의 1.7배)인데 이슈/포크 **0.0081**로 극저 = **가져가되 돌려주지 않는다**. ⚠️**실무 도입 보류 권고** (⭐31,655·포크 8,975·이슈 73·TypeScript·SPDX NOASSERTION·당일 푸시·API+LICENSE 원문 **high**) *(NEW 2026-09-02)*
- [[SMELT]] — 루프드 트랜스포머 평가가 **모델 크기만 맞춰 아키텍처 이득과 추가 FLOPs를 뒤섞었다**는 문제 제기 후, **토큰당 FLOPs·비임베딩 파라미터·KV 캐시 3개 예산을 모두 맞춘** 통제 실험. 중간 절반 레이어 2회 루프 레시피로 비임베딩 **54B까지 4개 규모** 친칠라 스케일링 법칙 각각 적합 → 연산 최적 프론티어에서 **학습 FLOPs 6.8~18.0% 절감**(단일 최대값 아닌 **구간 제시**가 정직). 이득은 **Code 최대**이며 **샘플 길이·인컨텍스트 예시 수에 따라 증가**(= 에이전트 워크로드 유리). 기전: 2회차 방문이 **어텐션 싱크를 줄이고 질량을 내용 관련 토큰으로 재분배**. ⚠️다운스트림 절대 점수·벤치명 미기재·MoE 한정 (업**55**·데일리 2위·저자 9인·초록 대조 **high**) *(NEW 2026-09-02)*
- [[Qwen-Drive-1.0]] — 사전학습 VLM 아키텍처를 **그대로 유지한 채** 3D 인지·VQA·모션 플래닝 통합. 핵심은 외부 BEV 헤드의 **역할 규정** — 성능 부품이 아니라 *"a **probe** of the 3D information... an **explicit, inspectable interface** to 3D scene structure"*, 즉 블랙박스 VLM에 **계기판을 다는 설계**. 3D 탐지·시맨틱 점유예측·BEV 맵 분할 동시 수행. [[검사가능성-공사]]의 대표 사례이며 탐침이 **기하학 과제**라 정답이 모델 **바깥**에 고정돼 심판 비독립성 함정을 구조적으로 회피. ⚠️**데일리 1위인데 수치 0개** — open/pseudo-closed/closed-loop 3종 평가 언급하나 *strong·largely·highly competitive* 형용사뿐, 벤치명·점수·비교 대상 전량 미기재. 제목이 **"An Initial Step"** 으로 스스로 기대치 하향 (업**76**·데일리 1위·저자 16인·초록 대조 **high**) *(NEW 2026-09-02)*
- [[UI-Venus-2]] — 모바일·웹·데스크톱 통합 GUI 파운데이션 에이전트. 병목을 **환경 커버리지·태스크 구성·보상 검증** 셋으로 지목하고 **3축 동시 확장** — 특히 **검증(Verification)을 스케일링 축으로 승격**시켜 시각 키포인트 + **다중모델 투표** 기반 트레이스/샘플 단위 평가자를 학습 인프라로 구축([[Agentic-Artifact-Creation]]의 *심판 비독립성* 경고에 대한 공학적 응답, 단 투표 모델 간 독립성은 미기재). **170개 이상 다국어 모바일 앱 + 네이티브 데스크톱 OS**·오픈소스 명시. ⚠️**저자 31인(배치 최다)·데일리 3위인데 벤치 점수 0개** — ScreenSpot·OSWorld 등 확립된 벤치 이름조차 없음. **셀 수 있는 것은 숫자로 적고 성능만 안 적은 비대칭** (업**50**·저자 31인·초록 대조 **high**) *(NEW 2026-09-02)*
- [[Safin-1]] — 안전성을 외부 가드레일·사후 정렬이 아닌 **모델 자체 연산(native computation)** 으로 구현하려는 시도. **MARCH**(Memory-Anchor Routing across Context History)로 구조화된 메모리 상태를 유지하고 **내용 조건부 라우팅**으로 과거 정보 선택 검색, **백본을 반복 수정하지 않고** 테스트타임에 상태만 적응 → [[국소-수리-원리]]의 **4번째 도달이자 새 층위(아키텍처)**. 개념적 핵심은 메모리를 *"과거의 수동적 기록"* 에서 **"행동을 유지·진화시키는 능동적 기질"** 로 재정의한 것. ⚠️수치 전무(*substantial safety improvements* 서술뿐)·**"안전성"의 정의조차 미기재**·코드 ⭐1로 외부 검증 사실상 없음. 단 저자들이 *"only an initial architectural exploration"* 이라 **스스로 한계 명시** (업17·저자 18인·초록 대조 **high**) *(NEW 2026-09-02)*
- [[DiagEvo]] — 셀프플레이의 방향 신호를 외부 자원(인간 예제·문서 코퍼스·난이도 지정) 없이 **솔버 자신의 실패 이력**에서 유도. 진단기가 반복 오류 원인을 추출해 **계층적 오류원인 메모리**에 저장, 스킬 노드 아래 묶고 **Active / Mastered**로 추적 — [[국소-수리-원리]]의 **문자 그대로의 구현**(Mastered = 건드리지 않는다). 🎯**배치 유일 완전 수치**: 4B 진단기로 솔버 3종(Qwen3-4B·8B·OctoThinker-8B) **전부 9벤치 평균 전 베이스라인 상회**, Qwen3-8B 수학 5벤치 **72.3%(R-Zero 대비 +4.5%p)** · 9벤치 전체 **57.4%(DARC 대비 +1.1%p)** · 제거 실험으로 기여 분리. 🚨**급소**: 이중 신뢰도 필터링이 문항 판정을 **솔버 자신**에게 맡겨 [[Agentic-Artifact-Creation]]의 심판 비독립성에 정면으로 걸린다 — **자신 있게 틀리는 영역이 커리큘럼에서 빠질 수 있다**. ⚠️개별 벤치명·점수·코드 공개 미기재, **전체 평균 이득은 +1.1%p로 완만** (업**9**·데일리 5위·**저자 8인 최소**·초록 대조 **high**) *(NEW 2026-09-02)*

<!-- ── 2026-08-31 자동수집 배치 (신규 5 · 갱신 8 / GitHub 5[전량 갱신] + HF논문 5[전량 신규] + HF모델 3[전량 갱신]. **13건 전량 API 실호출 검증** — HF 논문 5편 초록 원문 대조. 배치 최대 사건 **둘**: ①🚨**HF DL 집계 정지로 08-30 결론 2건 철회** — [[Qwen3.8-27B]] 4,511,348 · [[Qwen3.8-27B-GGUF]] 8,839,153이 **한 자리까지 08-30과 동일**(업로더 다른 두 모델 동시 정지 = 파이프라인 문제, 좋아요는 +109 정상). 08-30의 *"원본이 GGUF 첫 추월·추세로 확정 해소"* 와 *"양자화 우위 붕괴"* 를 **판정 보류로 되돌림**(해소 조건: 연속 2회 정상 증분). [[LTX-2.5]] +303,336이 대조군으로 전역 정지 아님을 입증. ②🎯**신규 5편이 한 원리로 수렴 → 개념 [[국소-수리-원리]] 신설** — [[DART-SD]](학습 층: 손실을 복구 스텝에만, 유효 접두부 보호) + [[LayerRecall]](추론 층: K/V를 메모리 민감 층에만, 국소 어텐션 보존) + [[EASEL]](평가 층: 25모델이 **정점 후 열화** = 실패 시 결과) + [[Agentic-Artifact-Creation]](259편 서베이: *피드백을 표적 수리로, 영향받은 상태만 재검증*). 서로 다른 팀·모달리티의 독립 도달 → 기법 아닌 원리. 부수 관측: [[archify]] **감쇠 후 재가속 첫 관측**(감쇠비 0.79→0.93→**1.12**) · [[scientific-agent-skills]] **이슈 15→23 반등으로 08-30 "첫 감소" 24시간 만에 뒤집힘** → 규칙 *단일 시점 방향 전환은 2회 연속 확인 전 미해석* · [[crawl4ai]] 워치 **409** 배치 최다로 성숙 레포 대비 선명 · [[heretic]] **AGPL-3.0**·푸시 14일 정체 · **린트 정정: 도메인 페이지 누락은 3건이 아니라 6건**(08-23·25·26·27·28·29). raw 기준선 오류 **6회 연속**([[archify]]). 신규 entity 0) ── -->
- [[Agentic-Artifact-Creation]] — 에이전트 산출물 생성 **서베이 259편**(시스템 230 + 벤치 29 · 6계열). "agentic artifact creation"을 **stateful construction**(AI가 실질 구축 + **중간 관찰이 이후 작업을 재지향**)으로 정의하고 운용표현·구축정책·**런타임 검증** 3부품으로 분해. ⚠️핵심은 두 부정적 결론 — **분해는 국소 복잡도를 낮추는 대신 조정·재조립 비용을 올린다**(서브에이전트 증설 통념 반박), **LLM 심판은 생성기와 선호·맹점을 공유하면 독립 증거가 못 된다**. 서베이라 성능 수치 없음·컷오프 08-20 (HF 데일리 **1위**·업**36**·저자 12인·초록 대조 **high**) *(NEW 2026-08-31)*
- [[LayerRecall]] — 자기회귀 비디오 확산의 최근 컨텍스트 캐싱이 **재등장 단서를 밀어내는** 문제를 층 선택적 메모리 라우터로 해결. 핵심 관찰은 *"접근만으로는 효과적 사용이 보장되지 않는다"* 이며 **video DiT 층마다 현재/최근/먼 과거 선호가 다름** → 과거 K/V를 **memory-sensitive 층에만 주입**하고 나머지는 국소 어텐션 보존. CHPM으로 장문맥 교사가 예측 공간 지도. 멀티샷 100개에서 MemoBench·MovieBench 최고 + **VBench-Long 백본과 동등 유지**(=장거리 회복을 얻으며 국소 연속성 미훼손). **메모리 유도 자기교정** 질적 관측. ⚠️**절대 수치 초록 전무** (업18·저자 5인·초록 대조 **high**) *(NEW 2026-08-31)*
- [[Puro-2B]] — **RTX 5090 단일 컨슈머 GPU · FP8 · 최대 1.4조 토큰** 사전학습 레시피를 **데이터·코드·가중치 전량 Apache 2.0** 공개. 빠져 있던 것은 오픈 모델이 아니라 **오픈 레시피**였다는 문제 제기. ⚠️**raw 요약 오류 교정** — 실측 최고 모델은 **<$6.9K로 Qwen2.5-1.5B 근접**이고 **$4.4K는 스케일링 법칙 외삽치**이며 목표도 한 세대 아래 **Qwen2**-1.5B(제목 "$5090 이내"는 예측값). 대조군 Llama-3.2-3B **>$1.5M** · SmolLM3-3B 재현 **>$700K**. **두 번째 결과가 더 중요** — 전체 파이프라인 접근 덕에 *사전학습 데이터 커리큘럼이 사후학습 성능을 어떻게 형성하는가*를 통제 실험 (업13·저자 11인·초록 대조 **high**) *(NEW 2026-08-31)*
- [[EASEL]] — **dexterous visual tool use**(시각 증거→도구 파라미터 추론→폐루프 정밀 실행) 벤치. 프록시 과제는 레퍼런스 보고 캔버스에 그려 맞추기. **모델 25종** 평가 결과 재현 유사도 **0.40~0.54 병목**, ⚠️궤적 진단상 **조기 포화 또는 정점 후 열화**가 공통 — 즉 **잘한 상태를 못 알아보고 계속 손대서 망친다**(특정 모델 결함 아닌 현 세대 구조적 한계). EASEL-Data **44만 샘플**로 학습한 EASEL-9B도 베이스 대비 **상대 +6.3%·전체 3위**에 그쳐 **데이터로 밀어도 병목 못 벗어남** (업13·저자 6인·초록 대조 **high**) *(NEW 2026-08-31)*
- [[DART-SD]] — 순서 무관 하위목표 과제의 최적해 공간은 **조합적 다이아몬드 격자**인데 단일 궤적으로 강제하면 **위상 붕괴**가 일어나 타당한 대안 탐색까지 무차별 벌점·정책 다양성 훼손. ISTG 모델링 → **CTB(임계 위상 분기점)** 탐지 → **손실을 복구 스텝에만 배타 계산하고 유효 추론 접두부는 파괴적 갱신에서 보호**. ⚠️**정량 수치 전무** — 벤치명조차 미기재, *"significantly outperforms"* 한 문장뿐. 공개 후 12일로 **관측 창 밖** (업11·저자 8인·문헌 실재 high / **효과 크기 미검증**) *(NEW 2026-08-31)*

- [[Agentic-Game-Development]] — 게임 엔진을 월드모델의 "컴파일러"로 쓰자는 제안(**RLHEV**). HF 데일리 **1위**·업130이나 **정량 결과 0건**의 포지션 페이퍼 *(2026-08-29 신규)*
- [[UrbanGround]] — 홍콩 전역 3D 실축척 복제본 샌드박스. 원자 능력은 되는데 **장기 탐색에서 합성 실패·오차 누적**. 데일리 3위·저자 18인·**정량 점수 미기재** *(2026-08-29 신규)*
- [[Self-OPD]] — flow matching **교사 없는 온폴리시 증류**(K개 SDE 갈래 + 결정론적 자기참조 기준선). 데일리 5위·**수치·벤치명·K값 전량 미기재** *(2026-08-29 신규)*
<!-- 소스별 요약 -->
<!-- ── 2026-08-27 자동수집 배치 (신규 7 · 갱신 6 / GitHub 5[신규1·갱신4] + HF논문 5[전량 신규] + HF모델 3[신규1·갱신2]. **13건 전량 API 실호출 검증** — HF 논문 5편 초록 원문 대조. 배치 주제: **"결과가 아니라 과정을 채점하라"가 두 도메인에서 독립적으로 등장**. [[FrontierChallenge]](과학 워크플로·데일리1위)와 [[VGI-BENCH]](영상생성·데일리4위)가 무관한 분야인데 동일 설계 원칙. 최대 관측은 **자기보고 완료 선언의 붕괴** — 미통과 Claude Code 트레이스 **75.5%가 "완료했다"로 종료**·전기화학/환경 **Avg. 94.9 대 Pass Rate 0%** → [[Thinkingbox]]→[[apache-maka]]→[[DiffusionOPSD]] 계보 5번째이자 에이전트 **진술**이 무너진 첫 사례. 하네스 축은 **패치→생성**으로 승격([[JIT-Agent]] 고정 4모듈 기계 생성·"모델 스케일링과 직교" 주장)하나 같은 날 FrontierChallenge의 스캐폴드 천장(20.6%)과 병치되어 **경계 미해결**. [[WarpSAC]]은 동형 메시지의 RL판(데이터 체제 함수·19.8%→96.4%). [[VoiceMem]]은 **top-5가 Mem0 top-200보다 +약30점**으로 검색량 아닌 구조 분리를 입증. HF 격일 갱신 **4회 연속 성립**·원본/GGUF 비율 0.402→**0.432** 2연속 상승. 이슈 밀도 배치 내 **470배 격차**(claude-plugins-official 0.26건/포크 vs ai-job-search 0.00056). 신규 entity 0) ── -->
- [[archify]] — 아키텍처·워크플로·시퀀스·데이터플로·라이프사이클 다이어그램을 **자체완결(self-contained) HTML**로 출력하는 에이전트 스킬(모션·내보내기 지원). [[에이전트-스킬]] 1층이되 *행동*이 아니라 **출력 형식**을 규정하는 변종 — 오늘 배치를 이으면 **생성([[JIT-Agent]]) → 전달(archify) → 판정([[FrontierChallenge]])**. ⚠️ **"verifiable(검증가능)·crisp export" 품질 주장은 전량 자체 서술이며 조작적 정의·근거 미확인**이고, **도식은 틀렸을 때 텍스트보다 위험**(정리된 것으로 받아들여져 의심을 덜 받음) (GitHub ⭐**20,152**·포크 1,346·이슈 35·JavaScript·MIT·생성 2026-04-15·**당일 푸시**·트렌딩 **1위**·API 실검증 **high**) *(NEW 2026-08-27)*
- [[FrontierChallenge]] — 최종 답이 아니라 **엔드투엔드 워크플로 완수와 산출물 완결성**을 채점하는 교차도메인 벤치(300개 중 **97개 공개**·양자화학/분자동역학/재료/분석화학/생명과학/전기화학·환경). 프런티어 모델 **12종 × 스캐폴드 3종** 평가에서 최고 구성도 97개 중 20개만 완수 = **Pass Rate 20.6%**. **부분점수가 완수로 이어지지 않음이 극단적으로 드러남** — 분석화학 Avg. **87.6** 대 Pass **4%**, 전기화학/환경 Avg. **94.9** 대 Pass **0%**. 결정적으로 **미통과 [[anthropic-claude-code]] 트레이스의 75.5%가 "완료했다"는 문장으로 종료** → 부분점수도 자기보고 완료도 완수 지표가 못 된다는 실증. ⚠️ **완수 기준 채점 규칙·12모델 개별 점수·저자 소속·코드 공개 여부 전량 미기재**, 비공개 203개 난이도 분포 미상 (HF 데일리 **1위**·업**97**·저자 16인·공개 08-25·**초록 원문 대조 high**) *(NEW 2026-08-27)*
- [[VoiceMem]] — 듀플렉스 음성 언어모델용 메모리로 정보 담당 **좌뇌** ∥ 감정 담당 **우뇌**를 병렬 배치 + 스트리밍 메모리 I/O·**백엔드 교체형 분리 배포**. 핵심은 **top-5 검색으로 Mem0의 top-200보다 약 30점 우위**(후보 **40배 감소**인데 더 정확) = "많이 긁어오면 낫다"는 RAG식 직관의 반박, 설계 논점이 저장소 선택에서 **경로 분리**로 이동. 우뇌는 페르소나 벤치 3종 SOTA·종합 **+4.29점**, 검색 **134ms로 표준 VAD 지연 안쪽**이라 **대화 지연 추가 0**(절대속도가 아니라 기존 대기시간 안에 숨기는 재정의). [[MemTrapBench]](메모리 5종이 no-memory보다 못함)와 같은 방향이나 **벤치·태스크가 달라 반증 아님** → 정식화는 **"성패는 도입 여부가 아니라 경로 분리 방식"**. ⚠️ **절대 점수·벤치 3종 이름·134ms 측정 조건 미기재**·코드 ★28로 재현 보고 부재 (HF 데일리 2위·업**82**·저자 10인·공개 08-26·초록 원문 대조 **high**) *(NEW 2026-08-27 · local-llm)*
- [[WarpSAC]] — 대규모 병렬 시뮬레이션이 **데이터 체제(regime)를 바꾸면서 기존 안정화 기법이 체제 의존적**이 된다는 진단(벤치 8계열 통제실험): 파라미터 정규화는 리플레이 커버리지가 좁을 때만 이롭고 데이터가 풍부하면 **가치 적합을 제약**, clipped double-Q는 고처리량 매니퓰레이션에서 완화 가능, age-biased replay 가중은 체제 무관 이로움. 이에 따라 **WarpSAC-L**(Norm ON·clipped double-Q·CPU 규모) / **WarpSAC-A**(Norm OFF·single-Q·GPU 병렬)로 분기 + Sample Weight Decay. FlashSAC 대비 AUC **CPU 9환경 +4.5% · GPU 병렬 14환경 +23.1%**(이득 차이 자체가 체제 의존성의 자기 증거)·**UnitreeG1TransportBox-v1 19.8%→96.4%**·MuJoCo Playground 실시간 AUC +19.1%·Unitree G1 sim-to-real **36.4% 단축**. [[JIT-Agent]]와 층은 다르나 **"모델 밖 설정이 성능을 지배"** 동형 메시지. ⚠️ **절대 성능값 없음(96.4%만 절대)·8개 벤치 이름 미기재·"데이터 체제"의 임계 정의 부재**(내 환경이 어느 체제인지 판별 불가)·코드 ★1 (HF 데일리 3위·업**78**·저자 12인·공개 08-25·초록 원문 대조 **high**) *(NEW 2026-08-27)*
- [[VGI-BENCH]] — 영상 생성 모델이 생성 프레임으로 제로샷 시각 추론을 하는지 검증하는 **27과제·810인스턴스** 벤치(과제도메인×스킬태그 2단 분류). **그럴듯한 최종 상태가 아니라 유효한 전개 과정을 요구**하도록 설계 — [[FrontierChallenge]]와 **무관한 도메인에서 동일 교정**. 최강 [[Seedance]] 2.0도 자체 기준 **51.0%**(생성 품질↔시각 추론이 별개 능력임을 보여주는 수치). 디노이징 내부 분석에서 **후반 스텝이 초기 가설을 다듬을 뿐 추론 오류를 교정 못 함**(자기교정 한계) → **실무 함의: 스텝 증가 재시도는 화질을 올리지 추론을 못 고친다**, 구도·인과 오류는 조건 변경으로 재생성해야 함. ⚠️ **다른 모델 개별 점수·"자체 평가 기준" 채점 규칙 미공개**·코드/데이터 **"공개 예정"** 상태로 제3자 재현 불가 (HF 데일리 4위·업**65**·저자 22인·공개 08-20·초록 원문 대조 **high**) *(NEW 2026-08-27 · video-saas)*
- [[JIT-Agent]] — 메모리 관리·계획 전략·행동 프로토콜·툴/스킬 오케스트레이션을 묶은 에이전트 하네스를 **고정 4모듈 프로토콜의 기계 생성 산출물로 형식화**하고, 임의의 기성 에이전트 LLM에 맞춰 **즉석 합성·수리·자가진화**(이전 구성 아카이브에서 성능 신호 증류)시키는 모델. 하네스 개선 축 계보의 **네 번째이자 첫 "생성"**([[AutoSaddler]] 설계시점 패치→[[Recuris]] 실행시점 진화→[[SkillEvo]] 스킬 갱신→JIT-Agent 전체 생성). DeepSeek-V4-Flash가 **DeepSearchQA +9.1·OdysseyBench +4.3으로 GPT-5.6 추월**·GLM-5.2 최대 **+20.2**·DeepSeek V4/Mimo-V2.5/Qwen3.6 다중 계열 일관 개선. 주장: 하네스 지능은 **학습·이전·누적 가능하며 모델 스케일링과 직교**. ⚠️ **절대 점수 전면 미기재·"OpenCode/Claude Code와 경쟁력 있다"의 판정 기준 부재·하네스 생성 비용 미기재**(태스크마다 뽑는다면 비용이 실용성을 좌우)·4모듈 표현력 한계 미논의. ❓ 같은 날 [[FrontierChallenge]]의 스캐폴드 천장(20.6%)과 병치 → **하네스 지배 구간↔모델 천장 구간 경계 미해결** (HF 데일리 5위·업**39**·저자 16인·공개 08-26·초록 원문 대조 **high**) *(NEW 2026-08-27)*
- [[Qwen3.8-27B-Uncensored-Aggressive-MTP-GGUF]] — [[Qwen3.8-27B]] 파생 **가드레일 제거 + MTP 계열 GGUF**. DL **911,795**로 [[Qwen3.8-27B-OBLITERATED]](**468,746** API 실검증)의 **약 1.95배** → **가드레일 제거 축 내부 선두 교체**(이 축 세 번째 상위 진입). 원본 대비 **27.6%**·GGUF 대비 **11.9%** 수요로 08-26의 **"양자화 ≫ 원본 ≫ 가드레일 제거"** 3분화는 선두 교체 후에도 유지 = **축 내부 순위는 바뀌어도 축 간 규모 차는 안정적**. ⚠️ **제거 방식·"Aggressive"의 조작적 정의·"MTP"의 의미(multi-token prediction 추정이나 미확인)·능력 손실 벤치·라이선스 준수 전량 미검증**이며 이 축 세 모델 모두 능력 손실 측정치 **0건**. 최종수정 08-17 이후 커밋 없이 다운로드만 유입 (HF DL 911,795·좋아요 671·트렌딩 10위·apache-2.0 베이스 상속·API 실검증 **low**) *(NEW 2026-08-27 · local-llm)*
<!-- ── 2026-08-26 자동수집 배치 (신규 7 · 갱신 6 / GitHub 5[신규1·갱신4] + HF논문 5[전량 신규] + HF모델 3[신규1·갱신2]. **13건 전량 API 실호출 검증** — HF 논문 5편 초록 원문 대조. 배치 주제: **"지표가 서로 다른 이야기를 할 때 어느 쪽을 믿을 것인가"**. 최대 발견은 **HF 다운로드 격일 갱신 패턴** — [[Qwen3.8-27B]]·[[Qwen3.8-27B-GGUF]] 다운로드가 08-25 값과 완전 동일하게 재동결(08-24 동결→08-25 해제→08-26 재동결·두 모델 3회 연속 동기)됐으나 **좋아요는 정상 갱신**(+205/+1) → 두 지표가 별개 파이프라인. 08-25 "동결=관측 실패" 규칙을 **2일 단위 해석 + 좋아요 교차 확인 + 비율은 양쪽 갱신 회차에만** 으로 정교화, 이번 회차 증분·비율은 미해석 처리. 부수 발견: **이슈 밀도 2,000배 격차**(apache-maka 0.91건/포크 vs ai-job-search 0.0004건/포크·awesome-gpt-image-2는 스타 +38.5%에 이슈 감소) = 이슈는 품질이 아니라 소프트웨어 종류의 지표. **인기≠신뢰성 3연속**(TradingAgents 10만 돌파+푸시 5주 정체 / claude-plugins-community 스타 최하위+트렌딩 2위 / Recuris 업보트 최하위+코드 공개). 신규 entity 0·엔티티 2 갱신[[[Anthropic]]·[[Tencent]]]) ── -->
- [[OraRL]] — 어노테이션을 **oracle rollout으로 on-policy 그룹에 직접 투입**하는 비디오 MLLM RL. 핵심 진단은 **advantage inversion**(고보상 oracle이 그룹 baseline을 올려 양수여야 할 정책 advantage를 음수로 뒤집음) → **decoupled advantage estimator**로 baseline은 oracle 제외·격차는 방향성 이득과 분리된 oracle advantage로만 작용. **CoT 없이 디코딩 130ms**(기존 4,780ms·약 36.8배)·**VSI-Bench 73.1**(GPT-5 55.0·Gemini-3-Pro 55.1)·temporal mIoU 62.5→66.0·tracking AO 73.0→78.2·segmentation 64.3→70.4·공간지능 매크로 51.0→56.1·**SFT 2.2배 스텝시간**(CoT GRPO 4.9배의 절반 이하)·0.8B~9B 스케일 (HF 데일리 **1위**·업65·저자7인·초록 원문 실검증 high·코드 공개 여부 미확인) *(NEW 2026-08-26)*
- [[WeMM-Embedding]] — [[Tencent]] 위챗 **실배포** 범용 멀티모달 임베딩 2B/4B/9B(텍스트·이미지·비디오·시각문서·인터리브 입력 + **가변 출력 차원**·가중치·코드 공개). **2B가 기존 최상위 8B 오픈소스를 MMEB-v2에서 추월**·**9B 전체 80.6 SOTA** = 임베딩에서 크기 프리미엄 붕괴 → 인덱싱 비용 구조 변화. 공개 벤치보다 **온라인 A/B 14건 일관 개선 + 사내 26태스크**라는 배포 면적이 더 강한 신호(채널스·공식계정·모먼트·이커머스) (HF 데일리 2위·업49·저자6인·초록 실검증 high·**사내 수치는 정성 서술뿐**·학습 데이터 비공개) *(NEW 2026-08-26)*
- [[AutoSaddler]] — 하네스(프롬프트+툴 설정+제어 로직)를 **코드로 보고 실패 트레이스로 자동 패치**하는 오프라인 학습 프레임워크. **GAIA2 +9.0%p·SWE-Bench Pro +9.6%p·Terminal-Bench 2.0 +10.0%p** — **모델 무수정·배선만으로** 세 벤치 두 자릿수 근처 상승 = 에이전트 실패의 큰 몫이 **하네스 설계 부채**. 어블레이션 3종이 실무 규칙: **깊은 디버깅**(얕은 자기반성 무효)·**표적 수정**(전체 재작성 금지)·**일반화 인식 선택**(케이스별 예외 처리는 과적합). [[Recuris]]와 **설계시점 vs 실행시점**으로 층이 갈림 (HF 데일리 3위·업45·저자13인·초록 실검증 high·**절대점수·사용모델·최적화 비용 미기재**) *(NEW 2026-08-26)*
- [[DiffusionOPSD]] — 디퓨전 **교사 없는 온폴리시 자가증류**. 문제 정의가 절반 — *"끝점 보상은 중간 디노이징 예측이 어떻게 바뀌어야 하는지 말하지 않는다"* → 이미지 단위 보상을 clean-output 예측의 명시적 타깃으로 변환(동결 정책이 앵커 공급 → 유계 양·음 타깃 → detached 유한 피팅 → EMA 갱신). SD 3.5-M+Z-Image-Turbo 2백본·10평가자·**20설정 중 19개 최고**·최강 경쟁 대비 **최대 +44.0%**·DiffusionNFT 대비 **GPU시간 40%/63% 절감**. 진짜 기여는 **타깃 구성 이득과 실현 이득의 분리 측정** — 둘은 비례하지 않았고, 이는 [[Thinkingbox]]·[[apache-maka]]·[[OPD-Dual-Nature]]에 이은 **"중간지표≠최종결과" 네 번째 사례** (HF 데일리 4위·업40·저자17인·초록 실검증 high·**진 1개 설정·평균 개선폭 미공개**) *(NEW 2026-08-26)*
- [[Recuris]] — 롱호라이즌 하네스용 **재귀적 경험-작업기억 진화**(코드 공개). Working Memory가 진행 상태를 추적해 *전체 히스토리가 아니라 현재 필요*로 Experiential Memory에서 스킬 선택 → 실행이 **구조화된 증거**가 되어 실패를 메모리 구성요소로 국소화 → Meta-Agent가 **검증 게이트 통과 갱신만** Skill Memory에 반영하는 유계 재귀 루프. **4벤치×10모델 완료 37쌍 중 35쌍 개선**·tau-bench **[[Claude Opus 5]] +15.6점 → 87.9%**·GPT-5.6 Sol +17.8·SkillFlow Qwen3.6-27B/35B +16.6/+13.5·**최장 태스크 +32.2점**·흔한 롱호라이즌 실패 **최대 80% 감소**. **지평이 길수록 이득이 커진다**는 것이 서명이며 [[MemTrapBench]]("메모리 5종 전부 no-memory보다 못함")의 **유력 반증 후보** (HF 데일리 5위·업16[배치 최하위인데 코드+수치 최다]·저자8인·초록 실검증 high⭐5·**게이트 판정 기준·Meta-Agent 비용·미완료 쌍 수 미기재**) *(NEW 2026-08-26)*
- [[claude-plugins-community]] — [[Anthropic]] 공식 **플러그인 커뮤니티 마켓플레이스 read-only 미러**(등록은 외부 폼으로만·레포는 인덱스 역할). [[에이전트-스킬]] 생태계에 **배포/레지스트리 층**이 벤더 주도로 처음 등장 — 커뮤니티 큐레이션([[mattpocock-skills]]·[[superpowers]])도 벤더 규격([[cursor-plugins]])도 아닌 **"남이 만든 것을 싣는 목록을 벤더가 소유"**. ⭐**1,957**(배치 최하위)인데 **트렌딩 2위**(당일 +351 = 절대값의 18%) = *트렌딩은 증분 속도*([[free-claude-code]] 선례) 재확인. 이슈 42건(0.23건/포크)은 자산 레포치고 높아 트래커가 등록 문의 창구로 쓰일 가능성 (포크181·Python·Apache-2.0·생성 2026-03-20·GitHub API 실검증 high·**심사 기준 미확인·등재≠검증**) *(NEW 2026-08-26)*
- [[Qwen3.8-27B-OBLITERATED]] — [[Qwen3.8-27B]] 파생 **가드레일 제거(abliterated)** 판·**safetensors+GGUF+MLX 3포맷 동시 배포**(로컬 런타임 전체 겨냥). DL **389,747** = 원본(295만)의 **13.2%**·GGUF(733만)의 5.3% → 트렌딩 3위지만 **절대 수요는 주변부**. 같은 베이스 파생이 **양자화 ≫ 원본 ≫ 가드레일 제거**로 3분화한 실측. ⚠️ **제거 방식·능력 손실 벤치·`ai-safety-research` 태그 정당성 전량 미검증**이며 라이선스는 베이스 상속 표기 ([[Qwen3.8-27B-Uncensored-GGUF]]에 이은 이 축 2번째 상위 진입·좋아요770·공개 2026-08-19·HF API 실검증 **low**) *(NEW 2026-08-26)*
...
- [[에이전트-스킬]] **[개념]** — 모델을 바꾸지 않고 행동을 바꾸는 **배포 가능한 지식 자산**, 생태계 4개 층 정리(1층 자산 [[mattpocock-skills]]·2층 방법론 [[superpowers]]·3층 벤더규격 [[cursor-plugins]]/[[claude-plugins-official]]·4층 진화 [[SkillEvo]])·구조적 약점(재현성 벤치 부재·스타≠품질·멀티턴 결함 비가시·규격 이식성 미해결)·스킬 소스 20건+ 누적에도 부재했던 **스키마 "3소스 규칙" 장기 미이행 해소** *(NEW 2026-08-22 · concepts)* *(**UPDATED 2026-09-03** — 🎯**개설 이래 처음으로 통제된 정량 근거가 붙었다**([[Repo-To-Skill]]: 백본·하네스·예산 고정 후 스킬만 토글 → MLE-bench +134.3%). 17개 소스가 전부 *스타·자체 주장* 이었던 상태가 끝났고, **평가 기준을 「백본 고정 대조가 있는가」로 올린다.** 🆕**「생산 방식」 축 신설**(①수작업 [[ponytail]] 1개·⭐12.2만·자체벤치 → ②경험 [[hermes-agent]] 미공개·⭐24.0만·**근거 없음** → ③증류 [[Repo-To-Skill]] **5,000+개**·**통제 실험 O**) — **규모와 근거가 정확히 반비례.** 급소 = ***증류는 \"하지 마라\"를 뽑지 못한다*** (레포에 실패는 안 적히므로 **억제 규범은 원리적으로 증류의 사각지대**) → 08-28의 억제 축이 **왜 별도로 존재해야 하는지** 설명됨. **5층 후보(검색·선택) 잠정 등재** — 승격 조건 *스킬 검색을 명시적으로 다루는 소스 2건 추가*)*
...
<!-- ── 2026-08-23 자동수집 배치 (신규 1 · 갱신 12 + 엔티티1·개념1 / GitHub 1[신규 openai-codex] + GitHub 4·HF모델 3·HF논문 업보트 5[갱신]. **GitHub REST·HF API 실호출로 13건 전량 절대 수치 대조 + HF논문 5편 초록 원문 재대조**. **raw 오류 2건 적발·정정: ECC·MiniMax-H3를 "신규"로 분류했으나 둘 다 기존 페이지** — 실제 신규는 openai-codex 1건. **HF 데일리 페이퍼 2026-08-21 정지(주말 미갱신)로 논문 5건은 업보트 재수집이지 신규 발표 아님**. 관측 3축: (1) 코딩 에이전트 **개방성 대비** 확보 — codex Apache-2.0·Rust 본체 공개 vs claude-code 라이선스 미명시·배포채널 (2) ECC README에서 **커맨드→스킬 인터페이스 전환** 포착 (3) 스킬 1층↔2층 **성장률 역전 이틀째 고착** + 상위 두 레포 모두 **커밋 정체 중 스타만 증가** → 트렌딩 자기강화 루프. 신규 entity 0) ── -->
- [[openai-codex]] — OpenAI 공식 **로컬 코딩 에이전트 CLI 본체**, [[anthropic-claude-code]]의 직접 경쟁 구현체이나 **개방성 축에서 역전**([[openai-codex]] Apache-2.0·Rust 본체 소스 공개 vs claude-code 라이선스 미명시·이슈 15,113의 배포채널)·배포 4경로(CLI·IDE확장·데스크톱 `codex app`·클라우드 Codex Web)·과금이 API키가 아닌 **ChatGPT 요금제 종속**·파생 [[codex-plugin-cc]]/[[awesome-codex-skills]]/[[CodexBar]]가 **원천보다 먼저 인제스트된 역순 구조 해소**·⚠️공식성≠성능([[SWE-bench-Science]] 최고 성능은 Claude Code+Opus-5) (GitHub ⭐**114,383**·포크 17,459·이슈 13,516·**Rust**·**Apache-2.0**·트렌딩 1위·당일 푸시·**API 실검증**·성능 벤치 레포 미제시 high) *(NEW 2026-08-23)*
- [[mattpocock-skills]] 갱신 — 에이전트 스킬 자산 1층 대표, ⭐230,381→**232,854**(이틀 **+2,473**·배치 최대 급상승·[[superpowers]]의 약 4.2배)·⚠️**최종 푸시 08-21 정체 상태에서 스타만 유입** → 급상승은 콘텐츠 개선이 아닌 **트렌딩 노출 기인**·품질 벤치 여전히 부재(**GitHub API 실검증**·포크 19,868·MIT) *(UPDATE 2026-08-23)*
- [[superpowers]] 갱신 — 에이전트 스킬 프레임워크+방법론 2층 대표, ⭐275,812→**276,399**(이틀 +587·증분 하루 300선까지 둔화)·**성장률 역전 이틀째 고착**·⚠️최종 푸시 08-19 **나흘째 정체**·단 방법론은 채택 후 반복 소비되지 않아 둔화≠쇠퇴(**GitHub API 실검증**·포크 24,726·MIT) *(UPDATE 2026-08-23)*
- [[ECC]] 갱신 — 코딩 에이전트 하네스 OS, ⭐236,434→**242,304**(23일 +5,870·배치 절대 2위)·**README 재실측으로 구성 변동 포착: 서브에이전트 67→68·스킬 281→286·슬래시커맨드 94는 개수 동일하나 표기가 "94 legacy command shims"로 격하** = **커맨드를 하위호환 계층으로 내리고 스킬을 주 인터페이스로 승격**([[에이전트-스킬]] "스킬=배포 단위" 명제의 제품 구조 증거)·AgentShield npm 독립 배포(`ecc-agentshield`)·⚠️07-31 기록한 "정적룰 102개" 표기는 이번 README에서 미발견(보류)·⚠️**raw의 "신규" 분류 오류**(07-29 생성 기존 페이지)(**API+README 실검증**) *(UPDATE 2026-08-23)*
- [[anthropic-claude-code]] 갱신 — Anthropic 공식 Claude Code 레포, ⭐135,978→**142,680**(50일 +6,702·**당일 증가폭 배치 최저 +127**)·**성격 재규정: 에이전트 본체 소스가 아니라 배포·이슈 트래킹 채널**(라이선스 미명시·오픈이슈 15,113·Python)·⚠️**본문 에코시스템 수치 전면 낡음 정정** — 파생([[superpowers]] 276,399·[[ECC]] 242,304·[[mattpocock-skills]] 232,854)이 **원천 142,680을 스타에서 크게 추월**, "원천=중심축" 서술을 "중심축은 제품이되 스타 중력은 스킬·하네스로 이동"으로 수정(**GitHub API 실검증**) *(UPDATE 2026-08-23)*
- [[Qwen3.8-27B-GGUF]] 갱신 — [[unsloth]] 재배포 GGUF(imatrix), DL 6,320,542→**6,674,515**(좋아요 2,659·이틀 +353,973)·**증분 일 52만→일 17.7만 약 1/3로 급감**·08-22 "첫 둔화" 신호가 **추세로 확정**·생성 08-13로 열흘 만 667만 = **확산 피크 통과·성숙 진입**(쇠퇴 아님)·최종수정 08-20 사흘째 정체(**HF API 실검증**) *(UPDATE 2026-08-23)*
- [[Qwen3.8-27B]] 갱신 — [[Alibaba]] 27B 베이스 원본(image-text-to-text), DL 2,090,699→**2,358,347**(좋아요 **12,196**·이틀 +267,648·**트렌딩 1위 이틀 연속**)·**핵심은 감속 비대칭** — GGUF는 일 52만→17.7만(약 1/3)인데 원본은 일 18만→13.4만(완만) → **원본/GGUF DL 비율 0.33→0.35로 08월 처음 원본 쪽 반등**·"원본=인정·재배포=실행" 분업이 **고정 구조가 아니라 확산 단계 종속 현상**임을 시사·좋아요는 여전히 GGUF의 4.6배(**HF API 실검증**) *(UPDATE 2026-08-23)*
- [[MiniMax-H3]] 갱신 — 베이스 원본 `MiniMaxAI/MiniMax-H3` DL 2.86M→**4,039,236**(좋아요 4,355·나흘 +약118만·**400만 돌파**·공개 2026-07-28로 약 4주 만)·⚠️**트렌딩 1위→11위 급락이나 다운로드 증분은 견조(일 29.5만)** → 순위는 상대 지표로 [[Qwen3.8-27B]] 계열·파생판의 상위 점유 결과이지 H3 수요 감소 아님·**트렌딩 상위에서 영상 생성 축을 지키는 사실상 유일 대형 모델**([[video-saas]] 오픈 i2v 백본 후보 유지·경쟁 부재로 대안 협소)·⚠️**raw의 "신규·raw 첫 등재" 분류 오류**(08-06 생성·베이스 원본 08-12부터 반복 등재)·모델 카드 벤치 미기재 medium(**HF API 실검증**) *(UPDATE 2026-08-23)*
- HF논문 업보트 갱신 5 (**HF 데일리 2026-08-21 정지로 신규 발표 아님·업보트 재수집**·초록 원문 재대조) — [[EnvHarness]] 242→**248**(데일리 1위 유지·이틀 +6으로 유입 사실상 정지·초록 확정: 4도메인 5벤치 원본·전용 파이프라인 모두 상회, held-out **최대 +9.0점**을 **실행 스텝 9.8% 절감**과 동시 달성·원본 verifier 보존·EnvRigger 블랙박스 결함 표적 합성) · [[FACET]] 111→**112**(+1 완전 정체·초록 확정: 컨테이너 실행 상태를 지시문/해법/verifier의 **공유 기준점**으로·실패 산출물만 표적 수리·⚠️**Terminal-Bench 2.1 향상 방향만 있고 구체 점수 초록 미기재**) · [[4DAnyone]] 61→**67**(+6·[[EnvHarness]]와 함께 유일하게 유입 잔존·초록 확정: 실패 원인을 **bounded-attention-context**로 규정, **RCP**가 참조 컨텍스트 O(N)→**O(1)** 압축·**TCR**이 고노이즈에서 그룹 간 컨텍스트 공유·저노이즈에서 디테일 안정화·MVGameHuman 자체 구축·⚠️정량 수치 미기재) · [[SWE-bench-Science]] 58→**60**(+2·초록 확정: 20분야 98레포 **119태스크**·3패러다임·**최고 성능 Claude Code+Opus-5(max)조차 pass@1 50% 미만**·실패 기전 4종·**절제 실험 — 과학 지식은 일률적으로 이롭지 않고 정렬 어긋나면 앵커링 유발**·이번 배치 [[openai-codex]] 편입으로 "공식성≠성능" 근거로 직결) · [[WithEveryone]] 38→**39**(+1 최저 유입이나 **정량 근거는 최다** — 얼굴 유사도 GPT-Image-2 0.462→**0.499**·복붙 아티팩트 0.169→**0.055**·커버리지 **97.3%**·중복률 **2.8%**·최대 10인·**Layout-Grounded ID Loss**가 불안정한 임베딩 매칭 대신 주석 얼굴 영역 직접 감독·[[FlashPrefill-V2]] 선례에 이어 **업보트 순위↔검증 가능성 역상관 반복 관측**) *(UPDATE 2026-08-23)*

<!-- ── 2026-08-24 자동수집 배치 (신규 7 · 갱신 6 + 개념 4 갱신 / GitHub 2[신규 awesome-gpt-image-2·apache-maka] + HF논문 5[신규·**데일리 정지 해제 후 첫 진짜 신규 발표**] + GitHub 3·HF모델 3[갱신]. **GitHub REST·HF Papers/Models API 실호출 13건 전량 + maka README 원문 + 동반 레포 1건 실검증**. **raw 오류 1건 적발·정정: "트렌딩 상위 3모델 다운로드 카운터 전량 미갱신"은 부분 오류 — [[MiniMax-H3]]는 4,465,161로 정상 갱신(하루 +425,925)이며 동결은 Qwen 계열 2건뿐**(08-22·08-23에 이어 **raw 파생 해석 오류 3회 연속 적발**). 관측 4축: (1) [[MiniMax-H3]]가 **트렌딩 11위 유지 중 수요 1.4배 가속** → "순위≠수요" 교과서적 실증 (2) [[Graph-Engineering]]의 **Prompt→Context→Harness→Loop 4단계 계보**를 위키 분류축으로 채택 (3) [[apache-maka]]의 *"짧은 컨텍스트 ≠ 삭제된 역사"* 로 메모리 오염에 **이벤트 소싱 대안** 편입 (4) 스킬 1층이 **이미지 도메인으로 확산**하는 동시에 코딩 1층은 **커밋 나흘 정체 중 스타만 유입**. 신규 entity 0) ── -->
- [[apache-maka]] — **ASF 인큐베이팅** 로컬 우선 에이전트 워크스페이스, 모델 메시지·툴 호출·툴 결과·**권한 결정**·종료 이벤트를 **append-only 로그**로 기록하고 UI·다음 모델 호출을 그 **뷰**로 취급 → *"Shorter context is not deleted history"*(오래된 툴 출력은 생략하되 **증거는 보존**)·[[MemTrapBench]]가 보인 메모리 오염에 **저장 방식이 아닌 아키텍처 층위의 답**·[[에이전트-메모리-레이어]]에 **이벤트 소싱 계열** 첫 편입·데스크톱/TUI/eval이 **단일 Runtime Host** 통과·[[Graph-Engineering]] 그래프 표현과 **경쟁 형식**(비교 실험 없음)·⚠️**Linux 미지원**(macOS arm64 초기 릴리스)·데이터 형식/CLI 변경 예고 → 프로덕션 아닌 **설계 참조 대상** (GitHub ⭐**2,497**·포크 279·이슈 239·TypeScript·**Apache-2.0**·트렌딩 7위·당일 푸시·**API+README 원문 실검증**·성능 벤치 미제시 **high**) *(NEW 2026-08-24)*
- [[awesome-gpt-image-2]] — GPT-Image-2용 **프롬프트 엔진·템플릿 자산 라이브러리**, [[에이전트-스킬]] 1층(자산)이 **코딩 도메인 밖 이미지 생성으로 확산한 첫 관측**(원문 *"Prompt as Code … 提炼出Skills"*)·이슈 **11건**이 *"복사되는 텍스트 자산 vs 실행되는 소프트웨어"*([[openai-codex]] 13,620) 구조 차이 재확인·⚠️**"530+ 사례·20+ 템플릿·공업급" 전량 자체 주장 미검증**·프롬프트 자산은 **모델 버전 종속**이라 코딩 스킬보다 부패가 빠를 수 있음(버전 대응표 유무 미확인) (GitHub ⭐**14,407**·포크 1,545·JavaScript·MIT·트렌딩 **2위**·생성 2026-04-25·당일 푸시·**API 실검증** **medium**) *(NEW 2026-08-24)*
- [[MoE-LR-Transfer]] — 대규모 MoE 학습률 전이(*Let's Scale Step by Step*), **μP를 MLA+Muon MoE에 적응**시켜 폭(width) 축 최적 학습률 일관 전이 + 소형 프록시 최적값에 **선형회귀로 10조 토큰 지평 R²=0.95 외삽** → **총 155B/활성 17B 파운데이션 모델을 실제로 처음부터 사전학습해 검증**·볼트 ai-news의 드물던 **사전학습(학습 축)** 빈칸을 채움([[FlashPrefill-V2]]는 추론 축)·⚠️저자 소속·모델 정체·평가 점수 초록 미기재(소속 추정 기록 안 함) (HF 데일리 **신규 목록 1위**·업**25**·저자 5인·**초록 원문 실검증 high**) (2608.20061) *(NEW 2026-08-24)*
- [[OmniAssistBench]] — Omni-LLM **실시간 어시스턴트 상호작용** 벤치, *모델 응답이 사용자 다음 행동을 바꿔* 정적 데이터셋으로 평가 불가한 문제를 **원본 영상 유래 사전정보로 경로를 강제**해 해결·인터넷 영상 **역공학**(전문가 **1,000시간+**)·**Gemini-3-Pro 66.4 vs Qwen3-Omni-Instruct 51.2**/100(**오픈-상용 15.2점 격차**·**1등조차 66.4**)·실패 3종(시각 프롬프트·히스토리 유지·**목표 이벤트까지 응답 지연 실패** ← 즉답 훈련과의 **학습 목표 수준 미스매치**)·[[SWE-bench-Science]] pass@1 50% 미만과 같은 결 (HF 신규 **2위**·업21·저자 9인·**초록 원문 실검증 high**·벤치 공개 여부/채점 구성 미기재) (2608.21360) *(NEW 2026-08-24)*
- [[Graph-Engineering]] — 에이전트 최적화 **4단계 계보**(**Prompt→Context→Harness→Loop Engineering**) 정리 후 개별 지능의 5대 한계(이질적 전문성·상호의존 서브태스크·병렬 실행·독립 검증·지속 상태)를 들어 **시스템 지능**·그래프 패러다임 제안·**볼트 소스를 그대로 정렬하는 분류 어휘**([[에이전트-메모리-레이어]]=Context·[[에이전트-스킬]]=Harness·[[SkillEvo]]=Loop)로 [[AI-에이전트-프레임워크]]에 채택·**선행 정량 구현체 [[Repo0]] Dual-DAG**(FC +20.08%p·Pass Rate +29.74%p)와 *추상 제안↔검증 구현* 짝·⚠️**포지션/서베이로 자체 실험·벤치 수치 전무**(어휘 채택이지 우월성 채택 아님)·동반 레포 DEEP-JLU ⭐29 실재 확인 (HF 신규 **3위**·업21·저자 **35인**·**초록 원문 실검증 high**) (2608.21156) *(NEW 2026-08-24)*
- [[OPD-Dual-Nature]] — 온폴리시 증류 일반화의 **양면성**(*Every Coin Has Two Sides*), 요인을 하나씩 통제한 실험으로 [[온폴리시-증류]] 통념 교정: **전이되는 것은 교사의 정답이 아니라 추론 행동**(난이도 거의 무관·**교사가 못 푸는 문제도 유효**)·성패는 **교사-학생 출신(origin) 동일성**이 좌우(같은 출신은 언어·추론지평·타 도메인까지 근접·다른 출신은 학습 분포에만 적합)·**다교사 결합의 능력 시소는 도메인 전문가 라우팅으로 격리 불가**(MoE·라우팅 해법을 사전 차단)·[[DOPD]] "특권 착각"에 이은 **성숙기 정점**·⚠️**절대 수치·모델·"출신" 정의 전무로 방향성만 인용 가능** (HF 신규 **4위**·업9·저자 16인·**초록 원문 실검증 high**) (2608.16647) *(NEW 2026-08-24)*
- [[InfinityEdit]] — **무한 비디오 편집** 과제 정의 + 경량 Edit-Ignition 어댑터, 기존 **in-place 편집 가정**(고정 구간 프레임 단위 정렬)을 파기하고 *"앞 구간+편집 요청→다음 구간 생성"* 무한 반복으로 재정의·모듈 3종(History cross-attn·**Temporal causal self-attn**[미래 정보 누수 차단]·Edit cross-attn)·**편집 요청 청크에서만 어댑터 활성화 후 앵커 리셋**해 원본 모델의 무한 생성 능력 보존 → *긴 영상 부분 수정 시 전체 재생성 회피* 패턴은 **백본 무관 이식 가능**·[[AI-영상-생성-2026]]의 "스트림 중간 편집" 빈칸을 채움·⚠️**정량 비교 수치·"경량" 근거(파라미터/지연시간/VRAM) 전무**·코드 공개 미확인·**스트리밍 생성기 백본 전제**([[MiniMax-H3]] 충족 여부 미확인) (HF 신규 **5위**·업9·저자 12인·**초록 원문 실검증 high**) (2608.20910) *(NEW 2026-08-24)*
- [[openai-codex]] 갱신 — OpenAI 공식 코딩 에이전트 본체, ⭐114,383→**116,429**(하루 **+2,046**·포크 17,749·이슈 13,620·**당일 푸시**·트렌딩 1위)·**이번 배치 유일한 "커밋+스타 동반 상승"** — 자산 레포 [[mattpocock-skills]]가 커밋 0건으로 스타만 받는 것과 대조되며 **스타 증분에서 처음 자산 레포를 추월**·⚠️트렌딩 표기 +2,715 vs 실증분 +2,046 불일치 지속(절대값 우선) *(UPDATE 2026-08-24)*
- [[mattpocock-skills]] 갱신 — 스킬 자산 1층 대표, ⭐232,854→**234,572**(하루 +1,718·포크 **2만 돌파**·이슈 391)·⚠️**푸시 08-21 나흘째 정체 — 08-22~08-24 누적 +4,191 유입 동안 커밋 0건**·증분도 +2,473→+1,718로 **약 30% 둔화** → 08-23의 *"트렌딩 노출 자기강화 루프"* 판단이 **추세로 확정**·1층 경쟁이 코딩 내에서 포화되자 [[awesome-gpt-image-2]]처럼 **새 도메인으로 확산** *(UPDATE 2026-08-24)*
- [[free-claude-code]] 갱신 — 코딩 에이전트 무료 토큰 브리지, ⭐10,167(2026-04-25)→**48,309**(**4개월 만 재등장·약 4.75배**·포크 7,915·Python·MIT·당일 푸시)·**성격 확장**: Claude Code 단일 우회 → **Claude Code·Codex·Pi·OpenCode 4종**을 터미널/앱/IDE/모바일에서·04-25 이후 방치된 페이지를 **옵시디언 풀 포맷으로 재작성**·[[apache-maka]](로컬 우선·데이터 통제)와 **정확히 반대 극**(접근 비용 인하)·⚠️**"13억+ 무료 토큰"·"ToS 친화" 전량 자체 주장 미검증**·제3자 경로 경유 시 코드/프롬프트 흐름 통제 불가 → **채택 대상 아님 low** *(UPDATE 2026-08-24)*
- [[MiniMax-H3]] 갱신 — 베이스 원본 DL 4,039,236→**4,465,161**(하루 **+425,925**·좋아요 4,392·**446만 돌파**)·⚠️**raw 오류 정정**: 수집 메모가 *"3모델 전량 카운터 미갱신"* 으로 분류했으나 본 모델은 **정상 갱신**·**증분이 일 29.5만→42.6만으로 오히려 1.4배 가속**한 반면 **트렌딩은 11위 유지** → 08-23의 *"순위 하락은 냉각이 아닌 Qwen 상위 점유"* 판단이 정면 입증, **트렌딩 순위≠수요의 교과서적 사례**·영상 축 상위 유일 대형 오픈 모델 지위 유지·⚠️**라이선스 `other` 로 상업 이용 조건 미검증** *(UPDATE 2026-08-24)*
- [[Qwen3.8-27B-GGUF]] 갱신 — DL **6,674,515 동결**(08-23과 바이트 단위 동일·`downloadsAllTime` null로 교차검증 불가)·좋아요 2,659→**2,765**(+106 정상 이동) → **지표 전체 정지가 아니라 30일 롤링 다운로드만 미갱신**·⚠️**증분을 "성장 0"으로 해석 금지**·08-23의 *"증분 급둔화(일 52만→17.7만)"* 후속 확인은 **다음 회차로 이월** *(UPDATE 2026-08-24)*
- [[Qwen3.8-27B]] 갱신 — DL **2,358,347 동결**·좋아요 12,196→**12,386**(+190·**배치 최대 절대 증가**·GGUF 2,765의 약 **4.5배**)·**트렌딩 1위 사흘 연속**·⚠️본 모델과 GGUF **둘 다 동결이라 원본/GGUF 비율(0.35) 변동 판단 불가 — 이월**·DL 정지 구간에도 좋아요는 상승해 *"원본=인정 기준점·재배포=실행 수단"* 분업은 좋아요 축에서 유지·`eval-results` 태그 보유하나 **구체 벤치 수치 여전히 미수집**(성능 주장 금지) *(UPDATE 2026-08-24)*
- 개념 갱신 4 — [[AI-에이전트-프레임워크]]([[Graph-Engineering]] **4단계 계보를 분류축으로 채택**·[[Repo0]] Dual-DAG↔[[apache-maka]] 로그의 경쟁 표현 형식 명시) · [[에이전트-메모리-레이어]]([[apache-maka]] **이벤트 소싱 대안** 편입·기록과 주입의 분리) · [[에이전트-스킬]](1층의 **이미지 도메인 확산** + 코딩 1층 **커밋 정체 고착**·프롬프트 자산의 **모델 버전 종속** 약점 추가) · [[온폴리시-증류]]([[OPD-Dual-Nature]] **성숙기 정점** 편입·교사 선택 규칙 교정) · [[AI-영상-생성-2026]]([[InfinityEdit]]로 **스트림 중간 편집** 축 신설·2026-04 이후 첫 갱신) *(UPDATE 2026-08-24)*
...
- [[PhysBrain]] — 🔴 **09-19 정정: 물리법칙 추론 모델이 아니라 사람 1인칭 영상 → VLA 이전 임바디드 모델**([[DeepCybo]] · 후속 [[PhysBrain-1.5]]) · 업보트 52→61 *(NEW · 정정 2026-09-19)*
...
- [[ponytail]] — "최고의 코드는 쓰지 않은 코드" 억제형 에이전트 행동 규범 자산, ⭐114,596 MIT (배치 최대·스킬 생태계 첫 빼는 축·효과 측정 0건) *(NEW 2026-08-28)*
- [[claude-obsidian]] — Karpathy LLM-Wiki 패턴의 패키지 구현체(Obsidian+Claude Code), ⭐14,215 MIT (**이 볼트의 첫 직접 대조군**·이슈밀도 0.098 배치최고) *(NEW 2026-08-28)*
- [[PAWBench]] — 월드모델을 확률 분포 샘플러로 평가(PAWEval·50시나리오×11시스템 **전멸**) (2608.27345, 초록 원문 대조) *(NEW 2026-08-28)*
- [[TTPO]] — 라벨 없는 테스트 시점 정책 최적화(불일치=대개 오답 비대칭성·Qwen3-1.7B 38.0→45.2%) (2608.27448, 초록 원문 대조) *(NEW 2026-08-28)*
...
### 2026-09-08 배치 (13건 — GitHub 5 · HF논문 5 · HF모델 3)

- [[browser-use]] — ⭐**113,014**(볼트 최대급)·MIT·22개월 개발. **10개월 지연 등록** — 파생 [[video-use]] 는 두 번 잡고 본체는 못 잡았다. 원인은 수집기가 **트렌딩 증가폭**으로만 후보를 만들어 **확산 끝난 초대형 레포가 구조적으로 배제**되는 것. ⚠️raw의 *"댕글링 4개"* 는 **실측 2개**
- [[hyperframes]] — ⭐47,070·[[HeyGen]] 공식·Apache-2.0. **모델이 아니라 렌더러**(HTML→puppeteer→ffmpeg). 생성모델 3대 고질병(일관성·텍스트·재현성) **회피**, 대신 실사 불가 → **경쟁이 아니라 분업**. ⚠️Remotion과 **대체재**
- [[camofox-browser]] — ⭐10,080·MIT. 차단 회피를 **모델이 아니라 브라우저 지문 층**에서 푼다(drop-in). 🔴**우회 성공률 없음** — 적대적 도메인에서 시점 없는 주장은 무효
- [[AutoHedge]] — ⭐5,428(당일 +517, 트렌딩 1위). 🔴**raw 기재 오류**: *"자동매매"* → README 실측 **솔라나 전용**. 🔴push **120일 정체**(deprecated 문자열 검사는 통과 — **동결은 안 잡힌다**). 🔴수익률 전무
- [[OmniVoice]] *(갱신)* — 이름 토큰 대조 **첫 히트**. **분리 아닌 병합** 판정(권리가 안 갈림 — [[timesfm-3.0-pytorch]] 와 대조). 갱신 중 **4월 인제스트가 arXiv 논문·GitHub를 통째로 놓쳤음**이 드러남. HF DL 958k→**1,153,331**(+20.4%)
- [[Uno]] — HF 업보트 **19(배치 1위)**. AR 가중치 유지 + **경량 diffusion만 증류** → **draft 모델 없이 최대 3배**, **8B가 26B DiffusionGemma·Mercury 2를 전 벤치 상회**. **코드·체크포인트 공개** = 볼트가 직접 검증 가능한 유일 논문. → [[선택비용과-중복성]] **4번째 낭비: 순서**
- [[One-Symptom-Three-Levers]] — 업 7·**리뷰**(신규 실험 없음 명시). 붕괴를 **3레버**(토큰가중/특권정보종류/교사동역학)로 정리. 🎯**리뷰의 출현 자체가 분야 수렴의 증거** — *수렴은 논문 개수가 아니라 메타 논문으로 판정한다*. [[DOPD]] 의 "특권 착각"이 리뷰 중심 주장으로 승격
- [[TGOPD]] — 업 5·저자 9. 프롬프트별 교사 신뢰도 **사전 검증** 후 통과→dense OPD / 실패→GRPO **배타 라우팅**. **6/6 설정 승리**(4B/**35B**). 💰**유휴 교사 GPU로 검증비 조달 9.8%→78.9%** — *"검사기는 공짜가 아니다"* 반론의 첫 구체적 답
- [[FlowBalance]] — 업 10·코드 공개. 자기 가이던스를 검증기로 **부호 교정**(+유지 / −**반전** / 무선호 비활성). **이론 보장 4건**. 붕괴를 **응답길이 + 전략다양성** 두 측정량으로 조작화. ⚠️향상 폭 수치 없음
- [[EmbodiedSkills]] — 업 5·저자 17·코드 공개. 스킬 결정을 **실행 제안으로 격하** → 사전조건검사→실행→**사후검증** 고정 인터페이스. RoboTwin **86.20%**·LIBERO **97.40%** / 🔴기억의존 RMBench **12.5%**(저자 자기공개). 🎯[[TGOPD]] 와 **2도메인 독립 수렴**
- [[gpt2]] — DL **14,748,356**(배치 최대). 2019 모델·최종수정 2024-02. **7개 런타임 포맷 + DOI + exbert** = 모델이 아니라 **테스트 픽스처**. → **DL 생애 단계 3분류**(신규/현역/**인프라**) 확립, **DL 순위가 최신성의 역순**
