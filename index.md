---
title: Wiki Index
updated: 2026-08-26 (자동수집 13건 배치 인제스트 — **신규 7**[GitHub 1: claude-plugins-community / HF논문 5: OraRL·WeMM-Embedding·AutoSaddler·DiffusionOPSD·Recuris / HF모델 1: Qwen3.8-27B-OBLITERATED]·**갱신 6**[GitHub 4 + HF모델 2] + **개념 4 갱신**·엔티티 2 갱신·actionable 7건 추가. **13건 전량 API 실호출 검증** — HF 논문 5편 초록 원문 대조. **최대 발견: HF 다운로드 카운터의 격일 갱신 패턴 확정 근거 확보** — Qwen 원본·GGUF 다운로드가 08-25 값과 완전 동일하게 재동결(08-24 동결→08-25 해제→08-26 재동결, 두 모델 3회 연속 동기화)됐는데 **좋아요 축은 정상 갱신**(원본 +205·GGUF +1). 한 페이지에서 한 축만 얼었다 = 별개 집계 파이프라인 → 08-25 규칙을 **"증분은 2일 단위 해석 + 좋아요 축 교차 확인 의무화 + 비율은 양쪽 갱신 회차에만 계산"** 으로 정교화. 관측 5축: (1) **이슈 밀도 2,000배 격차** — apache-maka 0.91건/포크(볼트 최고) vs ai-job-search 0.0004건/포크, awesome-gpt-image-2는 스타 +38.5%에 이슈 11→10 감소 = 이슈는 품질이 아니라 **소프트웨어의 종류**를 말함 (2) **인기≠신뢰성 3연속 실증** — TradingAgents 10만 돌파했으나 **푸시 5주 정체**(트렌딩 5위인데 커밋 0), claude-plugins-community는 스타 최하위(1,957)인데 트렌딩 2위, Recuris는 업보트 최하위(16)인데 코드 공개+수치 최다 (3) **에이전트 개선이 두 층으로 분화** — AutoSaddler(설계시점 하네스 코드 패치·3벤치 +9.0~10.0%p) vs Recuris(실행시점 메모리 진화·37쌍 중 35쌍·Opus 5 tau-bench 87.9%), 둘 다 **모델 무수정으로 두 자릿수 개선** (4) **"중간지표≠최종결과" 네 번째 사례** — DiffusionOPSD가 타깃 구성 이득과 실현 이득을 분리 측정해 비례하지 않음을 확인(Thinkingbox·apache-maka·OPD-Dual-Nature 계보) → 구조적 패턴으로 승격 (5) 스킬 생태계에 **배포 레지스트리 층**(Anthropic 공식 마켓플레이스)과 **자가진화 층**(Recuris Skill Memory) 동시 등장. 신규 엔티티 0)
total_pages: 944  # 2026-08-26 실측(sources 886 · entities 34 · concepts 18 · synthesis 2 · domains 4 · queries 0)
total_sources: 886  # 2026-08-26 실측 (신규 소스 7건 반영)
---
