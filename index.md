---
title: Wiki Index
updated: 2026-08-24 (자동수집 13건 배치 인제스트 — **신규 7**[GitHub 2: apache-maka·awesome-gpt-image-2 / HF논문 5: MoE-LR-Transfer·OmniAssistBench·Graph-Engineering·OPD-Dual-Nature·InfinityEdit]·**갱신 6**[GitHub 3 + HF모델 3] + **개념 5 갱신**. GitHub REST·HF Papers/Models API 실호출 13건 전량 + maka README 원문 + 동반 레포 1건 실검증. **HF 데일리 정지 해제** — 08-21~08-23 사흘간 동일 목록을 반환하던 데일리가 갱신돼 논문 5편 전부가 **08월 배치 최초의 진짜 신규 발표**(단 신규 목록 최상위 업보트 25는 정지 기간 누적 1위 EnvHarness 257의 약 1/10이라 순위 비교는 "신규 목록 내"로 한정). **raw 오류 1건 적발·정정: "트렌딩 상위 3모델 다운로드 카운터 전량 미갱신"은 부분 오류** — MiniMax-H3는 4,465,161로 정상 갱신(하루 +425,925)이며 동결은 Qwen 계열 2건뿐(**raw 파생 해석 오류 3회 연속 적발**). 관측 4축: (1) MiniMax-H3가 **트렌딩 11위 유지 중 수요 1.4배 가속** → "순위≠수요" 교과서적 실증 (2) Graph-Engineering의 **Prompt→Context→Harness→Loop 4단계 계보**를 AI-에이전트-프레임워크 분류축으로 채택(포지션 논문이라 어휘만 채택·벤치 수치 없음) (3) apache-maka의 *"짧은 컨텍스트≠삭제된 역사"* 로 MemTrapBench 오염 문제에 **이벤트 소싱 대안** 편입(단 Linux 미지원·설계 참조용) (4) 스킬 1층이 **이미지 도메인으로 확산**(awesome-gpt-image-2)하는 동시에 코딩 1층은 **커밋 나흘 정체 중 스타만 +4,191** — openai-codex가 당일 푸시로 스타 증분 첫 추월. free-claude-code 4개월 방치 페이지를 풀 포맷 재작성(⭐10,167→48,309·주장 전량 미검증 low). 신규 엔티티 0)
total_pages: 932  # 2026-08-24 실측(sources 874 · entities 34 · concepts 18 · synthesis 2 · domains 4 · queries 0)
total_sources: 874  # 2026-08-24 실측 (신규 소스 7건 반영)
---
