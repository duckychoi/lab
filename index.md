---
title: Wiki Index
updated: 2026-08-27 (자동수집 13건 배치 인제스트 — **신규 7**[GitHub 1: archify / HF논문 5: FrontierChallenge·VoiceMem·WarpSAC·VGI-BENCH·JIT-Agent / HF모델 1: Qwen3.8-27B-Uncensored-Aggressive-MTP-GGUF]·**갱신 6**[GitHub 4 + HF모델 2] + **개념 2 갱신**·도메인 3 갱신·actionable 6건 추가. **13건 전량 API 실호출 검증** — HF 논문 5편 초록 원문 대조. **최대 발견: 서로 무관한 두 도메인에서 같은 평가 교정이 독립적으로 나왔다** — [[FrontierChallenge]](과학 워크플로)와 [[VGI-BENCH]](영상 생성)가 같은 날 **"그럴듯한 최종 상태가 아니라 유효한 전개 과정을 요구한다"** 는 동일 원칙으로 설계됨 → 평가 방법론 전반의 이동. 관측 6축: (1) **자기보고 완료 선언이 처음으로 반증됨** — 미통과 Claude Code 트레이스의 **75.5%가 "완료했다"로 종료**, 전기화학/환경은 **Avg. 94.9인데 Pass Rate 0%**(분석화학 87.6 대 4%·최고 구성 20.6%). Thinkingbox→apache-maka→DiffusionOPSD 계보의 5번째이자 **외부 지표가 아닌 에이전트 진술이 무너진 첫 사례** (2) **하네스 축이 패치→생성으로 승격** — [[JIT-Agent]]가 하네스를 고정 4모듈 기계 생성물로 형식화(AutoSaddler 오프라인 패치→Recuris 온라인 진화→SkillEvo 스킬 갱신→**JIT-Agent 전체 생성**), "하네스 지능은 모델 스케일링과 **직교**" 주장. ⚠️단 같은 날 FrontierChallenge가 스캐폴드 3종 교체에도 20.6% 천장 보고 → **하네스 지배 구간↔모델 천장 구간의 경계**가 이 축 다음 쟁점 (3) **모델 밖 설정이 성능을 지배**의 RL판 — [[WarpSAC]]이 안정화 기법을 데이터 체제의 함수로 진단, 체제별 2변종 분기로 **UnitreeG1 성공률 19.8%→96.4%**(GPU 병렬 AUC +23.1% vs CPU +4.5%로 체제 의존성 자기 증거) (4) **메모리는 검색량이 아니라 구조 분리** — [[VoiceMem]] **top-5가 Mem0 top-200보다 약 30점 우위**(후보 40배 감소)·**134ms로 VAD 예산 안쪽**. MemTrapBench 반례와 함께 읽어 **"성패는 도입 여부가 아니라 경로 분리 방식"** 으로 정식화 (5) **HF 다운로드 격일 갱신 4회 연속 성립** — Qwen 원본·GGUF **완전 동기 해제**(+353,154 / +303,896)로 08-24 동결→25 해제→26 동결→**27 해제** 교대 확정. 원본/GGUF 비율 **0.402→0.432** 2회 연속 상승(양자화판보다 원본 유입이 빠름) (6) **가드레일 제거 축 선두 교체** — HauhauCS 911,795가 OBLITERATED 468,746의 **약 1.95배**, 단 "양자화≫원본≫가드레일 제거" 3분화는 유지. 신규 엔티티 0)
total_pages: 951  # 2026-08-27 실측(sources 893 · entities 34 · concepts 18 · synthesis 2 · domains 4 · queries 0)
total_sources: 893  # 2026-08-27 실측 (신규 소스 7건 반영)
---
