---
title: Wiki Index
updated: 2026-08-23 (자동수집 13건 배치 인제스트 — **신규 1**[GitHub 1: openai-codex]·**갱신 12**[GitHub 4 + HF모델 3 + HF논문 업보트 5] + 엔티티 1·개념 1 갱신. GitHub REST·HF Models/Papers API 실호출 검증 체제 유지 — 13건 전량 절대 수치 대조, HF논문 5편 초록 원문 재대조. **raw 오류 2건 적발**: ECC·MiniMax-H3를 '신규'로 분류했으나 둘 다 기존 페이지(ECC 07-29 생성·MiniMax-H3 08-06 생성·베이스 원본은 08-12부터 raw 반복 등재) → 실제 신규는 openai-codex 1건뿐. **HF 데일리 페이퍼가 2026-08-21에 정지**(주말 미갱신)해 논문 5건은 신규 발표가 아닌 업보트 재수집. openai-codex 편입으로 **코딩 에이전트 개방성 축 대비 확보**(codex Apache-2.0·Rust 본체 공개 vs claude-code 라이선스 미명시·배포채널). ECC README 재실측으로 **커맨드→스킬 인터페이스 전환** 포착(94 슬래시커맨드 → 'legacy command shims' 격하·스킬 281→286). 스킬 1층↔2층 성장률 역전 이틀째 고착(mattpocock +2,473 vs superpowers +587). Qwen GGUF 증분 급둔화(일 52만→17.7만)로 원본/GGUF 비율 첫 반등. **total_sources 906→867 실측 정정**. 신규 엔티티 0)
total_pages: 925  # 2026-08-23 실측(sources 867 · entities 34 · concepts 18 · synthesis 2 · domains 4 · queries 0)
total_sources: 867  # 2026-08-23 실측 정정 (이전 표기 906은 실제와 불일치)
---
