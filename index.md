---
title: Wiki Index
updated: 2026-08-28 (자동수집 13건 배치 인제스트 — **신규 7**[GitHub 2: ponytail·claude-obsidian / HF논문 3: PAWBench·TTPO·ACE-Agentic-Data / HF모델 2: Qwen3.8-Flash-Next·GLM-5.3-Flash]·**갱신 6**[archify·awesome-gpt-image-2·OpenMontage·VoiceMem·WarpSAC·Qwen3.8-27B] + **개념 4 갱신**[에이전트-스킬·온폴리시-증류·월드모델·**LLM-Wiki**]·엔티티 2 갱신[Alibaba·Zhipu-AI]·도메인 1 갱신·canvas 13노드 12엣지·actionable 7건 추가. **13건 전량 API 실호출 검증** — HF 논문 5편 초록 원문 대조. **최대 발견: 같은 날 세 논문이 독립적으로 "분포로 이동"했다** — [[PAWBench]](평가를 분포 정렬로·50시나리오×11시스템 **전멸**)·[[TTPO]](학습 신호를 분포적 비대칭성으로·라벨 없이 감독 OPSD와 동등·Qwen3-1.7B 38.0→45.2%)·[[ACE-Agentic-Data]](생성을 제약된 분포 설계로·(E,q,τ,v)+ACE). 08-27의 "점(결과)→과정" 수렴에 이어 **"단일 사례→분포"로 이틀 연속 독립 수렴**이며, 셋 중 [[ACE-Agentic-Data]]만 서베이라 **이미 진행 중인 전환의 뒤늦은 명명**으로 읽힌다. ⚠️내부 긴장: TTPO 전제(*불일치=대개 오답*)는 정답이 하나인 도메인의 성질일 수 있는데 PAWBench는 *유효 답이 여럿*을 전제한다. 관측 7축: (1) **HF 트렌딩 순위는 사용량 지표가 아님이 수치로 확정** — DL/좋아요가 [[GLM-5.3-Flash]] **0.024**(트렌딩2위·DL **34**·좋아요 1,393=41배)·[[Qwen3.8-Flash-Next]] **1.19**(1위)·[[Qwen3.8-27B]] **264.5**(3위)로, **1·2위 실사용 합계가 3위의 0.14%**. → **264.5를 볼트 실사용 기준선으로 승격** (2) ⚠️**raw 수집 메모 기준선 오류 3건 — 볼트 관측 계열을 정본 채택**: [[Qwen3.8-27B]] "+512,272 동결해제"→실제 **+159,118**(08-27에 이미 해제)·[[archify]] "+4,239"→**+4,834**·[[awesome-gpt-image-2]] "이틀+3,830"→**하루+1,516**. **규칙 확립: raw 증분 서술 불신·볼트 페이지의 직전 API 실측을 기준선으로.** 부수로 격일 동결의 강한 형태(*항상 교대*)는 **2회 연속 갱신 첫 관측으로 기각**, 약한 형태(*동결=집계 지연*)만 유지 (3) **에이전트 스킬에 억제(빼는) 축이 직교 개방** — [[ponytail]](⭐**114,596** 배치 최대)이 기존 4층(전부 더하는 축)에 처음으로 반대 방향을 세움. 5번째 층이 아니라 **두 번째 차원**. ⚠️효과 측정 0건·과소생성 위험 미측정·[[archify]]와 충돌 미확인 (4) **이 볼트의 첫 직접 대조군 등장** — [[claude-obsidian]](⭐14,215·MIT)이 README에 *"Based on Karpathy's LLM Wiki pattern"* 명시, **스택까지 동일**(Obsidian+Claude Code+순수 마크다운). [[LLM-Wiki]] 개념 페이지 **4월 이후 첫 갱신**. ⚠️정리 정확도 수치 0건 (5) **순위 하락 ≠ 열기 감소** — [[archify]] 트렌딩 1위→4위인데 증분은 **+4,834(+24.0%)로 가속**(볼트 단일 레포 일간 최고), [[awesome-gpt-image-2]]는 3위로 내려가며 +11.6%→**+6.8% 감속**. 정반대 조합 2건이 같은 날 (6) ⚠️**[[OpenMontage]] AGPL-3.0** — 62일 만의 재관측 ⭐24,213→**52,780(+118%)**, 배치 5건 중 **유일한 카피레프트**(나머지 전부 MIT) → **코드 이식 금지·설계 참고만**으로 재분류, 라이선스를 실용 등급에 반영하기로 (7) **관심도 급등 ≠ 검증 가능성** — [[VoiceMem]] 업82→**159**(하루 배증·데일리 1위·최근 3배치 최고)·[[WarpSAC]] 78→**134**, 둘 다 초록 재대조에서 **수치·한계 모두 변경 없음**. [[WarpSAC]]는 변종 구성 보강(**WarpSAC-L** Norm ON+clipped double-Q / **WarpSAC-A** Norm OFF+**단일 Q**). 신규 엔티티 0)
total_pages: 958  # 2026-08-28 실측(sources 900 · entities 34 · concepts 18 · synthesis 2 · domains 4 · queries 0)
total_sources: 900  # 2026-08-28 실측 (신규 소스 7건 반영)
---
