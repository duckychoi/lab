...
- **할 것**: [[LLMRouter]](LLM 라우터 개발·평가·배포 통합 인프라·업보트 2,340 최상위)의 코드/문서 공개 시, 라우팅 기준(비용·품질·지연 시그널)과 평가 방식만 발췌해 내 다중 모델·스킬 하네스의 "요청→모델 선택" 게이트 설계에 개념 참고 — 저지연([[DeepSeek-V4-Flash-0731]])·대형([[Kimi-K3]])·온디바이스([[needle]]) 분배 축. ⚠️미래형 arxiv ID(2608.06867)·원문 미검증·UIUC 소속 raw 기재 → 공개 전 대기, 수치·순위 인용 금지.
- **우선순위**: 낮음
- **상태**: 대기
